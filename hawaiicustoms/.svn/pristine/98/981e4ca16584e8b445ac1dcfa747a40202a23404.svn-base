 ...
 *
 * @version 	$Id: class.captcha.php 4165 2009-03-07 09:23:44Z chris.boulton $
...
class ISC_CAPTCHA
{
	...
	//zcs=>is this captcha for admin panel?
	...
	*/
	...
	{
		if($this->ischange){
			$return = $GLOBALS['ShopPath'] . "/captcha.php?" . rand(500,8000) . "&isadmin=".intval($this->isadmin);
			return $return;
		}
		// determine what we need to show for the captcha
		if($this->type == 'dynamic') {
			// single GD generated image
			$return = "<img id='captchaImg' src='" . $GLOBALS['ShopPath'] . "/captcha.php?" . rand(500,8000) . "&isadmin=".intval($this->isadmin)."' alt='img' />";
			$return .= "<br><a href='javascript:void(0)' onClick='ChangeCaptcha()'>Can't see? Try another image</a>";
			return $return;
			...
			for ($i=0;$i<strlen($this->__secret);$i++) {
				$return .= "<img src='" . $GLOBALS['ShopPath'] . "/captcha.php?" . $this->EncodeLetter(substr($this->__secret,$i,1)) . "&isadmin=".intval($this->isadmin)."' />";
			}
			return $return;
		}
	}
