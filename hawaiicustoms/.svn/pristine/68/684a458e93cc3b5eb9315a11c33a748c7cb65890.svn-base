	...
	class CHECKOUT_AMAZONFPS extends ISC_CHECKOUT_PROVIDER
	{
		...
		*/
		public function TransferToProvider()
		{
			...
			$postData .= "?callerKey=".$this->GetValue("accessid");
			$postData .= "&pipelineName=". 'SingleUse';
			$postData .= "&returnURL=". urlencode($returnUrl);
			$postData .= "&callerReference=". 'SenderToken-'.$transactionid."-".microtime(true);
			$postData .= "&paymentReason=". 'Payment for Order id : '.$transactionid;
			$postData .= "&transactionAmount=". $total;
			...
			$signedPostData = CBUISignatureHelper::signUrl($postData, $this->GetValue("secretkey"));

			header('Location: '.$signedPostData);
		}
