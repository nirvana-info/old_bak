...
var FormField = {
	...
	'GetFieldByPrivateId':
		function(formId, privateId)
		{
			...
			for (var i=0; i<formfields.length; i++) {
				if (formfields[i].privateId == privateId) {
					return formfields[i].field;
				}
			}
	...
	'GetFieldDefinitionData':
		function(field)
		{
			...
			if (!field || $(field).parent('dd').length == 0) {
				return false;
			}

			return $(field).parent('dd');
		},
			...
			$('.FormField').each(
				function()
				{
					...
					for (var i=0; i<formIdx.length; i++) {
						if (FormField.GetFieldFormId(this) == formIdx[i]) {
							values[values.length] = {
								'field': $(this),
								'fieldId': FormField.GetFieldId(this),
								'formId': FormField.GetFieldFormId(this),
