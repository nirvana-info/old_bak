	...
	class ISC_MAKEAOFFER
	{
		...
		switch ($action) {
				case "add": {
					...
					if(isset($_SESSION['OFFERCART']['NUM_ITEMS']))  unset($_SESSION['OFFERCART']['NUM_ITEMS']);
					if (isset($_SESSION['OFFERCART']['ITEMS'])) 	unset($_SESSION['OFFERCART']['ITEMS']);
					if (isset($_SESSION['the_offered_price'])) 	unset($_SESSION['the_offered_price']);
					if (isset($_SESSION['you_save'])) 	unset($_SESSION['you_save']);
...
public function FinishOffer()
		{
				...
				if(isset($_SESSION['OFFERCART']['ITEMS'])) unset($_SESSION['OFFERCART']);
				$_SESSION['makeaoffer'] = "No";
				

			

			$GLOBALS['ISC_CLASS_TEMPLATE']->SetPageTitle('Thanks For Your Offer');
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("offer");
...
public function EmptyCartAndKillCheckout()
		...
		private function RemoveGiftWrapping()
		{
			...
			$_SESSION['OFFERCART']['MESSAGE'] = GetLang('GiftWrappingRemoved');
			unset($_SESSION['CHECKOUT']);
			ob_end_clean();
		...
		private function SaveGiftWrapping()
		{
			...
			if(!$this->api->ApplyGiftWrapping($_POST['item_id'], $_POST['giftwraptype'], $_POST['giftwrapping'], $_POST['giftmessage'])) {
				$_SESSION['OFFERCART']['ERROR'] = implode('<br />', $this->api->GetErrors());
				ob_end_clean();
				header("Location: ".GetConfig('ShopPath').'/makeaoffer.php');
				exit;
			}
			...
			$_SESSION['OFFERCART']['MESSAGE'] = GetLang('GiftWrappingApplied');
			unset($_SESSION['CHECKOUT']);
			ob_end_clean();
			header("Location: ".GetConfig('ShopPath').'/makeaoffer.php');
			exit;
		...
		*/
		...
		private function UpdateInCart()
		{
			...
			$_REQUEST['qty'] = array(0 => 1);
			
			if(!isset($_SESSION['OFFERCART']['ITEMS']) or !isset($_POST['offerprice']) ) {
				ob_end_clean();
				header(sprintf("Location: %s/makeaoffer.php", $GLOBALS['ShopPath']));
				die();
			}
			...
			if ( $_POST['offerprice'] > 0 and is_numeric($_POST['offerprice']) )
			{
			$_SESSION['the_offered_price'] = $_POST['offerprice'];
			
	
			}

			// Just selected a shipping method - save accordingly
			if(isset($_REQUEST['selectedShippingMethod']) && isset($_SESSION['OFFERCART']['SHIPPING_QUOTES'])) {
				$shippingCost = 0;
				$handlingCost = 0;
				foreach ($_REQUEST['selectedShippingMethod'] as $vendorId => $shippingMethod) {
					// If an invalid quote was selected, skip it
					if (!isset($_SESSION['OFFERCART']['SHIPPING_QUOTES'][$vendorId][$shippingMethod])) {
						continue;
					}

					$quote = $_SESSION['OFFERCART']['SHIPPING_QUOTES'][$vendorId][$shippingMethod];
				...
				$_SESSION['OFFERCART']['SHIPPING']['CART_HASH'] = $this->api->GenerateCartHash();

				// Save the shipping details
				$_SESSION['OFFERCART']['SHIPPING']['SHIPPING_COST'] = $shippingCost;

				if (count($_REQUEST['selectedShippingMethod']) > 1) {
					$shippingDesc = GetLang('ShippingMethodCombined');
				}
				$_SESSION['OFFERCART']['SHIPPING']['SHIPPING_PROVIDER'] = $shippingDesc;

				if($handlingCost > 0) {
					$_SESSION['OFFERCART']['SHIPPING']['HANDLING_FEE'] = $quote['handling'];
				}
				else {
					unset($_SESSION['OFFERCART']['SHIPPING']['HANDLING_FEE']);
				}
			}
			else {
				unset($_SESSION['CHECKOUT']);
			...
			ob_end_clean();
//			header(sprintf("Location: %s/makeaoffer.php", $GLOBALS['ShopPath']));
			header(sprintf("Location: %s/finalizeoffer.php", $GLOBALS['ShopPath']));
			die();
		}
		...
		private function ShowRegularCart()
		{
			...
			if (isset($_SESSION['OFFERCART']['ERROR'])) {
				$this->cartErrorMessage = $_SESSION['OFFERCART']['ERROR'];
				unset($_SESSION['OFFERCART']['ERROR']);
			}
			...
			if (isset($_SESSION['OFFERCART']['MESSAGE'])) {
				$GLOBALS['CartStatusMessage'] = $_SESSION['OFFERCART']['MESSAGE'];
				unset($_SESSION['OFFERCART']['MESSAGE']);
			}
			else {
		...
		$tplquery = $GLOBALS['ISC_CLASS_DB']->Query("SELECT * FROM [|PREFIX|]coupon_settings where templateid = 1");
			...
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetPageTitle($this->GetPageTitle());
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("makeaoffer");
			$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
		}
		...
		{
			unset($_SESSION['OFFERCART']['SHIPPING']['SHIPPING_COST']);
			unset($_SESSION['OFFERCART']['SHIPPING']['SHIPPING_PROVIDER']);
			unset($_SESSION['OFFERCART']['SHIPPING']['HANDLING_FEE']);
			unset($_SESSION['OFFERCART']['SHIPPING_QUOTES']);
		}
...
//			}
		...
		public function ApplyGiftCertificate()
		{
			...
			if($this->api->ApplyGiftCertificate($_REQUEST['giftcertificatecode'])) {
				...
				$_SESSION['OFFERCART']['MESSAGE'] = GetLang('GiftCertificateAppliedToCart');
			}
			else {
				$_SESSION['OFFERCART']['ERROR'] = implode('<br />', $this->api->GetErrors());
			}
		...
		private function RemoveGiftCertificate()
		{
			...
			$_SESSION['OFFERCART']['MESSAGE'] = sprintf(GetLang('GiftCertificateRemovedFromCart'));
		...
		private function ApplyCoupon()
		{
			...
			if($this->api->ApplyCoupon($_POST['couponcode'])) {
                ...
                $this->api->ApplyCoupon($_POST['couponcode']);//Added by Simha temp fix to avoid having multiple times coupon for same item
				$GLOBALS['ISC_CLASS_MAKEAOFFER']->api->UpdateCartInformation();   
				ob_end_clean();
				header(sprintf("Location:%s/makeaoffer.php?coupon_applied=true", $GLOBALS['ShopPath']));
				die();
			}
			else {
		...
		private function RemoveCoupon()
		{
			if (isset($_REQUEST['couponid'])) {
				...
				// Throw the user back to their cart
				$_SESSION['OFFERCART']['MESSAGE'] = GetLang('CouponRemovedFromCart');
			}
...
$minamtforfreeship =  $shippingModule->rules['upper'][0];*/
		...
		private function ValidateReorder()
		{
			...
			if (isset($_REQUEST['orderid'])) {
				...
				if (!$CustId) {
					$_SESSION['OFFERCART']['ERROR'] = GetLang('MustBeLoggedInToReorder');
					$this->ShowRegularCart();
					exit;
				}
				...
				if($OrdCustId != $CustId) {
					$_SESSION['OFFERCART']['ERROR'] = GetLang('OrderWasntPlacedByYou');
					$this->ShowRegularCart();
					exit;
				}
			...
			// if order Id is not specified
				...
				$_SESSION['OFFERCART']['ERROR'] = GetLang('InvalidOrderId');
				$this->ShowRegularCart();
				exit;
			}
		}
