...
class ISC_ADMIN_STATISTICS_ORDERS extends ISC_ADMIN_STATISTICS
{
	...
	*/
	public function _GetOrdersByRevenue($FromStamp, $ToStamp)
	{
		...
		$mycustomsart = floor($mintotal/10)*10;

		for($i = $mycustomsart; $i < ceil($maxtotal); $i+=$increments) {
			$start = $i;
			$end = ($i + $increments) ;
			// $end = ($i + $increments) - 1;  blessen
...
//added by blessen
//removing range without products
	foreach ($order_list as $key => $value) {
		  if ($order_list[$key]['numorders'] == 0)
			{
