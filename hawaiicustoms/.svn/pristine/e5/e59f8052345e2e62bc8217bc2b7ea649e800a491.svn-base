...
if(typeof(ISSelectReplacement) == 'undefined') {
	var ISSelectReplacement = {
		...
		{        
			...
			var innerhtml = '<ul id="'+element.name.replace('[]','_')+'" >';
            ...
            //change the status as the select box has loaded
            //check whether the request is from product.form.tpl file, we have set a hidden field with id  'hid_chek_file'
		},
		...
		add_option: function(select, option, id)
		{
			...
			html = '<li id="ISSelect'+select.name.replace('[]', '')+'_'+option.value+'" class="'+element_class+'" onselectstart="return false;" style="-moz-user-select: none;" onmouseover="ISSelectReplacement.on_hover(this, \''+id+'\', \'over\');"' +
				'onmouseout=\"ISSelectReplacement.on_hover(this, \''+id+'\', \'out\');" onclick="Checkbulkvalue(this.id);ISSelectReplacement.on_click(this, \''+id+'\');">' + whitespace[0];
			if($(option).hasClass('freeform')) {
				...
				html += '<input type="checkbox" id="ISSelectReplacement_'+option.value+'" name="ISSelectReplacement_'+select.name+'['+extraKey+']'+extra+'" value="'+option.value+'" '+checked+disabled+'" onclick="ISSelectReplacement.on_click(this, \''+id+'\');" />' + label;
			}
			html += '</li>';           
