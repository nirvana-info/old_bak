<?php
	...
	class ISC_ADMIN_CUSTOMERS
	{
		...
		public function __construct()
		{
			...
			$this->photoEntity = new ISC_ENTITY_PICTURE();//zcs=
		...
		public function HandleToDo($Do)
		{
			switch (isc_strtolower($Do)) {
				...
				//<=zcs
				//zcs=>	
				case "viewcustomerphotos":
				{
					if($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Manage_Customers)) {

						$GLOBALS['BreadcrumEntries'] = array(
							GetLang('Home') => "index.php",
							GetLang('Customers') => "index.php?ToDo=viewCustomers",
							GetLang('Photos') => "index.php?ToDo=viewCustomerPhotos",
						);
						if(isset($_GET['searchQuery']))
							$GLOBALS['BreadcrumEntries'][GetLang('SearchResults')] = "index.php?ToDo=viewCustomerPhotos";
						
						if (!isset($_REQUEST['ajax'])) {
							$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintHeader();
						}
						$this->ManageCustomerPhotos();
						if (!isset($_REQUEST['ajax'])) {
							$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintFooter();
						}
					}
				...
				//<=zcs
				//zcs=>
				case "updatecustomerphotostatus":
				{
					if($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Edit_Customers)) {
						$GLOBALS['BreadcrumEntries'] = array(
							GetLang('Home') => "index.php",
		...
		//zcs=>---------------------------------View Customer Photos-----------------------------------
		private function ManageCustomerPhotos($MsgDesc = '', $MsgStatus = ''){
			$numPhotos = 0;

			// Fetch any results, place them in the data grid
			$GLOBALS['PhotoDataGrid'] = $this->ManageCustomerPhotosGrid($numPhotos);
			
			// Was this an ajax based sort? Return the table now
			if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] == 1) {
				echo $GLOBALS['PhotoDataGrid'];
				return;
			}
			
			// Do we need to disable the delete button?
			if (!$GLOBALS["ISC_CLASS_ADMIN_AUTH"]->HasPermission(AUTH_Manage_CustomerPhotos) || $numPhotos == 0) {
				$GLOBALS['DisableDelete'] = "DISABLED";
			}
			// Do we need to disable the expory button?
			if (!$GLOBALS["ISC_CLASS_ADMIN_AUTH"]->HasPermission(AUTH_Export_CustomerPhotos) || $numPhotos == 0) {
				$GLOBALS['DisableExport'] = "DISABLED";
			}
			
			$GLOBALS['PhotoIntro'] = GetLang('ManagePhotosIntro');

			if ($numPhotos > 0) {
				if ($MsgDesc == "" && (isset($_REQUEST['searchQuery']) || isset($_GET['searchId']))) {
					if ($numPhotos == 1) {
						$MsgDesc = GetLang('PhotoSearchResultsBelow1');
					}
					else {
						$MsgDesc = sprintf(GetLang('PhotoSearchResultsBelowX'), $numPhotos);
					}
				...
				if (count($_GET) > 1) {
					if ($MsgDesc == "") {
						$GLOBALS['Message'] = MessageBox(GetLang('NoPhotoResults'), MSG_ERROR);
					}
				}
				else {
					// No actual custoemrs
					$GLOBALS['DisplaySearch'] = "none";
					$GLOBALS['Message'] = MessageBox(GetLang('NoPhotos'), MSG_INFO);
				}
			}
			...
			$GLOBALS['ExportAction'] = "index.php?ToDo=startExport&t=photos";
			...
			$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate("photos.manage");
			$GLOBALS["ISC_CLASS_TEMPLATE"]->ParseTemplate();
		}
		
		private function ManageCustomerPhotosGrid(&$numPhotos){
			// Show a list of photos in a table
			$page = 0;
			$start = 0;
			$numPhotos = 0;
			$numPages = 0;
			...
			$validSortFields = array('customerid', 'custconlastname', 'custconfirstname', 'custconemail',
						 'picid', 'customerid', 'path', 'description', 'filename', 'dateline', 'deleted');
			if (isset($_REQUEST['sortField']) && in_array($_REQUEST['sortField'], $validSortFields)) {
				$sortField = $_REQUEST['sortField'];
				SaveDefaultSortField("ManagePhotos", $_REQUEST['sortField'], $sortOrder);
			} else {
				list($sortField, $sortOrder) = GetDefaultSortField("ManagePhotos", "dateline", $sortOrder);
			}
			...
			$photoResult = $this->_GetPhotoList($start, $sortField, $sortOrder, $numPhotos);

			$numPages = ceil($numPhotos / ISC_CUSTOMERS_PER_PAGE);

			// Add the "(Page x of n)" label
			if ($numPhotos > ISC_CUSTOMERS_PER_PAGE) {
				$GLOBALS['Nav'] = sprintf("(%s %d of %d) &nbsp;&nbsp;&nbsp;", GetLang('Page'), $page, $numPages);

				$GLOBALS['Nav'] .= BuildPagination($numPhotos, ISC_CUSTOMERS_PER_PAGE, $page, sprintf("index.php?ToDo=viewCustomerPhotos%s", $sortURL));
			}
			else {
				$GLOBALS['Nav'] = "";
			}
			...
			BuildAdminSortingLinks($sortLinks, "index.php?ToDo=viewCustomerPhotos&amp;".$searchURL."&amp;page=".$page, $sortField, $sortOrder);

			// Workout the maximum size of the array
			$max = $start + ISC_CUSTOMERS_PER_PAGE;

			if ($max > $GLOBALS["ISC_CLASS_DB"]->CountResult($photoResult)) {
				$max = $GLOBALS["ISC_CLASS_DB"]->CountResult($photoResult);
			}

			if ($numPhotos > 0) {
				while ($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($photoResult)) {
					$GLOBALS['PhotoId'] = (int) $row['picid'];
					$GLOBALS['CustomerUrl'] = sprintf("index.php?ToDo=searchCustomersRedirect&amp;idFrom=%d&amp;idTo=%d", (int) $row['customerid'], (int) $row['customerid']);
					$GLOBALS['UploaderName'] = isc_html_escape($row['uploaderLastName']).', '.isc_html_escape($row['uploaderFirstName']);
					$GLOBALS['Address'] = isc_html_escape($row['address1']).'<br />'.isc_html_escape($row['address2']);
					$GLOBALS['Description'] = isc_html_escape($row['description']);
					$GLOBALS['Comment'] = isc_html_escape($row['adminnote']);
					...
					$GLOBALS['Date'] = CDateExtended($row['dateline']);
					$GLOBALS['Path'] = is_file(ISC_BASE_PATH.$row['path']) ? $row['path'] : '/admin/images/file_missing.png';
					switch($row['deleted']){
						case 0: // not deleted
						{
							if($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Manage_CustomerPhotos)) {
								$GLOBALS['Deleted'] = sprintf("<a title='%s' href='index.php?ToDo=updateCustomerPhotoStatus&amp;picid=%d&amp;deleted=1'><img border='0' src='images/tick.gif'></a>", GetLang('PhotoSetDeletedTip'), $row['picid']);
							} else {
								$GLOBALS['Deleted'] = "<img border='0' src='images/tick.gif'>";
							}
						...
						{
							if($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Manage_CustomerPhotos)) {
								$GLOBALS['Deleted'] = sprintf("<a title='%s' href='index.php?ToDo=updateCustomerPhotoStatus&amp;picid=%d&amp;deleted=0'><img border='0' src='images/cross.gif'></a>", GetLang('PhotoSetNotDeletedTip'), $row['picid']);
							} else {
								$GLOBALS['Deleted'] = "<img border='0' src='images/cross.gif'></a>";
							}
					...
					$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("photos.manage.row");
					$GLOBALS['PhotoGrid'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate(true);
				}
			}

			$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate("photos.manage.grid");
			return $GLOBALS["ISC_CLASS_TEMPLATE"]->ParseTemplate(true);
		}
		
		public function _GetPhotoList($Start, $SortField, $SortOrder, &$NumPhotos, $AddLimit=true){
			$queryWhere = "";
			...
			if (isset($_REQUEST['searchQuery']) && $_REQUEST['searchQuery'] != "") {
				...
				$queryWhere .= "
					AND (
						LOWER(description) LIKE '%" . $search_query . "%' OR
				...
				//LOWER(filename) LIKE '%" . $search_query . "%' OR
				//LOWER(custconemail) LIKE '%" . $search_query . "%' OR
			}
			
			$query = "SELECT DISTINCT
				cust.customerid AS customerid, custconlastname, custconfirstname, CONCAT(custconfirstname, ' ', custconlastname) as custfullname, custconemail,
				picid, path, description, filename, dateline, adminnote, uploaderLastName, uploaderFirstName, address1, address2, deleted 
				FROM
				[|PREFIX|]pic pic
				LEFT JOIN
				[|PREFIX|]customers cust
				ON (pic.customerid=cust.customerid)
				WHERE 1=1 ".$queryWhere." GROUP BY pic.picid";
			...
			$NumPhotos = $GLOBALS['ISC_CLASS_DB']->CountResult($countQuery);
		...
		private function DeleteCustomerPhotos()
		{
			$queries = array();
			if (isset($_REQUEST['photos'])) {
				$message = '';
				list($done, $deleted) = $this->photoEntity->multiDelete($_REQUEST['photos']);
				if (!$done) {
					$err = $GLOBALS["ISC_CLASS_DB"]->GetErrorMsg();
					$message = empty($err) ? GetLang('PhotosDeletedFailed') : $err;
					//$this->ManageCustomerPhotos($err, MSG_ERROR);
				} else {
					$message = GetLang('PhotosDeletedSuccessfully');
					//$this->ManageCustomerPhotos(GetLang('PhotosDeletedSuccessfully'), MSG_SUCCESS);
				}
				$logdata['photoNum'] = count($deleted);
				$logdata['photoIDs'] = implode(',', $deleted);
				// Log this action
				$GLOBALS['ISC_CLASS_LOG']->LogAdminAction($logdata);
				echo "<script language=\"javascript\">".
					"alert('".$message."');".
					"location.href='index.php?ToDo=viewCustomerPhotos';".
					"</script>";
				exit;
			}
			else {
		...
		//zcs=>
		private function UpdatePhotoStatus()
		{
			...
			$oldPhotoData = $this->photoEntity->get($id);
			
			$GLOBALS['ISC_CLASS_DB']->UpdateQuery("pic", $updated, "picid='".$GLOBALS['ISC_CLASS_DB']->Quote($id)."'");
			if ($GLOBALS['ISC_CLASS_DB']->_Error == "") {
				$is_delete = $deleted - $oldPhotoData['deleted'];//-1:is add; 1:is delete
				if($is_delete > 0){//is delete, so image num--
					$this->customerEntity->DecreaseImageNum($oldPhotoData['customerid']);
				}else if($is_delete < 0){//is add, so image num++
					$this->customerEntity->IncreaseImageNum($oldPhotoData['customerid']);
				}
				
				if($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Manage_CustomerPhotos)) {
					// Log this action
					$GLOBALS['ISC_CLASS_LOG']->LogAdminAction(array(
						'photo_id' => $id,
						'set_deleted_to' => $deleted,
					));

					$this->ManageCustomerPhotos(GetLang('PhotoStatusSuccessfully'), MSG_SUCCESS);
				}
				else {
					$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->DoHomePage(GetLang('PhotoStatusSuccessfully'), MSG_SUCCESS);
				}
			}
			else {
				$err = '';
				if ($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Manage_CustomerPhotos)) {
					$this->ManageCustomerPhotos(sprintf(GetLang('ErrPhotoStatusNotChanged'), $err), MSG_ERROR);
				} else {
					$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->DoHomePage(sprintf(GetLang('ErrPhotoStatusNotChanged'), $err), MSG_ERROR);
				}
			}
		}
		//<=zcs
		...
		private function ManageCustomersGrid(&$numCustomers)
		{
			...
			//zcs= add "status"
			// johnny add "isguest"
			$validSortFields = array('customerid', 'custconlastname', 'custconfirstname', 'custconemail', 'custconphone', 'custconcompany', 'custdatejoined', 'numorders', 'custstorecredit', 'status', 'isguest');
			if (isset($_REQUEST['sortField']) && in_array($_REQUEST['sortField'], $validSortFields)) {
				$sortField = $_REQUEST['sortField'];
			...
			if ($numCustomers > 0) {
				while ($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($customerResult)) {
					...
					switch($row['status']){
						...
						{
							if($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Edit_Customers)) {
								$GLOBALS['Status'] = sprintf("<a title='%s' href='index.php?ToDo=updateCustomerStatus&amp;userId=%d&amp;status=1'><img border='0' src='images/cross.gif'></a>", GetLang('CustomerActiveTip'), $row['customerid']);
							} else {
						...
						{
							if($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Edit_Customers)) {
								$GLOBALS['Status'] = sprintf("<a title='%s' href='index.php?ToDo=updateCustomerStatus&amp;userId=%d&amp;status=0'><img border='0' src='images/tick.gif'></a>", GetLang('CustomerInactiveTip'), $row['customerid']);
							} else {
					...
					//<=zcs
					// 2011-07-04 johnny modify
					if($row['isguest'] == 0 && $GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Login_Customers)){
						$GLOBALS['isguest'] = "NO";
						$GLOBALS['LoginCustomerLink']= sprintf("<a href='index.php?ToDo=loginCustomer&amp;customerId=%d' class='Action' target='_blank'>%s</a>", $row['customerid'], GetLang('CustomerLoginAsThem'));
					}else{
						$GLOBALS['isguest'] = "YES";
						$GLOBALS['LoginCustomerLink'] = '';
					}
		...
		public function _GetCustomerList($Start, $SortField, $SortOrder, &$NumCustomers, $AddLimit=true,$isAjaxSearch=false)
		{
			...
			//zcs=add "status"
			   ...
			   $query = "
				SELECT DISTINCT customerid, custconlastname, custconfirstname, CONCAT(custconfirstname, ' ', custconlastname) as custfullname, custconemail, custconphone, custconcompany, custdatejoined, custgroupid, COUNT(o.orderid) AS numorders, custstorecredit, custnotes, status, isguest
				FROM [|PREFIX|]customers
				LEFT JOIN [|PREFIX|]orders o ON (ordcustid=customerid) AND o.ordstatus !=  '0' ";
			
			   //(if(subemail<>'NULL','Yes','No')) as pemail,
		  ...
		  # Added not to include "Incomplete orders(0)" for the fetched records -- Baskaran
			...
			if($isAjaxSearch){
				$query .=" and isguest=0";
				$countQuery .="and isguest=0";
			}
			
			$query .= $queryWhere;
			$countQuery .= $queryWhere." GROUP BY customerid";
		...
		public function BuildWhereFromVars($array)
		{
			...
			if (isset($array['searchQuery']) && $array['searchQuery'] != "") {
				// PostgreSQL is case sensitive for likes, so all matches are done in lower case
				$search_query = $GLOBALS['ISC_CLASS_DB']->QuoteEx(trim(isc_strtolower($array['searchQuery'])));//zcs=Fix BUG,escape additional characters
				$queryWhere .= "
					AND (
		...
		*/
		...
		public function AddCustomerStep1($MsgDesc = "", $MsgStatus = "", $PreservePost=false)
		{
			...
			if($PreservePost == true) {
				...
				$GLOBALS['SubscribedChecked'] = $customer['subscribed'] == 1 ? 'checked' : '';
			}else{
				$GLOBALS['SubscribedChecked'] = 'checked';
			}
		...
		public function AddCustomerStep2()
		{
			...
			$StoreCustomer['subscribed'] = $Postcustomer['subscribed'];
			...
			if (isId($customerId)) {
				//wirror20110315: handling subcribed/unsubscribed customer
				$this->HandleSubscribe($StoreCustomer);
		...
		public function EditCustomerStep1($MsgDesc = "", $MsgStatus = "", $PreservePost=false)
		{
			...
			$GLOBALS['SubscribedChecked'] = $customer['subscribed']==1 ? 'checked' : '';
			$GLOBALS['CustomerStatusOptions'] = $this->GetCustomerStatusAsOptions($customer['status']);//zcs=
		...
		public function EditCustomerStep2()
		{
			...
			$StoreCustomer['customergroupid'] = $PostCustomer['custgroupid'];
			$StoreCustomer['subscribed'] = $PostCustomer['subscribed'];
			$StoreCustomer['status'] = $PostCustomer['status'];//zcs=
		...
		private function _GetCustomerData($CustomerId=null, $entitiesData=true)
		{
			...
			if (!isId($CustomerId)) {
				...
				$customer['subscribed'] = isset($_POST['subscribed']) ? 1 : 0;
				$customer['status'] = intval($_POST['status']);//zcs=
			} else {
		...
		*/
		...
		//zcs=>
		public function GetCustomerStatusAsOptions($SelectedStatus=0){
			$customerStatus = array(
				array('status' => 1, 'title' => 'Active'),
				array('status' => 0, 'title' => 'Inactive'),
			);
			$options = "";
			...
			foreach($customerStatus as $status) {
				...
				$options .= sprintf("<option value='%d' %s>%s</option>", (int) $status['status'], $sel, isc_html_escape($status['title']));
			}
...
";
		 ...
		 * HandleSubscribe
		 * @author Wirror Yin
		 * @param Array $StoreCustomer
		 *
		 * @return Array $result
		 */
		public function HandleSubscribe($StoreCustomer){
			$GLOBALS['ISC_CLASS_SUBSCRIBE'] = GetClass('ISC_SUBSCRIBE');
			$first_name = $StoreCustomer['firstname'];
			$email = $StoreCustomer['email'];

			if($StoreCustomer['subscribed'] == 1){ // Subscribe customer
				
				$result = $GLOBALS['ISC_CLASS_SUBSCRIBE']->AddSubscriberToNewsletter($first_name, $email, true);
			
			}else{ // Unsubscribe customer
				
				$result = $GLOBALS['ISC_CLASS_SUBSCRIBE']->UnsubscribeEmail($first_name, $email, 0, 0, true);
			}
		}
		...
		private function UpdateCustomerStatus()
		{
			...
			$GLOBALS['ISC_CLASS_DB']->UpdateQuery("customers", $updatedUser, "customerid='".$GLOBALS['ISC_CLASS_DB']->Quote($userId)."'");
			if ($GLOBALS['ISC_CLASS_DB']->_Error == "") {
				if($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Manage_Customers)) {
					$query = sprintf("SELECT username FROM [|PREFIX|]users WHERE customerid='%d'", $userId);
					$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
					...
					$this->ManageCustomers(GetLang('CustomerStatusSuccessfully'), MSG_SUCCESS);
				}
				else {
					$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->DoHomePage(GetLang('CustomerStatusSuccessfully'), MSG_SUCCESS);
				}
			}
			else {
				$err = '';
				if ($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Manage_Customers)) {
					$this->ManageCustomers(sprintf(GetLang('ErrCustomerStatusNotChanged'), $err), MSG_ERROR);
				} else {
					$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->DoHomePage(sprintf(GetLang('ErrCustomerStatusNotChanged'), $err), MSG_ERROR);
				}
			}
		}
		//<=zcs
