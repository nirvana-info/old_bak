...
var ProductSelect = {
   ...
   LoadProducts: function() {
         ...
         var brandid =     document.getElementById("productbrand").value;
         var searchParams = 'brandid='+brandid+"&seriesid=";  
         var multidropdown = document.getElementById("productseries");
           
         for (i = 0; i < multidropdown.options.length; i++) {
                   if (multidropdown.options[i].selected == true) {
                        searchParams+= multidropdown.options[i].value + ",";
                         }
               }
    ...
    LinksLoaded: function(xml) {
        ...
        if(status == 0) {
            ...
            if(%%GLOBAL_ProductSelectSingle%% == 1) {
                ...
                $('#results').html('<select style="width: 800px;height: 200px;" multiple="multiple" name="products" id="prodSelect" class="ISSelectReplacement"></select>');
            }
            ...
            $('product', xml).each(function() {
                ...
                cat = $('catName',this).text();  
                ser =  $('seriesName',this).text(); 
                    var temp = '' ;  
                    if (cat != "")    temp+=  " , "+cat; 
                    if (ser != "")    temp+=  " , "+ser;  
                ...
                $('<option>')
                    .val(productId)
                    .html(productName+" , $"+prodprice+" , "+partNumber + temp)      
                    .click(function() {
...
<script type="text/javascript">  
   function select_all()
        ...
        function on_click_temp(element, id,type)
        {
      ...
      <tr>
        <td>
           ...
           <table class="Panel" id="brandlist" style="display:none;">
              <tr >
                <td class="Heading2" colspan="3">&nbsp;&nbsp; %%LNG_ProductSelectByBrand%%</td>
              </tr>
                <tr>
                    <td colspan="3">
                ...
                <tr>        
                    ...
                    </td>    <td class="Intro"> Use Control Key for multiple selection </td>                                                                                         <td> <input name="sel" value="List Products" type = button   onclick="ProductSelect.LoadProducts();" />  </td> 
                </tr>
            </table>
...
<!--                    <td><input type='text' name='searchQuery' id='searchQuery' class='Field250' style='width: 100%' onkeyup='if(!this.oldValue || this.oldValue != this.value) { this.oldValue = value; ProductSelect.LoadLinks("category="+current_category); }' /></td>-->
         ...
         <a href='javascript:select_all();'>Check All </a>&nbsp;&nbsp; <a href='javascript:unselect_all();'>Uncheck All </a> 
      </td>
