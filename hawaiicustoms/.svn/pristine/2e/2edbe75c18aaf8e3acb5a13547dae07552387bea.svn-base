...
//            echo "<noscript>".$_SERVER['REQUEST_URI']."test".$_SERVER['SCRIPT_NAME']."</noscript>";
			$uri = explode("/", $_SERVER['PHP_SELF']);
			$page = $uri[count($uri)-1];
			
			
//			$cartpage = strpos($_SERVER['REQUEST_URI'], '/cart');
//			$sweepstakespage = strpos(strtolower($_SERVER['REQUEST_URI']), '/sweepstakes');
            $cartpage = strpos(isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '', '/cart');
            $sweepstakespage = strpos(strtolower(isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI']:''), '/sweepstakes');
			
			if (in_array($page, $ssl_pages) || $cartpage !== false || $sweepstakespage !== false) {
				...
				if((!isset($_SERVER['HTTPS']) || $_SERVER['HTTPS'] == "off") && $_SERVER['REQUEST_METHOD'] == "GET") {
					...
					if ($sweepstakespage !== false )
                    {
                        $location = "sweepstakes";
                    }
