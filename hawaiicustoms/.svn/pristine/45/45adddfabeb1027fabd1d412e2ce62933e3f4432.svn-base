	...
	class ISC_WISHLIST
	{
		...
		*/
		private function DisplayAddWishListForm()
		{
            ...
            $wishlistredirect ='';
            if($GLOBALS['EnableSEOUrls'] == 1) {
                $wishlistredirect = $GLOBALS['ShopPath']."/wishlist";
                $actionlist = $GLOBALS['ShopPath'].'/wishlist/addwishlist';
            }
            else {
                $wishlistredirect = $GLOBALS['ShopPath']."/wishlist.php";
                $actionlist = $GLOBALS['ShopPath'].'/wishlist.php?action=addwishlist';
            }
            /**/
			if (isset($_POST['submit'])) {
				...
				if ($this->AddWishList($customerid, $_POST['wishlistname'], $ispublic)) {
					ob_end_clean();
					header("Location: ".$wishlistredirect);  # Baskaran
					return true;
				}
			}
			...
			$GLOBALS['WishListAction'] = $actionlist; # Baskaran
		...
		*/
		private function AddWishList($customerid, $wishlistname = 'My Wish List', $ispublic=0)
		{
			...
			if ($ispublic!=0 && $ispublic!=1) {
				$this->HandleMessages(GetLang('InvalidePublicWishlistValue'), MSG_ERROR);
				return false;
			}
		...
		*/
		private function DeleteWishLists()
		{
            ...
            /* Baskaran */
            if($GLOBALS['EnableSEOUrls'] == 1) { 
                        $_GET[$GLOBALS['PathInfo'][2]] = $GLOBALS['PathInfo'][3];
                        $wishlistredirect = $GLOBALS['ShopPath']."/wishlist";
                }     
                else {
                    $wishlistredirect = $GLOBALS['ShopPath']."/wishlist.php";
			...
			// bulk delete wish lists
			    ...
			    FlashMessage(GetLang('SuccessDeleteWishList'), MSG_SUCCESS);
                header("Location:".$wishlistredirect);
                exit;
		}
		...
		*/
		private function DisplayEditWishListForm()
		{
            ...
            $wishlistaction = '';
             if($GLOBALS['EnableSEOUrls'] == 1) { 
                 $_GET[$GLOBALS['PathInfo'][2]] = $GLOBALS['PathInfo'][3];
                 $wishlistaction = '%%GLOBAL_ShopPath%%/wishlist/editwishlist/wishlistid/';
             }
             else {
                 $wishlistaction = '%%GLOBAL_ShopPath%%/wishlist.php?action=editwishlist&wishlistid=';
             }
             /**/
			if (isset($_GET['wishlistid']) && is_numeric($_GET['wishlistid'])) {
				...
				if ($wishlist['ispublic']==1) {
                 ...
                 /* Baskaran */
				    $GLOBALS['WishListAction'] = $wishlistaction.(int)$GLOBALS['WishListID'];
                 /**/
				$GLOBALS['HideWishListAddFrom'] = '';
				$GLOBALS['HideWishListItems'] = 'display:none;';
				$GLOBALS['HideWishLists']  = 'display:none;';
		...
		*/
		private function DisplayPublicWishListItems()
		{
            /* Baskaran */
             if($GLOBALS['EnableSEOUrls'] == 1) { 
                 $_REQUEST[$GLOBALS['PathInfo'][1]] = $GLOBALS['PathInfo'][2];
             }
             /**/
		...
		*/
		private function DisplayWishListItems($MsgDesc = "", $MsgStatus = "", $wishlistid=0)
		{
            ...
            /* Baskaran */
             if($GLOBALS['EnableSEOUrls'] == 1) { 
                 $_REQUEST[$GLOBALS['PathInfo'][2]] = $GLOBALS['PathInfo'][3];
                 $publicwishlist = '/wishlist/publicwishlist/';
                 $GLOBALS['removelink'] = $GLOBALS['ShopPath']."/wishlist/remove/item_id/%%GLOBAL_ItemId%%";
             }
             else {
                 $publicwishlist = '/wishlist.php?publicwishlist=';
                 $GLOBALS['removelink'] = $GLOBALS['ShopPath']."/wishlist.php?action=remove&item_id=%%GLOBAL_ItemId%%";
			...
			if (empty($wishlist)) {
				...
				// if user trying to view wish list which is not public
					...
					if ($wishlist['ispublic']==1) {
						$GLOBALS['PublicWishListUrl'] = GetConfig('ShopPath').$publicwishlist.$wishlist['wishlisttoken']; # Baskaran
		...
		private function DisplayShareWishList()
		{
			...
			if (empty($wishlist)) {
				...
				if (!$this->IsValidWishListUser($_GET['wishlistid'], $customerid)) {
					...
					if ($wishlist['ispublic']==1) {
						$GLOBALS['PublicWishListUrl'] = GetConfig('ShopPath').$publicwishlist.$wishlist['wishlisttoken'];
						$GLOBALS['HideShareWishList'] = '';
		...
		*/
		private function _GetWishListItemsByID(&$NumItems, $wishlistid=0)
		{
			...
			$query = "
				SELECT w.wishlistitemid, p.*, FLOOR(prodratingtotal/prodnumratings) AS prodavgrating, imageisthumb, imagefile, ".GetProdCustomerGroupPriceSQL()."
				...
				WHERE w.wishlistid='".(int)$wishlistid."' AND (imageisthumb=1 OR ISNULL(imageisthumb))
		...
		*/
		private function _GetWishListItemsByToken($wishlisttoken="")
		{
			...
			$query = "
				SELECT wi.wishlistitemid, p.*, FLOOR(prodratingtotal/prodnumratings) AS prodavgrating, imageisthumb, imagefile, ".GetProdCustomerGroupPriceSQL()."
				...
				LEFT JOIN [|PREFIX|]product_images pi ON (p.productid=pi.imageprodid)
				WHERE w.wishlisttoken='".$GLOBALS['ISC_CLASS_DB']->Quote($wishlisttoken)."' AND (imageisthumb=1 OR ISNULL(imageisthumb))
				".GetProdCustomerGroupPermissionsSQL()."
			";
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		...
		*/
		private function MyWishLists($MsgDesc = "", $MsgStatus = "")
		{
			...
			$wishlists = $this->LoadCustomerWishLists($customerid);
            if($GLOBALS['EnableSEOUrls'] == 1) {
                $addlink = $GLOBALS['ShopPath'].'/wishlist/addwishlist';
            }
            else {
                $addlink = $GLOBALS['ShopPath'].'/wishlist.php?action=addwishlist';
            }
			...
			if(!is_numeric($MsgStatus)) {
				...
				if(count($wishlists) == 0) {    
					$MsgDesc = sprintf(GetLang('NoWishListsMessage'),$addlink);
			...
			foreach ($wishlists as $wishlist) {
                ...
                /* Baskaran */
                if($GLOBALS['EnableSEOUrls'] == 1) {
                    $GLOBALS['WishListEdit'] = $GLOBALS['ShopPath']."/wishlist/editwishlist/wishlistid/%%GLOBAL_WishListID%%";
                    $GLOBALS['WishListNameItems'] = "%%GLOBAL_ShopPath%%/wishlist/viewwishlistitems/wishlistid/%%GLOBAL_WishListID%%";
                    $GLOBALS['WishListDeleteLink'] = $GLOBALS['ShopPath'].'/wishlist/deletewishlist/wishlistid/';
                    $GLOBALS['WishListBulkDeleteLink'] = "%%GLOBAL_ShopPath%%/wishlist/deletewishlist";
                }
                else {
                    $GLOBALS['WishListEdit'] = $GLOBALS['ShopPath']."/wishlist.php?action=editwishlist&amp;wishlistid=%%GLOBAL_WishListID%%";
                    $GLOBALS['WishListNameItems'] = "%%GLOBAL_ShopPath%%/wishlist.php?action=viewwishlistitems&amp;wishlistid=%%GLOBAL_WishListID%%";
                    $GLOBALS['WishListDeleteLink'] = $GLOBALS['ShopPath'].'/wishlist.php?action=deletewishlist&wishlistid=';
                    $GLOBALS['WishListBulkDeleteLink'] = "%%GLOBAL_ShopPath%%/wishlist.php?action=deletewishlist";
                }
               
				if ($wishlist['ispublic']==1) {
					$GLOBALS['WishListShared'] = GetLang('SearchLangYes');
                    if($GLOBALS['EnableSEOUrls'] == 1) { 
					    $GLOBALS['ShareWishListLink'] = '<a href="%%GLOBAL_ShopPath%%/wishlist/sharewishlist/wishlistid/'.$GLOBALS['WishListID'].'">'.GetLang('Share').'</a>';
                    }
                    else {
                        $GLOBALS['ShareWishListLink'] = '<a href="%%GLOBAL_ShopPath%%/wishlist.php?action=sharewishlist&amp;wishlistid='.$GLOBALS['WishListID'].'">'.GetLang('Share').'</a>';
                    }
                     /**/
				} else {
		...
		*/
		private function RemoveItemFromWishList()
		{
            ...
            if($GLOBALS['EnableSEOUrls'] == 1) {
                $removelink = $GLOBALS['ShopPath']."/wishlist/remove/item_id/";
                $actionlink = $GLOBALS['ShopPath']."/wishlist/viewwishlistitems/wishlistid/";
                $_GET[$GLOBALS['PathInfo'][2]] = $GLOBALS['PathInfo'][3];
            }
            else {
                $removelink = $GLOBALS['ShopPath']."/wishlist.php?action=remove&item_id=";
                $actionlink = $GLOBALS['ShopPath']."/wishlist.php?action=viewwishlistitems&wishlistid=";
            }
			if(isset($_GET['item_id']) && is_numeric($_GET['item_id'])) {
				...
				if (empty($row)) {
					$remove_link = $removelink.$item_id;
				...
				if(!$GLOBALS['ISC_CLASS_DB']->Query($query)) {
					...
					ob_end_clean();
					header("Location: ".$actionlink.$row['wishlistid']);
					die();
				}
			}
			else {
