<?php           
	class ISC_ADMIN_QVALUE_ASSOCIATIONS
	{
        ...
        public function _GetQualifierAssociationList(&$Query, $Start, $SortField, $SortOrder, &$NumResults)
        {
            ...
            $query = "
                SELECT qa.associd, qa.categoryid, qa.qualifierid, qa.displayname, mn.column_name 
		...
		public function _GetQValueAssociationList(&$Query, $Start, $SortField, $SortOrder, &$NumResults)
		{
            ...
            */
            ...
            $query = "
                SELECT qva.qvalueassocid, qva.categoryid, qva.associd, qa.qualifierid, qva.displayname, qva.qvalue 
                FROM [|PREFIX|]qvalue_associations qva 
                LEFT JOIN [|PREFIX|]categories c ON qva.categoryid = c.categoryid
                LEFT JOIN [|PREFIX|]qualifier_associations qa ON qva.associd = qa.associd
                LEFT JOIN [|PREFIX|]qualifier_names qn ON qn.qid = qa.qualifierid 
            ";                                   //qva.comments, c.catname, qn.column_name, 
        ...
        {    
            ...
            //Added by Simha to handle Irregular values in bedsize and Cabsize    
            if($qualifiername=='VQbedsize')    {                  
                $bs_query = "SELECT DISTINCT `bedsize_generalname`  
                            FROM [|PREFIX|]import_variations iv
                            WHERE `bedsize_generalname`!=''";
            ...
            //Added by Simha to handle Irregular values in bedsize and Cabsize ends
