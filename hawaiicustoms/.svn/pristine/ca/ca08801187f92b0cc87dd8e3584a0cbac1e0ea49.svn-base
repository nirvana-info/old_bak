<?php
	...
	class ISC_ADMIN_BANNERS
	{
		...
		private function AddBannerStep1()
		{
			...
			$GLOBALS['InEntireDay'] = 'checked="checked"';
			...
			$GLOBALS['WeeklyOptions'] = $this->GetWeekly();
		...
		*/
		private function _CommitBanner($ExistingBannerId = 0)
		{
			...
			$showtime = $_POST["bannershowtime"];
			$displayweekly = $_POST["ISSelectReplacement_displayweekly"];
			...
			$banner = array(
				...
				"showtime" => $showtime,
				"displayweekly" => empty($displayweekly) ? '' : implode(',', $displayweekly)
			);
		...
		private function EditBannerStep1()
		{
			if (isset($_GET['bannerId']) && is_numeric($_GET['bannerId'])) {
				...
				if ($banner = $this->GetBannerById($banner_id)) {
					...
					$GLOBALS['ControlScript'] = isc_html_escape($banner['controlscript']);
					...
					switch ($banner['page']) {
						...
						case "entire_site": {
							$GLOBALS['IsEntireSite'] = 'checked="checked"';
							$GLOBALS['SelectedJS'] = "selected_page='entire_site';";
							break;
						}
					}
					...
					else if($banner['datetype'] == "custom"){
						$GLOBALS['IsCustomDate'] = 'checked="checked"';
					}else{
						$GLOBALS['IsWeeklyDate'] = 'checked="checked"';
					}

					if($banner['datetype'] == "custom") {
						$from_day = date("d", $banner['datefrom']);
						$from_month = date("m", $banner['datefrom']);
						$from_year = date("Y", $banner['datefrom']);
						$to_day = date("d", $banner['dateto']);
						$to_month = date("m", $banner['dateto']);
						$to_year = date("Y", $banner['dateto']);
						...
						$GLOBALS['FromDayOptions'] = $this->GetDays(date("d"));
						$GLOBALS['FromMonthOptions'] = $this->GetMonths(date("m"));
						$GLOBALS['FromYearOptions'] = $this->GetYears(date("Y"));
						$GLOBALS['ToDayOptions'] = $this->GetDays(date("d"));
						...
						if ($banner['datetype'] == "always") {
							...
							$displayweekly = $banner['displayweekly'];
							if(empty($displayweekly))
								$selected = array(-1);
							else
								$selected = explode(',', $displayweekly);
							$GLOBALS['WeeklyOptions'] = $this->GetWeekly($selected);
							
							$GLOBALS['ShowCustomDate'] = "$('#DisplayWeekly').css('display', '');";
						}
					}
					
					if ($banner['showtime'] == "entireday") {
						$GLOBALS['InEntireDay'] = 'checked="checked"';
					}
					else {
						$GLOBALS['InCustomerService'] = 'checked="checked"';
					}
		 ...
		 *  the array can be charaters from monday to sunday,or digit from 0 to 6
		 * @return String $options
		 */
		private function GetWeekly($selected=array(-1)){
			$weeklyArr = array("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday");
			$options = '';
			foreach($weeklyArr as $key=>$day){
				$saveDay = strtolower($day);
				if(in_array($saveDay, $selected, true) || in_array($key, $selected, true)){
					$select = 'selected';
				}else{
					$select = '';
				}
				
				$options .= sprintf("<option value='%s' %s>%s</option>", $saveDay, $select, $day);
			}
