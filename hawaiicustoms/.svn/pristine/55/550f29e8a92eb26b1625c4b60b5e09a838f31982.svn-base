...
Private function SaveCouponSettings()
		{
				...
				$sql1 = " update isc_categories set offer = 'no' where catparentid = 0 ";
				mysql_query($sql1);

				$sql1 = " update isc_categories set offer = 'yes' where categoryid in (".$comma_separated_cats.") and catparentid = 0 ";
				mysql_query($sql1);

				$sql2 = " update isc_categories set offer = 'no' where catparentid != 0 ";
				mysql_query($sql2);

				$sql2 = " update isc_categories set offer = 'yes' where categoryid in (".$comma_separated_subcats.") and catparentid != 0 ";
				mysql_query($sql2);
...
$query2 = " SELECT `categoryid` , `catname`   FROM [|PREFIX|]categories where offer = 'yes' and catparentid	 = 0  ";
...
while ($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($result2))
	...
	$query1 = " SELECT `categoryid` , `catname`   FROM [|PREFIX|]categories where offer = 'yes' and catparentid	 =  ".$row['categoryid'];
	...
	while ($row1 = $GLOBALS["ISC_CLASS_DB"]->Fetch($result1))
		...
		if (mysql_num_rows($result1) == 0 ) $temptable.= "All categories";
	$temptable.= "</div></td></tr>";
