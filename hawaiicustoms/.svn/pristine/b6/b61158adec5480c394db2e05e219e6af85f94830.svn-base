<?php
	...
	class ISC_ADMIN_ABTESTINGCUSTOMCONTENTS
	{
		...
		public function HandleToDo($Do)
		{
			...
			switch (isc_strtolower($Do))
			{
				...
				case "updatecateabtestingcustom":
				{
					if ($GLOBALS["ISC_CLASS_ADMIN_AUTH"]->HasPermission(AUTH_Edit_Categories)) {
				...
				case "deletecateabtestingcustom":
				{
					if ($GLOBALS["ISC_CLASS_ADMIN_AUTH"]->HasPermission(AUTH_Edit_Categories)) {
						$itemIds = $_GET['itemId'];
				...
				case "deletecateabtestingcustommulti":
				{
					if ($GLOBALS["ISC_CLASS_ADMIN_AUTH"]->HasPermission(AUTH_Edit_Categories)) {
		...
		public function EnableCustomPageItem(){
			...
			if ($GLOBALS['ISC_CLASS_DB']->UpdateQuery("abtesting_custom_products", $updatedStatus, "itemid='".$GLOBALS['ISC_CLASS_DB']->Quote($itemid)."'")) {
		...
		public function GetCustomItemById($itemId){
			$query = "SELECT * FROM [|PREFIX|]abtesting_custom_products WHERE itemid='$itemId'";
		...
		private function DeleteCustomItems($itemIds){
			$GLOBALS['ISC_CLASS_DB']->DeleteQuery('abtesting_custom_products', "WHERE itemId IN ('".$itemIds."')");
			...
			if ($err != "") {
				...
				$location= 'index.php?ToDo=listCateabtestingcustom&customContentId='.$_GET['contentId'].'&catId='.$_GET['catId'].'&pid='.$_GET['pid'];
		...
		public function UpdateCustomPageItem()
		{
			...
			if ($result) {
				...
				if(isset($_GET['AddAnother'])) {
					$location = 'index.php?ToDo=editCateabtestingcustom&itemId='.$_POST['itemId'].'&catId='.$_POST['catId'].'&pid='.$_POST['pid'];
				}
				else {
					$location= 'index.php?ToDo=listCateabtestingcustom&customContentId='.$_POST['contentId'].'&catId='.$_POST['catId'].'&pid='.$_POST['pid'];
				}
		...
		public function SaveCustomPageItem()
		{
			...
			if ($result) {
				...
				if(isset($_GET['AddAnother'])) {
					$location = 'index.php?ToDo=showCateabtestingcustom&contentId='.$_POST['contentId'].'&catId='.$_POST['catId'];
				}
				else {
					$location= 'index.php?ToDo=listCateabtestingcustom&customContentId='.$_POST['contentId'].'&catId='.$_POST['catId'].'&pid='.$_POST['pid'];
				}
		...
		{
			return $GLOBALS['ISC_CLASS_DB']->InsertQuery('abtesting_custom_products', $customItem);
		}
		...
		public function updateCustomProducts($itemid, $customItem)
		{
			...
			// Update the status for this order review request
			if ($GLOBALS['ISC_CLASS_DB']->UpdateQuery("abtesting_custom_products", $customItem, "itemid='".$GLOBALS['ISC_CLASS_DB']->Quote($itemid)."'")) {
				// Log this action if we are in the control panel
				if (defined('ISC_ADMIN_CP')) {
					//$GLOBALS['ISC_CLASS_LOG']->LogAdminAction($orderId, $statusName);
				}
		...
		{
			return $GLOBALS['ISC_CLASS_DB']->InsertQuery('abtesting_custom_contents', $customItem);
		}
		...
		private function ListCustomPage()
		{
			...
			$GLOBALS["CreateCustomItemAction"] = "showCateabtestingcustom&contentId=$contentId&catId=$catId&pid=$pageId";
			$GLOBALS["FormAction"] = "savecateabtestingcustom";
			$GLOBALS["ContentId"] = $contentId;
			$GLOBALS["CatId"] = $catId;
			$GLOBALS['PageId'] = $pageId;
			
            /*$query = "SELECT *
            		  FROM [|PREFIX|]abtesting_custom_products cp 
            		  INNER JOIN [|PREFIX|]abtesting_custom_contents cc on cc.contentid=cp.contentid
			...
			while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
				...
				*/
				...
				if ($GLOBALS["ISC_CLASS_ADMIN_AUTH"]->HasPermission(AUTH_Edit_Categories)) {
					$GLOBALS['EditLink'] = sprintf("<a title='%s' href=\"index.php?ToDo=editCateabtestingcustom&amp;itemId=%s&amp;contentId=%s&amp;catId=%s&amp;pid=%s\" class=\"bodylink\">%s</a>", GetLang('EditCustomPageItem'), $row['itemid'], $contentId, $catId, $pageId, GetLang('Edit'));
				} else {
				...
				if ($GLOBALS["ISC_CLASS_ADMIN_AUTH"]->HasPermission(AUTH_Delete_Categories)) {
					$GLOBALS['DeleteLink'] = sprintf("<a title='%s' onclick='return deleteConfirm();' href=\"index.php?ToDo=deleteCateabtestingcustom&amp;itemId=%s&amp;contentId=%s&amp;catId=%s&amp;pid=%s\" class=\"bodylink\">%s</a>", GetLang('DeleteCustomPageItem'), $row['itemid'], $contentId, $catId, $pageId, GetLang('Delete'));
				} else {
			...
			$GLOBALS["ISC_CLASS_TEMPLATE"]->SetTemplate("abtesting.category.custompage.list");
