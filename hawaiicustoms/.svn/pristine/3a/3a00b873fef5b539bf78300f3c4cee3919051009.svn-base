...
{
	//$GLOBALS['ISC_CLASS_DB']->InsertQuery("orderlogs",22);
	...
	{
		//mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( 'xmlresponse: ".print_r($xml_response,true)."')");		
		...
		switch ($root) {
			...
			{
				 //mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( '1: ".print_r($data,true)."')");
			...
			{
				//mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES (  '2: ".print_r($data,true)."')");
			...
			{
				//mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( '3: ".print_r($data,true)."')");
				$this->HandleAmountNotification($root, $data);
				$this->response->SendAck();
				break;
			}
			...
			{
				//mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( '4: ".print_r($data,true)."')");
				$this->HandleAmountNotification($root, $data);
				$this->response->SendAck();
				break;
			}
			...
			{
				//mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( '5: ".print_r($data,true)."')");
			...
			{
				//mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( '6: ".print_r($data,true)."')");
				$this->HandleMerchantCallback($data);
				break;
			}
			default:
			{
				//mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( '7: ".print_r($data,true)."')");
		...
		foreach($addresses as $curr_address) {
			...
			if(isset($data[$root]['calculate']['shipping']['method'])) {
				...
				foreach($shipping as $curr_ship) {
					...
					else if( isset($data[$root]['calculate']['merchant-code-strings']) && !isset($data[$root]['calculate']['merchant-code-strings']['merchant-code-string'] ) ) {
	 ...
	 **/
	private function ValidateCouponCode($code, $data, $count_cc=0)
	{
		$error = '';
		$root = 'merchant-calculation-callback';
		...
		$numCouponsApplied++;
		//mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( 'numCouponsApplied: ".print_r($numCouponsApplied,true)."')");
		...
		$sescouponvalue = $GLOBALS['ISC_CLASS_CART']->api->GetAppliedCouponCodes();
		//mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( 'coupons: ".print_r($sescouponvalue,true)."')");
        if( !empty($sescouponvalue) && $count_cc != 2 ) {
			$invalid_coupon = new GoogleCoupons("false", $code, 0, 'Coupon code has been already applied to the cart');
			return $invalid_coupon;
		}

		if( empty($sescouponvalue) && $count_cc == 1 )
		{
			$coupon_result = $GLOBALS['ISC_CLASS_CART']->api->ApplyCoupon($code);
			//mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( 'coupon_result1: ".print_r($coupon_result,true)."')");
			if (!$coupon_result) {
			...
			$GLOBALS['ISC_CLASS_CART']->api->UpdateCartInformation();
			//mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( 'coupon_result2: ".print_r($coupon_result,true)."')");
			$sescouponvalue = $GLOBALS['ISC_CLASS_CART']->api->GetAppliedCouponCodes();
			//mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( 'ses 3: ".print_r($sescouponvalue,true)."')");
		}
		...
		$discount_total = 0;
		$normal_total = 0;
		//mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( 'products: ".print_r($products,true)."')");
		...
		$total1 = array('normal'=>$normal_total,'discount'=>$discount_total);
		$coupon = new GoogleCoupons('true', $code, $coupon_amount, $this->GetCouponName($code));
		//mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( 'coupon: ".print_r($coupon,true)."')");
		//mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( 'total1: ".print_r($total1,true)."')");
	...
	private function CreateOrder()
	{
		...
		$cartContent = $GLOBALS['ISC_CLASS_CHECKOUT']->BreakdownCartByAddressVendorforshipping();	//Changed to merging function by Simha
	...
	{
        $GLOBALS['ISC_CLASS_CART'] = GetClass('ISC_CART');  					//$GLOBALS['ISC_CLASS_CART'] not declared earlier, Added by Simha
		...
		foreach ($product_array as $k => $product) {
            ...
            */
            ...
            $itemTotal += ($curprice * $product['quantity']) + $comptotal; # $comptotal added to add the complementary amount to the table -- Baskaran    
		}
		...
		$vendorIds = array_shift($vendorIds);//array_pop replaced by array_shift to consider only the first vendor after merging vendors
