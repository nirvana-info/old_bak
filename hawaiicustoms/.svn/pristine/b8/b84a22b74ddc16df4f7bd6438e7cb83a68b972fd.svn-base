...
//        $realPath = ISC_BASE_PATH.'/description_images/'.'/'.$this->foldername().'/'.$path;
        $realPath = ISC_BASE_PATH.'/'.$this->foldername().'/'.$path;
        if(!is_dir($realPath)) {
            return $images;
        }
...
//            echo '["'.$image.'","'.GetConfig('AppPath').'/description_images/'.$this->foldername().'/'.$image.'"]'.$comma."\n";
            echo '["'.$image.'","'.GetConfig('AppPath').'/'.$this->foldername().'/'.$image.'"]'.$comma."\n";
        }
        echo ');';
        exit;
    } 
...
//        $destination = ISC_BASE_PATH.'/description_images/'.$this->foldername().'/'.$name;
    ...
    public function foldername() {
        $vendor = $GLOBALS['ISC_CLASS_ADMIN_AUTH']->GetVendor();
        $user = $GLOBALS['ISC_CLASS_ADMIN_AUTH']->GetUser();

        $subfolder = "description_images";
        $commonfolder = "uploaded_images";
        $subcatfolder = "category_images";
        $defaultfolder = "Default";
        $fname = '';
        if($_SESSION['congobrand'] != '') {
            $vendor_array = array('Westin'=>'1','Extang'=>'2','K&N'=>'3','Truck Champ'=>'4','Magnaflow'=>'5','Advantage'=>'6','Truxedo'=>'7','BakFlip'=>'8','Backrack'=>'9','Bedrug'=>'10','Husky'=>'11','Stampede'=>'12');
            $vendorid = $_SESSION['congobrand'];
            $folder = array_search($vendorid,$vendor_array);
            # key is the folder name here,value is the search
            $folder_product = array('Westin'=>'Westin','Extang'=>'Extang','KandN'=>'K&N','TruckChamp'=>'Truck Champ','Magnaflow'=>'Magnaflow','Advantage'=>'Advantage','Truxedo'=>'Truxedo','BakFlip'=>'BakFlip','Backrack'=>'Backrack','Bedrug'=>'Bedrug','Husky'=>'Husky','Stampede'=>'Stampede'); 

            $brandkey = array_search($folder,$folder_product);
            
            # When the $brandkey is null means we are taking the vendor name from vendortable and creating folder on that name
            $vendorquery = "SELECT * from [|PREFIX|]vendors where vendorid = $vendorid";
            $vendorresult = $GLOBALS["ISC_CLASS_DB"]->Query($vendorquery);
            $vendorrow = $GLOBALS["ISC_CLASS_DB"]->Fetch($vendorresult);
            ...
            $branddir = ISC_BASE_PATH.'/'.$subfolder.'/'.$brandname;
            
            if(is_dir($branddir)) {
                $fname = $subfolder.'/'.$brandname;
            }
            else {
                # If the folder doesn't exit creating the folder name in the directory.
                @mkdir($branddir, 0777);
                $fname = $subfolder.'/'.$brandname;
            }
            
        }
        else if($_SESSION['congoseries'] != ''){
            $seriesbrand = $_SESSION['congoseries'];
            $query = "SELECT * from [|PREFIX|]brands where brandid = $seriesbrand";
            $result = $GLOBALS["ISC_CLASS_DB"]->Query($query);
            $row = $GLOBALS["ISC_CLASS_DB"]->Fetch($result);
            $seriesbrandname = $row['brandname'];
            $folder_brand = array('Westin'=>'Westin','Extang'=>'Extang','KandN'=>'K&N','TruckChamp'=>'Truck Champ','Magnaflow'=>'Magnaflow','Advantage'=>'Advantage','Truxedo'=>'Truxedo','BakFlip'=>'BakFlip','Backrack'=>'Backrack','Bedrug'=>'Bedrug','Husky'=>'Husky','Stampede'=>'Stampede'); 
            
            $keyvalue = array_search($seriesbrandname,$folder_brand);
            if($keyvalue != NULL) {
                $seriesname = $keyvalue;
            }
            else {
                $seriesname = $seriesbrandname;
            }
            $seriesdir = ISC_BASE_PATH.'/'.$subfolder.'/'.$seriesname;
            
            if(is_dir($seriesdir)) {
                $fname = $subfolder.'/'.$seriesname;
            }
            else {
                # If the folder doesn't exit creating the folder name in the directory.
                @mkdir($seriesdir, 0777);
                $fname = $subfolder.'/'.$seriesname;
            } 
        }
        else if($_SESSION['congocatname'] != '') {
            $catfolder = $_SESSION['congocatname'];
            $catdir = ISC_BASE_PATH.'/'.$subcatfolder.'/'.$catfolder;
            
            if(is_dir($catdir)) {
                $fname = $subcatfolder.'/'.$catfolder;
            }
            else {
                @mkdir($catdir, 0777);
                $fname = $subcatfolder.'/'.$catfolder;
            ...
            $fname = GetConfig('ImageDirectory').'/'. $commonfolder;
        }
