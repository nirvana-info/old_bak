<?php
	...
	{   
		...
		public function _GetBrandList(&$Query, $Start, $SortField, $SortOrder, &$NumResults)
		{
			...
			$query = "
				SELECT b.*, 
                SUM(p.prodratingtotal)/SUM(p.prodnumratings) AS prodavgrating, 
				...
				while ($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($brandResult)) {
                    ...
                    if($row['prodavgrating'] > 0)    {          
                        ...
                        for ($r = $row['prodavgrating']; $r < 5; $r++) {
                         ...
                         $ratedValue = $this->getRoundValue( $row['prodavgrating']);
                    ...
                    $squery = "
                                SELECT b.*, SUM(p.prodratingtotal)/SUM(p.prodnumratings) AS seriesprodavgrating, 
                        ...
                        $GLOBALS['Prodcnt'] = $srow['prodcnt'];
                                ...
                                if($srow['seriesprodavgrating'] > 0)    {          
                                    ...
                                    for ($r = $srow['seriesprodavgrating']; $r < 5; $r++) {
                                     ...
                                     $ratedValue = $this->getRoundValue( $srow['seriesprodavgrating']);
			        				//$ratingText = sprintf(GetLang('ReviewRated'), $ratedValue);
			        				$ratingHtml = sprintf("<img title='%s'  src='images/IcoRating%s.gif'>",$ratingText, $ratedValue);
			        				 $GLOBALS['SeriesRating'] = $ratingHtml;
                                }
                                else    {
        ...
        # Used to clear the HTML Editor image upload folder name Baskaran
            ...
            if(isset($_POST['seriesname'])) {
                ...
                // Save the brands to the database
					...
					/* Added for series logo image */
					$serieslogoimage = $this->_StoreFileAndReturnId('serieslogoimagefile');   
					$serieslogoimage = $this->_AutoGenerateThumbForLogoImage($serieslogoimage);
					...
					*/
					...
					$newSeries = array(
						...
						"serieslogoimage" => "$serieslogoimage",
                        'trackingscript' => $trackingscript,
                        'controlscript' => $controlscript
					);
        ...
        public function EditSeries($MsgDesc = "", $MsgStatus = "")
        {
            ...
            if(isset($_GET['seriesId'])) {
                ...
                if($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($result)) {
                    ...
                    $GLOBALS['DisplayName'] = $row['displayname'];
					...
					//blessen
					$GLOBALS['seriespagetitle'] = $row['seriespagetitle'];
					$GLOBALS['seriesmetakeywords'] = $row['seriesmetakeywords'];
					$GLOBALS['seriesmetadesc'] = $row['seriesmetadesc'];
					$GLOBALS['SeriesImageAlt'] = $row['seriesimagealt'];
                    $GLOBALS['controlscript'] = $row['controlscript'];
                    $GLOBALS['trackingscript'] = $row['trackingscript'];
					...
					$GLOBALS['SeriesLogoImageMessage'] = '';
                    if ($row['serieslogoimage'] != '') {
                        $limage = '../' . 'series_images' . '/' . $row['serieslogoimage'];
                        $GLOBALS['SeriesLogoImageMessage'] = sprintf(GetLang('SeriesLogoImageDesc'), $limage, $row['serieslogoimage']);
                    }
				...
				*/
                    ...
                    if($cnt != 0) {    
                        ...
                        if($GLOBALS["ISC_CLASS_DB"]->GetErrorMsg() == "") {
							...
							/* Logo Image deletion */
							if (array_key_exists('delserieslogoimagefile', $_POST) && $_POST['delserieslogoimagefile']) {
                                $this->DelSeriesLogoImage($seriesId);
                                $GLOBALS['ISC_CLASS_DB']->UpdateQuery('brand_series', array('serieslogoimage' => ''), "seriesid='" . (int)$seriesId . "'");
                            } else if (array_key_exists('serieslogoimagefile', $_FILES) && ($serieslogoimagefile = $this->_StoreFileAndReturnId('serieslogoimagefile'))) {
                                
								$serieslogoimagefile = $this->_AutoGenerateThumbForLogoImage($serieslogoimagefile);
                                $GLOBALS['ISC_CLASS_DB']->UpdateQuery('brand_series', array('serieslogoimage' => $serieslogoimagefile), "seriesid='" . (int)$seriesId . "'");
                            }
...
//            echo $query;exit;
		...
		private function DelSeriesLogoImage($file)
        {
            if (isId($file)) {
                if (!($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($GLOBALS["ISC_CLASS_DB"]->Query("SELECT * FROM [|PREFIX|]brand_series WHERE seriesid='" . (int)$file . "'")))) {
                    return false;
		...
		/**
        * _AutoGenerateThumbForLogoImage
        * @param string $ImageName Image name
        *
        * @return string file name stored
        */
		public function _AutoGenerateThumbForLogoImage($ImageName)
        {
            // Takes the filename of an image already uploaded into the
            // image directory, generates a thumbnal from it, stores it
            // in the image directory and returns its name
