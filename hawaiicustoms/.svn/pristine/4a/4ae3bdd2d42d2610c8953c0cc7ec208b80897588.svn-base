	...
	class ISC_ADMIN_DISCOUNTS
	{
		...
		private function ManageDiscountsGrid(&$numDiscounts)
		{
			...
			$sortLinks = array(
                ...
                "DiscountStartDate" => "c.discountstart",
		...
		{
			$query = "SELECT * FROM [|PREFIX|]discounts c ORDER BY sortorder";
		...
		private function DeleteDiscounts()
		{
			if (isset($_POST['discount'])) {
				...
				$this->UpdatePriceInBackground(0);
		...
		private function CreateDiscountStep1()
		{
            ...
            $GLOBALS['DiscountStartDateDisabled'] = 'readonly="readonly"';
		...
		private function EditDiscountStep1()
		{
			...
			if ($discountAPI->DiscountExists($discountId)) {
				...
				if ($discount['discountexpiry'] != 0) {
                    ...
                    $GLOBALS['DiscountExpiryHours'] = date("H:i", isc_html_escape($discount['discountexpiry']));
				} else {
					$GLOBALS['DiscountExpiryDate'] = '';
                    $GLOBALS['DiscountExpiryHours'] = '';
				}

                if ($discount['discountstart'] != 0) {
                    $GLOBALS['DiscountStartDate'] = date("m/d/Y", isc_html_escape($discount['discountstart']));
                    $GLOBALS['DiscountStartHours'] = date("H:i", isc_html_escape($discount['discountstart']));
                } else {
                    $GLOBALS['DiscountStartDate'] = '';
                    $GLOBALS['DiscountStartHours'] = '';
                }
                ...
                if (!empty($GLOBALS['DiscountStartDate'])) {
                    $GLOBALS['DiscountStartDateCheck'] = 'checked="checked"';
                    $GLOBALS['DiscountStartDateDisabled'] = '';
                }
        ...
        private function FilterSeries() {   
            ...
            $GLOBALS['FilteredSeriesList']  = '<select multiple="multiple" size="12" name="var_seriesids[]" id="var_seriesids" class="Field250 ISSelectReplacement">';
            ...
            echo $GLOBALS['FilteredSeriesList'];
		...
		*/
		...
		private function UpdatePriceInBackground($discountId)	{
				exec("/usr/bin/php ../updatefinalprice.php discount $discountId > logs1 &#038;");
                //exec("C:/wamp/bin/php/php5.2.6/php D:/narasimha_workspace/store/updatefinalprice.php discount $discountId"); 
