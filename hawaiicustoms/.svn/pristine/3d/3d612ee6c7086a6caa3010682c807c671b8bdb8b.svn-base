				    ...
				    for($i = 0; $i < $cntarrOrder; $i++){
					  ...
					  $pricefeature = $esc.isc_html_escape($compprice);    
						...
						if($i == 0 and $productid != 0) {
							...
							$complementary .= "<tr><td><input type='checkbox' name='rdprod[]' id='rdprod_".$i."' value='".$productid."' $checked onclick='ActiveProductTab(\"CompItem_Tab\");ShowCompDesc(\"$skucode\",this.id);unCheck()'>"."<label id='complabel' for='rdprod_".$i."' onmouseover= 'loadHoverImage(event, \"$prodimage\", \"$pricefeature\");' onmouseout = 'hideTip();'>&nbsp;<a href='#Compl'>$skucode</a></label>"." $compdesc"."</td>

                            <td><div $styleval id='compqty_".$productid."'>".
                                $this->BuildOption('compqty['.$productid.']', $sel)
                                ."</div></td><td><div id='pr_".$productid."' $styleval><b>$price<b></div></td>
                                </tr>";
						}
