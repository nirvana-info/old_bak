		...
		*/
		...
		private function SaveNewOrder()
		{
			...
			if(!$orderId) {
				...
				@ob_end_clean(); 
                    /*$location = 'index.php?ToDo=payOrder&oid='.$orderId;*/  /*$GLOBALS['ShopPathSSL'].*/     header("Location:".$GLOBALS['ShopPathSSL']."/admin/index.php?ToDo=payOrder&oid=".$orderId);  
					//die();
                }
...
//                $this->ManageOrders($response, MSG_SUCCESS);
				...
				header("Location:index.php?ToDo=addOrder");
	...
	/* Baskaran Ends */
		...
		private function CommitOrder($data, $orderId=0)
		{
			...
			************************************************************/
			...
			/*if($data['ordstatus'] != $newOrder['ordstatus']) { -- Baskaran
