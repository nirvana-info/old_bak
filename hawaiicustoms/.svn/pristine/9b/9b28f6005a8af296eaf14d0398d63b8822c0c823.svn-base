...
class ISC_NEWSEARCH
{
	...
	function _SetSearchData()
	{
		...
		$isSmartSearch = $_COOKIE['is_smart_search'];
		setcookie( "is_smart_search", '', $date_of_expiry ,"/");
		
		if($isSmartSearch == 1){
			$_REQUEST['is_smart_search'] = 1;
			$this->_issmartsearch = 1;
		}
...
WHERE prodbrandid in(select brandid from [|PREFIX|]brands WHERE brandname='". $this->_searchterms['brand']."')  and brandseriesid != 0 $where2 $outer_condition group by brandseriesid";
	...
	function DoSearch($Start, &$Search_Result, &$Search_Count)
	{
		...
		else if(isset($params['flag_srch_category']) && $params['flag_srch_category'] == 0 && ( !isset($GLOBALS['BRAND_SERIES_FLAG']) || (  isset($GLOBALS['BRAND_SERIES_FLAG']) && $GLOBALS['BRAND_SERIES_FLAG'] == 0 ) ) && !isset($params['partnumber'])  )
		{
			...
			{
				//alandy_2011-11-9 mark.notice,$params['srch_category'] first item is parentcategory.
			...
			if(isset($params['series']))
			{
				...
				$seo_query = "select seriesname , seriespagetitle , seriesmetakeywords , seriesmetadesc,controlscript,trackingscript from [|PREFIX|]brand_series where seriesname = '".$params['series']."'";
				$seo_result = $GLOBALS['ISC_CLASS_DB']->Query($seo_query);
                ...
                $GLOBALS['CategoryTrackingCodeTop'] = $series_array['controlscript'];
                $GLOBALS['CategoryTrackingCodeBottom'] = $series_array['trackingscript'];
	...
	function ShowSearchPage()
	{
		...
		if ((isset($_GET['mode']) && $_GET['mode'] == "advanced") || $this->_searchterms['model'] == "advanced"){
		...
		*/
		...
		if (@$this->_searchterms['search_query'] != "") {
			...
			if ($start == 0) {
				...
				//alandy_2012-2-2 modify.the smart search don't fetch from url.
				if($GLOBALS['EnableSEOUrls'] == 1)
				{
					$url_array = split('/', strtolower($_SERVER['REQUEST_URI']));
					//$is_smart_search = $url_array[array_search('is_smart_search', $url_array, true)+1];
					$is_smart_search = $this->_issmartsearch;
					if ($is_smart_search==1 && isset($this->_searchterms['search_query']) && strlen($this->_searchterms['search_query']) > 0 && $noSpider)	{
	...
	function BuildProductSearchTerms2($input)
	{
		...
		if(!empty($_COOKIE['last_search_selection']['year'])){
			$input['year'] = $_COOKIE['last_search_selection']['year'];
		}
		if(!empty($_COOKIE['last_search_selection']['make'])){
			$input['make'] = $_COOKIE['last_search_selection']['make'];
		}
		if(!empty($_COOKIE['last_search_selection']['model'])){
			$input['model'] = $_COOKIE['last_search_selection']['model'];
		}
					...
					if($brand_name == $in_str) {
						...
						else if($common_arr['type'] == 'category')
						{
							...
							if($catgoryname[$key]['catparentid'] != 0){   // checking its not a parent catg ?
								...
								$flag_catg = 1;
								$catg_parentname = $catgoryname[$key]['catname'];
							}
						...
						//alandy_mark,this will be commit out.
						break;
					}
				}
			}
					...
					foreach($brandseriesaltname as $bsaltkey => $bsaltvalue) {
						if(!empty($bsaltvalue)) {
							...
							for($i=0;$i<count($brandseriesalternate_names);$i++)
							{
								...
								if(!empty($in_str) && $this->_isMatchedKeyword($brandseriesalternate_names[$i], $in_str, $input['search_query'])){//zcs=[New version]
									$compare_brand_name = $in_str;
									foreach($track_srch_key as $key => $val) {
										unset($srch_arr[$val]);
									}
								...
								for($k=0;$k<$count_srch;$k++)
								{
									if(strlen(stristr($altname,rtrim($srch_arr[$k], 's'))) > 0 ) {
										...
										if($this->_isMatchedKeyword($altname, $in_str, $input['search_query'])){//zcs=[New version]
											...
											$altcat[] = $altname;
											$find_catg_id[] = $altkey;
											//alandy_2012-3-21 add.if match category,replace this keyword.
											for($j=0;$j<count($track_srch_key);$j++) {
												unset($srch_arr[$track_srch_key[$j]]);
											}
		...
		if(!empty($find_catg_id)){
			...
			//alandy_2012-2-23 mark importent.
...
//exit;
		...
		if(isset($input['category'])) {
			...
			//alandy_mark.This way need to notice,when search category.
		...
		if($flag_catg == 1 ) {
			...
			//alandy_2012-2-23 add parentid.
			$GLOBALS['ISC_SRCH_PCATG_NAME'] =  isset($catg_parentname) ? ucwords($catg_parentname) : '';
...
/*
		...
		*/	
		...
		if(isset($input['subcategory']) && !empty($input['subcategory'])) {
			...
			//alandy_2011-11-11 modify.importent category search.
		...
		//alandy_2012-2-27 modify.if clear ymm,don't set ymm cookie by search keywords.
		$removeymmFlag = 0;    //default value 0.
		if(isset($_COOKIE['last_search_selection']['removeymm']) && !empty($_COOKIE['last_search_selection']['removeymm'])){
			$removeymmFlag = 1;
		}
		...
		//last reset cookie removeymm flag.
		setcookie( "last_search_selection[removeymm]", '', $date_of_expiry ,"/" );
		//NI CLOUD 2010-06-18
...
WHERE prodbrandid in(select brandid from [|PREFIX|]brands WHERE brandname='". $searchTerms['brand']."')  and brandseriesid != 0 $where2 $outer_condition group by brandseriesid";
	...
	//alandy_2012-2-29 add brand lanting page.
	function GetBrandQuery()
	{
		...
		$query ="select " .$GLOBALS['srch_where'] ." from ".$fromTable;
		$query .= $this->GetJoinQueryTables().$joinQuery.$where.$group;
		return array(
			'query' => $query,
			'countQuery' => $countQuery,
			'orderby' => $orderBy
