							...
							if (file_exists($dir) and $tempart1 != "" ) {
								...
								while (!feof($file_handle) ) {
									...
									if ($k==0)
										...
										{
										if ($year == ""  and $make == "" and $model == "" and $submodel == "")
											{
											// do nothing
											}
											else
...
if ($id != "") {
										...
										// Table one cabbed_table
										 $query2 = "SELECT * FROM isc_cabbed_table  WHERE ymm_id  = '".$id."' and cabsize  = '".$cabsize."' and bedsize = '".$bedsize."' limit 0,1 ";
										$result2 = $GLOBALS['ISC_CLASS_DB']->Query($query2);
...
if ($id != "") {
									...
									$k++;		
											...
											$GLOBALS['Message'] .= $upcounteng. " Records added to the Engliter Table<BR>";
