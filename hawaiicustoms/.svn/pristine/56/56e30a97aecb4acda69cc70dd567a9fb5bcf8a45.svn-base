									...
									if ($k==0)
										...
										if (mysql_num_rows($result2) == 0 )
											...
											if ($cabsize != ""  or $bedsize != "")
												...
												$oldcabid = $row['id'];
												$sql = "update  isc_cabbed_table  set cabsize  = '".$cabsize."' , bedsize = '".$bedsize."'  where `id` = ".$oldcabid." ";
												mysql_query($sql);
												$upcountcab++;
												}
											...
											if (mysql_num_rows($result2) == 0 )
												...
												if ($engtype != ""  or $liter != "") 
													...
													$row = $GLOBALS["ISC_CLASS_DB"]->Fetch($result2);
													$oldengid = $row['id'];	
													$sql = "update  isc_engine_table  set engtype  = '".$engtype."' , liter = '".$liter."'  where `id` = ".$oldengid." ";
													mysql_query($sql);
													$upcounteng++;
...
$query2 = "SELECT * FROM isc_cabbed_table  WHERE `ymm_id`  = ".$Id." ";
$result2 = $GLOBALS['ISC_CLASS_DB']->Query($query2);
if (mysql_num_rows($result2) == 1 )
 ...
 $GLOBALS['ISC_CLASS_DB']->DeleteQuery('product_mmy', "WHERE id = '$Id'");
	...
	$GLOBALS['ISC_CLASS_DB']->DeleteQuery('cabbed_table', "WHERE ymm_id = '$Id' and cabsize = '$cabsize' and bedsize = '$bedsize' ");									
...
//                        $GLOBALS['EditMMYLink'] = "<a title=GetLang(\'MMYEdit\') class='Action' href='index.php?ToDo=editMMY&amp;Id=$mid&amp;cid=$cid&amp;eid=$eid'>$edit</a>";
 ...
 $GLOBALS['DeleteMMYLink'] = "<a title='".GetLang('MMYDelete')."' class='Action' href='#' onclick=\"deletemmyid(".$row['id'].",'".$row['cabsize']."','".$row['bedsize']."')\">".GetLang('Delete')."</a>";
                    } else {
...
$query2 = "SELECT * FROM isc_engine_table  WHERE `ymm_id`  = ".$Id." ";
$result2 = $GLOBALS['ISC_CLASS_DB']->Query($query2);
if (mysql_num_rows($result2) == 1 )
			{
 $GLOBALS['ISC_CLASS_DB']->DeleteQuery('product_mmy', "WHERE id = '$Id'");
...
$GLOBALS['ISC_CLASS_DB']->DeleteQuery('engine_table', "WHERE ymm_id = '$Id' and engtype = '$engtype' and liter = '$liter' ");
