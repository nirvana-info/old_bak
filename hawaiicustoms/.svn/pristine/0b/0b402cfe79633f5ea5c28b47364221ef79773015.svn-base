...
include_once dirname(__FILE__).'/class.updateymmcache.php';
...
@unlink($ins_path);
                ...
                //$RefArray['complementaryitems'] = $_POST['complementaryitems'];
...
$this->updateYmmsCacheData();
				//update cache file
				$uc = new UPDATE_YMM_CACHE_FILE();
				$uc->get_product_data($prodId);
				$uc->update_product_ymm_cache();
				return true;
			}
			else {
				// The product commit failed
	...
	public function _CommitImportProduct($ProductId, &$Data, &$Images, &$Variations, &$CustomFields, $DiscountRules=array(), &$Err = null, &$ProductFields=array(), $isImport=false)
		{
			...
			$query = "";
			$Err = null;
			if(!isset($Data['comp_type']) || $Data['comp_type']== null)
			{
				$Data['comp_type']=1;
			}
			$temp = array(
...
$this->updateYmmsCacheData();
			...
			if($Err == "") {
				...
				//update cache file
				//$uc = new UPDATE_YMM_CACHE_FILE();
				//$uc->update_product_ymm_cache($prodId);
				return true;
			}
			else {
				// The product commit failed
...
//blessen goto
$GLOBALS['GOTO'] = '<INPUT TYPE="text" NAME="page_no" size="2" id="page_no" onkeyup="go(this.value);"> &nbsp;<a href="index.php?ToDo=viewProducts&sortField=productid&sortOrder=desc&page=1&ajax=1" id="gotopage");>Go to page</a>';

$GLOBALS['GOTO1'] = '<INPUT TYPE="text" NAME="page_no1" size="2" id="page_no1" onkeyup="go1(this.value);"> &nbsp;<a href="index.php?ToDo=viewProducts&sortField=productid&sortOrder=desc&page=1&ajax=1" id="gotopage1");>Go to page</a>';
			...
			if(isset($array['searchQuery']) && $array['searchQuery'] != "") {
				...
				///  Code Added by Mayank
					if(isset($_REQUEST['ajaxsearchtype']) && $_REQUEST['ajaxsearchtype']=='2') {
						...
						$search_query = isc_strtolower($array['searchQuery']);
						//$fulltext_fields = array("p.prodcode");
						//$queryWhere .= "((" . $GLOBALS["ISC_CLASS_DB"]->FullText($fulltext_fields, $search_query, true);
						$queryWhere .= "(( ";
						...
						$search_query = isc_strtolower($array['searchQuery']);
						//$fulltext_fields = array("p.prodname", "p.prodcode");
						//$queryWhere .= "((" . $GLOBALS["ISC_CLASS_DB"]->FullText($fulltext_fields, $search_query, true);
						$queryWhere .= "(( ";
...
$GLOBALS['article_file'] =  "No Article file Found ";
                ...
                //$GLOBALS['Complementaryitems'] = $arrData['complementaryitems'];
