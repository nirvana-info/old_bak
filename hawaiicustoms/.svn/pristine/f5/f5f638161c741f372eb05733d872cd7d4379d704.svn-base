...
//                $query = "SELECT * FROM [|PREFIX|]categories ORDER BY catdeptid ASC, catsort ASC, catname ASC";
				...
				$query = "select c.categoryid, count(c.productid) as total,floor(SUM(p.prodratingtotal)/SUM(p.prodnumratings))AS prodavgrating ";
...
//                    $catquery = $GLOBALS['ISC_CLASS_DB']->Query("SELECT categoryid FROM [|PREFIX|]categories where catparentid = $catid and catdeptid = ".$resdeptid);
                    ...
                    if(count($catlistid) > 0){
                        $prodcount = $GLOBALS['ISC_CLASS_DB']->Query("SELECT count(*) as prodcount, floor(SUM(p.prodratingtotal)/SUM(p.prodnumratings))AS prodavgrating FROM [|PREFIX|]categoryassociations c INNER JOIN [|PREFIX|]products p on c.productid=p.productid where c.categoryid IN ($catid,$catcountlist)"); 
                    }
                    else {
                        $prodcount = $GLOBALS['ISC_CLASS_DB']->Query("SELECT count(*) as prodcount, floor(SUM(p.prodratingtotal)/SUM(p.prodnumratings))AS prodavgrating FROM [|PREFIX|]categoryassociations c INNER JOIN [|PREFIX|]products p on c.productid=p.productid where c.categoryid IN ($catid)"); 
                    }
...
//                    if(count($catlistid) > 0){
		...
		{
			//remove customcontentid in cookie
			if(isset($_COOKIE['CustomContentId'])){
				setcookie("CustomContentId", '');
			}
		...
		*/
		...
		private function EditCategory()
		{
			...
			if (isset($_GET['catId'])) {
				...
				}else if(isset($_COOKIE['CustomContentId']) && is_numeric($_COOKIE['CustomContentId'])) {
					$contentId = $_COOKIE['CustomContentId'];
				}else{
					$customItem = array(
						'contenttype' => 1,
