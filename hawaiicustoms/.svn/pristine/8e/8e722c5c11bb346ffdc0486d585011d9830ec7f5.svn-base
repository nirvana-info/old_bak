	...
	*/
	public function ProductStatsByNumViewsGrid()
	{
		...
		if ($total_products > 0) {
            ...
            $sortFields = array('productid','prodcode',$cursortfield,'numsold','prodnumviews','unitssoldpercent','avgrating');
            if(isset($_GET['SortBy']) && in_array($_GET['SortBy'], $sortFields)) {
                $sortField = $_GET['SortBy'];
		...
		if (isset($_REQUEST['export']) )
			{
				...
				$content =  "productid,Item,Views  ,Units Sold ,Units Sold/Views,Average Rating \n";
				$resultexp = $GLOBALS['ISC_CLASS_DB']->Query($mainQuery);
				while($rowex = $GLOBALS['ISC_CLASS_DB']->Fetch($resultexp))
				{

					$pname = str_replace(",", " ", $rowex['prodname']);
					
					$content.=  $rowex['productid'].",".$pname.",".$rowex['prodnumviews'].",".$rowex['numsold'].",".$rowex['avgrating']."\n";
                ...
                case 'category':    {
                    ...
                    $mainQuery = "
                                SELECT
                                    ...
                                    (SUM(p.prodratingtotal) / SUM(p.prodnumratings)) AS avgrating,
                                    IF(p.prodnumviews > 0, SUM(ordprodqty) / SUM(p.prodnumviews), 0) AS unitssoldpercent
                                FROM
                                    [|PREFIX|]products p
                                    ...
                                    (
                                    SELECT 
                                        ordprodqty, ordprodid FROM isc_order_products
                                        LEFT JOIN isc_orders ON orderid = orderorderid 
                                        WHERE ordstatus IN (".implode(',', GetPaidOrderStatusArray()).") AND ordprodtype != 'giftcertificate' 
                ...
                case 'series':    {
                    ...
                    SELECT * FROM 
                                ...
                                SELECT
                                    ...
                                    p.prodname,
                                    bs.seriesid, 
                                    CONCAT(b.brandname, ' - ', bs.seriesname) AS seriescomname,
                                    SUM(ordprodqty) AS numsold,
                                    SUM(p.prodnumviews) AS prodnumviews,
                                    (SUM(p.prodratingtotal) / SUM(p.prodnumratings)) AS avgrating,
                                    IF(p.prodnumviews > 0, SUM(ordprodqty)/SUM(p.prodnumviews), 0) AS unitssoldpercent
