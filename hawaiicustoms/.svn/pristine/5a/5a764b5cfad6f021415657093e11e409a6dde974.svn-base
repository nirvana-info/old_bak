...
if ($i == $pages) $increment = $last;
 ...
 $new_query = $query. ' LIMIT '.$SL.' , '.$increment.'';
					...
					// As the instruction_file moved from product table, this code needed to be updated.
					if (isset($new_row['instruction_file']) and !empty($new_row['instruction_file']) )
					{
					$pos1 = stripos($new_row['instruction_file'], "http://");
					if ($pos1 === false) {
						 $new_row['instruction_file'] = GetConfig('ShopPath')."/instruction_file/".$new_row['instruction_file'];
						} 
					}
