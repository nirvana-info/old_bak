...
class ISC_ADMIN_SETTINGS_ORDER_REVIEWREQUEST
{
	...
	public function HandleToDo($Do)
	{
		...
		switch(isc_strtolower($Do))
		{
			...
			/* Baskaran */
			case "setenablerequestscriptsettings": {  
                if(!isset($_REQUEST['ajax'])) {  
                    $GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintHeader();
                }
                $this->SetRequestEnableSettings();
                if(!isset($_REQUEST['ajax'])) {
                    $GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintFooter();
                }
                break;
            ...
            case "setrequestafterrequestscriptsettings": {  
                if(!isset($_REQUEST['ajax'])) {  
                    $GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintHeader();
                }
                $this->SetRequestAfter();
                if(!isset($_REQUEST['ajax'])) {
                    $GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintFooter();
                }
                break;
            ...
            case "setnorequestrequestscriptsettings": {  
                if(!isset($_REQUEST['ajax'])) {  
                    $GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintHeader();
                }
                $this->SetNoRequest();
            ...
            case "setresendrequestscriptsettings": {  
                if(!isset($_REQUEST['ajax'])) {  
                    $GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintHeader();
                }
                $this->SetResendRequest();
                if(!isset($_REQUEST['ajax'])) {
                    $GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintFooter();
                }
                break;
            ...
            case "setrepeatrequestscriptsettings": {  
                if(!isset($_REQUEST['ajax'])) {  
                    $GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintHeader();
                }
                $this->SetRepeatRequest();
                if(!isset($_REQUEST['ajax'])) {
                    $GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintFooter();
                }
                break;
	...
	public function ManageOrderReviewRequestSettings($MsgDesc = "", $MsgStatus = "")
	{
			...
			/* Tod get the selected date of review request -- Baskaran */
			$query = $GLOBALS['ISC_CLASS_DB']->Query("SELECT * FROM [|PREFIX|]order_review_config WHERE id = '1'");
            $row = $GLOBALS['ISC_CLASS_DB']->Fetch($query);
            if($row['sendrequest'] == '0') {
                $GLOBALS['Disable'] = 'checked="checked"';
            }
            else {
                $GLOBALS['Enable'] = 'checked="checked"';
            }
            $GLOBALS['RequestDays'] = $this->gettotaldays($row['request']);
            $GLOBALS['NoRequestDays'] = $this->gettotaldays($row['norequest']);
            $GLOBALS['ResendDays'] = $this->gettotaldays($row['resend']);
            $GLOBALS['RepeatDays'] = $this->gettotaldays($row['repeat']);
			/* Ends */
			if($numRequest == 0) {
    ...
    public function gettotaldays($selected = 1) {
        $option = '';
        for($i = 1;$i <= 31;$i++) {
            $sel = '';
            if($selected == $i) {
            ...
            $options .= '<option value='.(int)$i.' '.$sel.'>'.$i.'</option>';
        }
        return $options;
    }
	...
	*/	
	...
	public function CopyOrderReviewRequestSettings()
	{
		...
		if($this->RequestrExists($requestId))
		{
			...
			//2010-11-11 Ronnie add,email subject
			$GLOBALS['ReviewRequestEmailSubject']=htmlspecialchars($arrData['emailsubject']);
	...
	public function CreateRequestReviewSettings()
	{
		/*if($message = str_strip($_REQUEST, '#')) {
			...
			//2010-11-11 Ronnie add,email subject
			$GLOBALS['ReviewRequestEmailSubject']="";
			$wysiwygOptions = array(
					'id'		=> 'wysiwyg',
					'width'		=> '60%',
					'height'	=> '350px',
	...
	private function _MyGetRequestData($requestId, &$RefArray)
	{
		...
		if ($requestId == 0) {
			...
			if(isset($_POST['emailsubject']) && $_POST['emailsubject'] != '') {
				$RefArray['emailsubject'] = $_POST['emailsubject'];
			}
			else{
				$RefArray['emailsubject'] = '';
			}
	...
	{		
		...
		//2010-11-23 ronnie add,system don't show
		$query .= " and coupongeneratetype=0 ";
	...
	private function SetRequestAfter() {      
            $requestafter = $_GET["requestafter"];
            $query = "update [|PREFIX|]order_review_config set `request`= $requestafter where id = '1' ";
            $GLOBALS['ISC_CLASS_DB']->Query($query);
            echo "1";
            exit;
    }
    
    private function SetNoRequest() {      
            $norequest = $_GET["norequest"];
            $query = "update [|PREFIX|]order_review_config set `norequest`= $norequest where id = '1' ";
            $GLOBALS['ISC_CLASS_DB']->Query($query);
            echo "1";
            exit;
    }
    
    private function SetResendRequest() {      
            $resend = $_GET["resend"];
            $query = "update [|PREFIX|]order_review_config set `resend`= $resend where id = '1' ";
            $GLOBALS['ISC_CLASS_DB']->Query($query);
            echo "1";
            exit;
    }
    
    private function SetRepeatRequest() {      
            $repeatrequest = $_GET["repeatrequest"];
            $query = "update [|PREFIX|]order_review_config set `repeat`= $repeatrequest where id = '1' ";
            $GLOBALS['ISC_CLASS_DB']->Query($query);
            echo "1";
            exit;
    }
