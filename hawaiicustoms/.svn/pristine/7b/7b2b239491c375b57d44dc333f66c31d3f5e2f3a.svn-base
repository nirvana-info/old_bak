...
class ISC_ADMIN_RETURNS
{
	...
	private function ManageReturnsGrid(&$numReturns)
	{
		...
		while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($returnResult)) {
			...
			$GLOBALS['AmountPaid'] = FormatPrice($row['retprodcost'] * $row['retprodqty']);
	...
	public function SendNewReturnNotification($return, $items)
	{
		...
		$obj_email->AddBody("html", $message);
		$obj_email->AddRecipient(GetConfig('OrderEmail'), "", "h");
		$email_result = $obj_email->Send();
	...
	{
		$query = "
			SELECT r.*, CONCAT(c.custconfirstname, ' ', c.custconlastname) AS custname
			FROM [|PREFIX|]returns r
