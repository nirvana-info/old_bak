var ExpressCheckout = {
        ...
        GuestCheckout: function()
    {
        ...
        if($('#CheckoutGuestForm').css('display') != 'none' && !$('#checkout_type_register:checked').val()) {
            // johnny add
	    $("#CheckoutStepBillingAddress #FormField_1").unbind('blur');
	    if($("#errorMessage").length != 0){
			$("#errorMessage").remove();
	    }
	    // end johnny add
            $('#CheckoutStepAccountDetails .ExpressCheckoutCompletedContent').html(lang.ExpressCheckoutCheckingOutAsGuest);
	    $("#CheckoutStepBillingAddress .billingButton").removeAttr("disabled");
	    ...
	    // johnny add
	       $("#CheckoutStepBillingAddress #FormField_1").bind('blur', function(){
			if($("#errorMessage").length == 0){
				$(this).after($("<span style='color:red;' id='errorMessage'></span>"));
			}
			var phpUrl = config.ShopPath + '/remote.php';
			var email = $(this).val();
			$.ajax({
				url: phpUrl,
				type: 'POST', 
				...
				{   
					if(data == 0){
						$("#errorMessage").html("Email already exist!");
						$("#CheckoutStepBillingAddress .billingButton").attr("disabled", "true");
					}else{
						// email avaliable
						$("#errorMessage").html("");
						$("#CheckoutStepBillingAddress .billingButton").removeAttr("disabled");
	...
	{         
		...
		{
			window.setTimeout(ExpressCheckout.PaymentFormFocus, 1500);
		}
	...
	GetSteps: function()
	{
		...
		steps[steps.length] = 'AddonProducts';
	...
	ChooseShippingProvider: function()
	{
		...
		if(numShippers == 1) {
			...
			titleContent = shipperName + ' '+lang.ExpressCheckoutFor+' '+shipperPrice;
		}
		else {
			titleContent = lang.ShippingMethodCombined;
		}
		...
		$('#CheckoutStepShippingProvider .ExpressCheckoutCompletedContent').html(titleContent);
		
		ExpressCheckout.ResetNextSteps();
		if($('#CheckoutStepAddonProducts').size() > 0){
			ExpressCheckout.LoadAddonProducts();
		}else{
			ExpressCheckout.LoadOrderConfirmation();
		}
	},
	
	ChooseAddonProducts: function(skip)
	{
		if(skip){
			$('input[type=checkbox]:checked', '#CheckoutStepAddonProducts').each(function(){
				$(this).attr('checked', false)
				$(this).parent().find('select').hide();
			});
			ExpressCheckout.LoadOrderConfirmation(1);
		}else{
		var checkedProds = $('input[type=checkbox]:checked', '#CheckoutStepAddonProducts').size();
		if(checkedProds > 0){
			ExpressCheckout.LoadOrderConfirmation(1);
		}else{
			alert("Please select an add-on product at least!");
			}
		}
	},
	...
	LoadOrderConfirmation: function(addToCart)
	{
		var postVars = {};
		...
		if(addToCart){
			var serialize = $('#CheckoutStepAddonProducts form').serializeArray();
	...
	OrderConfirmationLoaded: function(xml)
	{
		...
		if($('status', xml).text() == 0) {
			$('#CheckoutStepAddonProducts .ExpressCheckoutCompletedContent').html('');
			//$('#CheckoutStepShippingProvider .ExpressCheckoutContent').html($('providers', xml).html());
		...
		if(!ExpressCheckout.digitalOrder) {
			ExpressCheckout.completedSteps[ExpressCheckout.completedSteps.length] = 'AddonProducts';
		}
		else {
			ExpressCheckout.completedSteps[ExpressCheckout.completedSteps.length] = 'BillingAddress';
		}
	...
	LoadAddonProducts: function()
	{
		...
		postVars.w = 'expressCheckoutGetAddonProducts';
		...
		$.ajax({
			...
			dataType: 'xml',
			data: postVars,
			success: ExpressCheckout.AddonProductsLoaded
		});
	},

	AddonProductsLoaded: function(xml)
	{
		ExpressCheckout.HideLoadingIndicators();
		...
		if($('status', xml).text() == 0) {
			alert($('message', xml).text().replace('\n', "\n"));
			if($('step', xml).text()) {
				$('#CheckoutStep'+$('step', xml).text()+' .ExpressCheckoutCompletedContent').html('');
				ExpressCheckout.ChangeStep($('step', xml).text());
			}
			return false;
		}
		
		if($('confirmationContents', xml).text() == ''){
			ExpressCheckout.LoadOrderConfirmation();
		}else{
		
			$('#CheckoutStepAddonProducts .ExpressCheckoutContent').html($('confirmationContents', xml).text());
			if($('#CheckoutStepAddonProducts ul.AddonProductList > li').size() < 7){
				$('#CheckoutStepAddonProducts ul.AddonProductList').css({'overflow': 'visible'});
			}
			ExpressCheckout.completedSteps[ExpressCheckout.completedSteps.length] = 'ShippingProvider';
			
			ExpressCheckout.ChangeStep('AddonProducts');
		}
	},
	
	ShowAddonProductQuantity: function(obj){
		var $obj = $(obj);
		var addonProductQuantity = $obj.parent(".prod_select").find('select');
		if($obj.attr('checked')){
			addonProductQuantity.attr('selectedIndex', 0);
			addonProductQuantity.show();
		}else{
			addonProductQuantity.hide();
		}
	},
	
	ShowAddonProductImg: function(obj){
		var Left = $(obj).parent().parent().parent().position().left;
		var Top = $(obj).parent().parent().parent().position().top;
		//var offsetTop = $(obj).parent().parent().parent().parent().scrollTop();
		$('#addon_prod_desc').html($(obj).parent().find('.prod_desc').html());
		
		$('#addon_prod_desc').css({top: Top + 2, left: Left + 120 + 3});
		
		$('#addon_prod_desc').show();
	},
	
	HideAddonProductImg: function(obj){
		$('#addon_prod_desc').hide();
	},
	...
	PaymentFormFocus: function()
	{
		location.href="#CheckoutStepPaymentDetails";
	},
	...
	PaymentFormLoaded: function(xml)
	{
		...
		var $parentCheckout = $('#CheckoutStepPaymentDetails .ExpressCheckoutContent');
		if($parentCheckout.children().length > 1)
			$parentCheckout.children(':not(:first-child)').remove();
		$parentCheckout.append($('paymentContents', xml).text());
		$('#CheckoutStepPaymentDetails').show();
...
//aland_20110525 modify.
	...
	LoadShippingProviders: function()
	{
		...
		var shippingProvidersLoaded = function(xml) {
			...
			$('#CheckoutStepShippingProvider .ShippingProviderList').children(':first').find('input[type=radio]').attr('checked', 'checked');
		};
	...
	ConfirmPaymentProvider: function(str)
	{
		...
		if(paymentProvider != '' && $(paymentProvider).hasClass('ProviderHasPaymentForm')) {
			...
			if(str=="yes"){
				...
				alert('Email Address has existed!');
			}else{
