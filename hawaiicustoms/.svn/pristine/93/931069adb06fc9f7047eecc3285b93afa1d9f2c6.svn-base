                ...
                if(isset($_SERVER["HTTP_REFERER"]) && trim(strtolower($_SERVER["HTTP_REFERER"]), '/')) {
                    ...
                    if(in_array($url,$referer)) {
                        $querycode = $GLOBALS["ISC_CLASS_DB"]->Query("SELECT couponcode from [|PREFIX|]coupons
                                            where ( couponcode = 'FX4598' OR couponcode = 'FX3222' ) 
                        ...
                        $GLOBALS['Promotion'] = '1';
                        if($coupon == 'FX4598') {
                            $GLOBALS['PopupCoupon'] = "<script>alert('The coupon code FX4598 will be applied to your cart. If you choose to purchase this product, the final discounted price will be reflected at checkout.');</script>";
                        }
                        else {
                            $GLOBALS['PopupCoupon'] = "<script>alert('The coupon code FX3222 will be applied to your cart. If you choose to purchase this product, the final discounted price will be reflected at checkout.');</script>";    
                        }
                    }
                    else {
                        $GLOBALS['Promotion'] = '0';
