...
class Thumbnail {
	...
	function resizeToWidth($width) {
	  $ratio = $width / $this->getCurrentWidth();
      $height = $this->getCurrentHeight() * $ratio;
      $this->resize($width,$height);
   }

   function resizeToHeight($height) {
      $ratio = $height / $this->getCurrentHeight();
      $width = $this->getCurrentWidth() * $ratio;
      $this->resize($width,$height);
   }
    ...
    function resize($maxWidth = 0, $maxHeight = 0) {
		...
		if( $this->currentDimensions['width'] <= $this->maxWidth && $this->currentDimensions['height'] <= $this->maxHeight )
		{
