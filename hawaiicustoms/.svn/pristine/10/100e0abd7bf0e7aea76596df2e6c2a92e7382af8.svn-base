...
$GLOBALS['Nav'] = "";
			...
			if($numBrands > 0) {
				...
				while ($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($brandResult)) {
                    ...
                    $squery = "SELECT *,(SELECT count(productid) FROM [|PREFIX|]products where brandseriesid = b.seriesid) as prodcnt FROM [|PREFIX|]brand_series b where b.brandid =  ".$row['brandid']." order by seriessort asc ";
