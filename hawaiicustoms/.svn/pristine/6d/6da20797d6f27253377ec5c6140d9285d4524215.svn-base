        ...
        */
		...
		// blessen
		 private function Redirectsettings() {
			...
			while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result2))
			{
				...
				$id = $row['id'];
				$GLOBALS['urlgrid'].= '<tr> <td>   <input type="text" name="source_url[]" id="source_url[]" value="'.$source_url.'" class="Field250"> </td>  <td>   <input type="text" name="destination_url[]" id="destination_url[]" value="'.$destination_url.'" class="Field250">  </td><td><INPUT TYPE="hidden" name = "urlid[]" value = "'.$id.'"></td> </tr> ';
			}
		...
		public function SaveUpdated301()
        {
			...
			if ($newsource_url != "" and $newdestination_url != "")
			{
				...
				$sql = "INSERT INTO [|PREFIX|]redirect_pages (`source_url`, `destination_url`) VALUES ('".$newsource_url."', '".$newdestination_url."')";
				$result2 = $GLOBALS['ISC_CLASS_DB']->Query($sql);
					...
					else
					{
						...
						$sql = "update  [|PREFIX|]redirect_pages set `source_url` = '".$source_url."' , `destination_url` = '".$destination_url."'  where id = ".$urlid."  ";
						$result2 = $GLOBALS['ISC_CLASS_DB']->Query($sql);
