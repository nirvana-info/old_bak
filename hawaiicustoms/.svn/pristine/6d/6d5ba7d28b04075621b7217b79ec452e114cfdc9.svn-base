...
Private function SaveCouponSettings()
			{
				...
				foreach ($catids as $key => $value) {
					...
					$result = array_intersect($subids, $all_categoryid_array);
					if (count($result) == 0)
					{

						 $sql2 = " update isc_categories set offer = 'yes' where catparentid =  ".$value." ";
						mysql_query($sql2);
				...
				foreach ($brandids as $key => $value) {
					...
					$query1 = " SELECT GROUP_CONCAT( `seriesid` ) AS all_seriesid FROM [|PREFIX|]brand_series where brandid =  ".$value."  GROUP BY `brandid` ";
					$result1 = $GLOBALS['ISC_CLASS_DB']->Query($query1);
					$row1 = $GLOBALS["ISC_CLASS_DB"]->Fetch($result1);
					$all_seriesid = $row1['all_seriesid'];
					$all_seriesid_array = array_unique(explode(",", $all_seriesid));


					$result = array_intersect($seriesids, $all_seriesid_array);
					if (count($result) == 0)
					{

						 $sql2 = " update isc_brand_series set offer = 'yes' where brandid =  ".$value." ";
						mysql_query($sql2);
