		...
		public function HandleToDo($Do)
		{
			switch (isc_strtolower($Do)) {
				case "deleteproductvariations":
					...
					break;
			case "delete_product_importvariations":
					$this->DeleteImport_Variations();					
				break;
				...
				case "copyproduct2":
					...
					break;
				case "updateproductymmcachedata":
                        $GLOBALS['BreadcrumEntries'] = array(GetLang('Home') => "index.php", GetLang('Products') => "index.php?ToDo=viewProducts", GetLang('UpdateProductYmmCacheFile') => "index.php?ToDo=viewProducts");
						$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintHeader();
						$this->updateYmmsCacheData();
                        $now_str = isc_date('Y-m-d H:i:s');
                        echo 'YMM Cache File was successfully updated at '.$now_str.'.<br>';
						$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintFooter();
					break;
				case "updateproduct_categorybrandcachedata":
                        $GLOBALS['BreadcrumEntries'] = array(GetLang('Home') => "index.php", GetLang('Products') => "index.php?ToDo=viewProducts", GetLang('UpdateCategoryBrandCacheFile') => "index.php?ToDo=viewProducts");
						$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintHeader();
						
						$this->UpdateCategoryBrandProductCache();
                        $now_str = isc_date('Y-m-d H:i:s');
                        echo 'Category Brand and Product Cache File was successfully updated at '.$now_str.'.<br>';
						$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintFooter();
					break;
				default:
					if ($GLOBALS["ISC_CLASS_ADMIN_AUTH"]->HasPermission(AUTH_Manage_Products)) {
		...
		*/
		...
		public function _AutoGenerateThumb($ImageName, $Size="thumb", $OverrideExisting=false, $FileType=FT_IMAGE)
		{
			...
			if($Size=='large')    {
				...
				$AutoThumbSize = 240;	//	Added by vikas as thumbnail images need to be 240px as per client request.
			}
...
//blessen
	...
	if ($ProductId == 0) {
		...
		if ($prodId!=0) {
			$isContentChangeAdded = $this->MakeChangesReport($prodId, $Data, $impData, $a);
		}
		//Code for product change report   --- Simha
		...
		if($_POST['hdnname'] == 'hdnsave') {
			...
			if(isnumeric($id))
			{
				...
				if($id > 0)
				{	
					$GLOBALS['ISC_CLASS_DB']->UpdateQuery("import_variations", $merge, "id = '$id' and productid='".$prodId."'");
				}
				else 
				{
					$merge['productid']=$prodId;
					$GLOBALS['ISC_CLASS_DB']->InsertQuery("import_variations", $merge);
					$new_vid = $GLOBALS['ISC_CLASS_DB']->LastId();
					$comData = array(
						"complementaryitems" => $_POST["complementaryitems"],
						"variationid" => $new_vid,
						"productid" => $prodId
					);
					$GLOBALS['ISC_CLASS_DB']->InsertQuery("application_data", $comData);
...
// Add by NI_20100917_Jack for update YMM automatic
$this->updateYmmsCacheData();
					return true;
				}
				else {
	...
	public function _CommitImportProduct($ProductId, &$Data, &$Images, &$Variations, &$CustomFields, $DiscountRules=array(), &$Err = null, &$ProductFields=array(), $isImport=false)
		{
			...
			if ($ProductId == 0) {
				// Add the date this product was modified
				$entity = new ISC_ENTITY_PRODUCT();
				$prodId	=(int)$entity->add($Data);
				//2011-6-9 alandy modify.
				if($prodId==0){
					$GLOBALS["ISC_CLASS_DB"]->Query("rollback");
					return false;
				}
			 ...
			 */
			if ($isImport) {
				//alandy_2011-6-14 modify.
				$msg="";
				$msg=$GLOBALS["ISC_CLASS_DB"]->GetErrorMsg();
...
//blessen
		...
		public function _GetProductList($Start, $SortField, $SortOrder, &$NumProducts, $fields='', $AddLimit=true, $exportPrimaryImage=false)
		{
			...
			if($fields == '') {
				$fields = "DISTINCT(p.productid), p.prodname, p.prodvariationid, p.prodprice, prodinvtrack, p.prodcode, p.proddesc, IF(prodinvtrack = 0, 0, prodcurrentinv) AS currentinv, prodvisible, prodlowinv, prodcalculatedprice, p.prodfeatured, p.prodvendorfeatured, t.imagefile AS tinythumb, p.prodistaxable ";
			}
...
//blessen
			...
			if($AddLimit) {
				if(isset($_REQUEST['w']) && $_REQUEST['w']=='orderSearchProducts') { 
					$limit = $GLOBALS["ISC_CLASS_DB"]->AddLimit($Start, 50);
				}
				else {
...
/*zcs=>old version of searching categories
...
*/
//zcs=>new version of searching category
$categorySearch = FALSE;
$searchCategories = array();
if(isset($array['category']) && $array['category']){
	$categorySearch = TRUE;
	$searchCategories[] = (int) $array['category'];
}
if(isset($array['subcategory']) && $array['subcategory']){
	$categorySearch = TRUE;
	$searchCategories[] = (int) $array['subcategory'];
}elseif(isset($array['category']) && $array['category']){//not select subcategory, check if select Main Category
	$categorySearch = TRUE;
	$searchCategories = array_merge($searchCategories, $this->_GetChildCats($array['category']));
}
if($categorySearch) {
	$queryWhere .= " ca.categoryid IN (" . implode(',', $searchCategories) . ") AND ";
}
//<=zcs
			if(isset($array['searchQuery']) && $array['searchQuery'] != "") {
				...
				/********* Original Code
				$search_query = $GLOBALS['ISC_CLASS_DB']->QuoteEx(isc_strtolower($array['searchQuery']));//zcs=>Fix BUG,escape additional characters
				...
				/*********************************************************/
					if(isset($_REQUEST['ymmID']) && $_REQUEST['ymmID']!='0') {
						...
						$YmmBedSize=$ymmRow['cab_size'];
						$YmmCabSize=$ymmRow['bed_size'];
					}
				...
				///  Code Added by Mayank
					if(isset($_REQUEST['ajaxsearchtype']) && $_REQUEST['ajaxsearchtype']=='2') {
						...
						//$queryWhere .= "((" . $GLOBALS["ISC_CLASS_DB"]->FullText($fulltext_fields, $search_query, true);
						$queryWhere .= "(( ";
						$queryWhere .= "lower(p.prodcode) like '%" . $GLOBALS['ISC_CLASS_DB']->Quote($search_query) . "%' ";
						$queryWhere .= "OR lower(p.prodcode) = '" . $GLOBALS['ISC_CLASS_DB']->Quote($search_query) . "') ";
						...
						//--------------------------------------------------------------------------//
						if(isset($_REQUEST['ymmID']) && $_REQUEST['ymmID']!='0') {
							if( isset($YmmBedSize) && $YmmBedSize != '' && $YmmBedSize != 0 )
							{
								$queryWhere .= " AND ( ( (iv.VQbedsize = '$YmmBedSize') OR (iv.VQbedsize regexp ';' AND iv.VQbedsize regexp '$YmmBedSize') ) OR ( ( iv.bedsize_generalname = '$YmmBedSize' ) OR (  iv.bedsize_generalname regexp ';' AND iv.bedsize_generalname regexp '$YmmBedSize' ) ) ) ";
							}
						}
						else {
							if(isset($_REQUEST['ymmbedsize']) && $_REQUEST['ymmbedsize']!='') {
								$queryWhere .= " AND ( ( (iv.VQbedsize = '".$_REQUEST['ymmbedsize']."') OR (iv.VQbedsize regexp ';' AND iv.VQbedsize regexp '".$_REQUEST['ymmbedsize']."') ) OR ( ( iv.bedsize_generalname = '".$_REQUEST['ymmbedsize']."' ) OR (  iv.bedsize_generalname regexp ';' AND iv.bedsize_generalname regexp '".$_REQUEST['ymmbedsize']."' ) ) ) ";
							}
						}
						//--------------------------------------------------------------------------------//
						...
						//--------------------------------------------------------------------------//
						if(isset($_REQUEST['ymmID']) && $_REQUEST['ymmID']!='0') {
							if( isset($YmmCabSize) && $YmmCabSize != '' && $YmmCabSize != 0 )
							{
								$queryWhere .= " AND ( ( (iv.VQcabsize = '$YmmCabSize') OR (iv.VQcabsize regexp ';' AND iv.VQcabsize regexp '$YmmCabSize') ) OR ( ( iv.cabsize_generalname = '$YmmCabSize' ) OR (  iv.cabsize_generalname regexp ';' AND iv.cabsize_generalname regexp '$YmmCabSize' ) ) ) ";
							}
						}
						else {
							if(isset($_REQUEST['ymmcabsize']) && $_REQUEST['ymmcabsize']!='') {
								$queryWhere .= " AND ( ( (iv.VQcabsize = '".$_REQUEST['ymmcabsize']."') OR (iv.VQcabsize regexp ';' AND iv.VQcabsize regexp '".$_REQUEST['ymmcabsize']."') ) OR ( ( iv.cabsize_generalname = '".$_REQUEST['ymmcabsize']."' ) OR (  iv.cabsize_generalname regexp ';' AND iv.cabsize_generalname regexp '".$_REQUEST['ymmcabsize']."' ) ) ) ";
							}
						}
						//--------------------------------------------------------------------------------//
						...
						//$queryWhere .= "((" . $GLOBALS["ISC_CLASS_DB"]->FullText($fulltext_fields, $search_query, true);
						$queryWhere .= "(( ";
						$queryWhere .= "lower(p.prodname) like '%" . $GLOBALS['ISC_CLASS_DB']->Quote($search_query) . "%' ";
						$queryWhere .= "OR lower(p.prodcode) = '" . $GLOBALS['ISC_CLASS_DB']->Quote($search_query) . "') ";
				...
				***********************************************************************************/
				if(isset($_REQUEST['w']) && $_REQUEST['w']=='orderSearchProducts') {
					// Add the join for the fulltext column
				//$joinQuery .= " INNER JOIN [|PREFIX|]product_search ps ON p.productid=ps.productid LEFT JOIN [|PREFIX|]import_variations iv ON p.productid=iv.productid // commented as no need of product_search table
				$joinQuery .= " LEFT JOIN [|PREFIX|]import_variations iv ON p.productid=iv.productid 
			...
			//zcs=>for searching series
			if(isset($array['series']) && $array['series']) {
				$queryWhere .= sprintf("brandseriesid = '%d' AND ", $array['series']);
			}
			//<=zcs
...
//blessen  error may come if wysiwyg editor is disabled from admin panel
		...
		**/
		...
		public function SearchProducts()
		{
			...
			$GLOBALS['CategoryOptions'] = $GLOBALS["ISC_CLASS_ADMIN_CATEGORY"]->GetCategoryAsOptions(FALSE);
			$GLOBALS['SubCategoryOptions'] = $GLOBALS["ISC_CLASS_ADMIN_CATEGORY"]->GetCategoryAsOptions(TRUE);
			//<=zcs
			
			$GLOBALS['ISC_CLASS_ADMIN_BRANDS'] = GetClass('ISC_ADMIN_BRANDS');
			$GLOBALS['BrandNameOptions'] = $GLOBALS['ISC_CLASS_ADMIN_BRANDS']->GetBrandsAsOptions();
			$GLOBALS['SeriesNameOptions'] = $GLOBALS['ISC_CLASS_ADMIN_BRANDS']->GetSeriesOptions();//zcs=Series
...
//blessen
		...
		*/
		...
		$options .= '<option value="-1">'.GetLang("NewVariationsRecord").'</option>';
            return $options;
        }
		...
		*/
        ...
        // Add by NI_20100909_Jack
        // rebuild ymms cached data
        public function updateYmmsCacheData(){
            $GLOBALS['ISC_YMMS'] = GetClass('ISC_YMMS');
            $GLOBALS['ISC_YMMS']->setYmmsData();
            $GLOBALS['ISC_CLASS_LOG']->LogSystemSuccess(array('general', 'YMM back_end'),'YMM cache file is updated.','YMM cache file is updated.');
	...
	//add by Ni_20101027_Mike  for ajax remove
