	...
	class SHIPPING_BYTOTAL extends ISC_SHIPPING
	{
		...
		*/
		...
		public function GetQuote()
		{
            ...
            $lower = $upper = 0;
            foreach ($this->rules['lower'] as $key => $value) {
                if ($value < $this->_subtotal && $this->rules['upper'][$key] > $this->_subtotal) {
                    $lower = $value;
                    $upper = $this->rules['upper'][$key];
                    $this->_shippingcost = $this->rules['cost'][$key];
                    break;
                }
            }
            
			$pi_quote = new ISC_SHIPPING_QUOTE($this->GetId(), $this->GetName(), $this->_shippingcost, $this->_description, '', $lower, $upper);
			return $pi_quote;
		}
