	...
	public function ProductStatsByNumViewsGrid()
	{
		...
		if ($total_products > 0) {
            ...
            */                                           //changed field name and commented
            ...
            // How many products are there in total?
            $rescount = $GLOBALS['ISC_CLASS_DB']->Query($mainQuery);
            
            $result = $GLOBALS['ISC_CLASS_DB']->Query($countQuery);

            $row = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
            if($_REQUEST['showby']=='products') { /* Baskaran */
                $total_records = $GLOBALS['ISC_CLASS_DB']->CountResult($result);
            }
            else {
                $total_records = $GLOBALS['ISC_CLASS_DB']->CountResult($rescount);
            }
