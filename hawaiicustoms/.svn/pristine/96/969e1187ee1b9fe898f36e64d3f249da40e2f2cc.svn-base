	...
	<table class="OuterPanel">
	  <tr>
		...
		<tr>
				...
				<tr>
					...
					<td style="padding-bottom: 3px;">
						...
						<div id="usedforcatdiv" style="padding-left:25px">
							...
							<img onmouseout="HideHelp('d7');" onmouseover="ShowHelp('d7', '%%LNG_ChooseCategories%%', '%%LNG_ChooseCategoriesHelp%%')" src="images/help.gif" width="24" height="16" border="0">
			...
			<table class="Panel">
				<tr>
					<td class="Heading2" colspan=3>Promotion Pages</td>
				</tr>
				<tr>
					<td class="FieldLabel">Promotional URL:</td>
					<td>
					<table border="0" cellpadding="0" cellspacing="0">
						<tbody id="promourltable">
							%%GLOBAL_PromotionalURL%%
							<tr id="promourl_rowtemplate" style="display:none">
								<td width="50%"><input type="text" name="promotionurltemp[]" class="Field500">
								<input type="hidden" name="promotionidhiddentemp[]" class="Field500" value="0">
								</td>
								<td nowrap="nowrap" class="Right" style="padding: 6px; vertical-align: top;">
									<a onclick="AddRow(this); return false;" href="#">
										<img border="0" alt="" src="images/addicon.gif">
									</a>
									<a onclick="RemoveRow(this); return false;" href="#">
										<img border="0" alt="" src="images/delicon.gif">
									</a>
								</td>
							</tr>
						</tbody>
	...
	<script type="text/javascript">
		...
		function AddRow(element) {
			var itemRow = $('#promourl_rowtemplate');
			var newRow = $(itemRow).clone(true);
			$(newRow).find('input:text').val('');
			$(newRow).find('input:hidden').val('0');
			$(newRow).appendTo($(itemRow).parents('#promourltable'));
			rowId = $(newRow).get(0).rowIndex;
			$(newRow).attr('id', 'promourl_'+rowId);

			$(newRow).find('input:text').attr('name', 'promotionurl[]');
			$(newRow).find('input:hidden').attr('name', 'promotionidhidden[]');
			
			$(newRow).show();
		}

		function RemoveRow(element) {
			if(!confirm("%%LNG_ConfirmRemovePromoURL%%")) {
				return false;
			}
			var kids = $('#promourltable').children().length;
			if(kids == 2)
			{
				$(element).parent().parent().find('input:text').val('');
