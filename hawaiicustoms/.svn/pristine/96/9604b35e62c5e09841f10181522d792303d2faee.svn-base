	...
	class ISC_INSTALLVIDEO
	{
		...
		function _SetVideoData()
		{
			if(isset($_GET['product_id'])) {
				...
				$query = "SELECT videoid, videoprodid, videofile, videotype, isdownloaded, systemvideofile FROM [|PREFIX|]install_videos WHERE videoprodid='".$GLOBALS['ISC_CLASS_DB']->Quote($this->GetProductId())."' AND (videofile LIKE '%.flv' || systemvideofile LIKE '%.flv' || videofile LIKE '%.swf' || systemvideofile LIKE '%.swf') order by videosort asc";
