...
function show_product_review_form_wholeStart() {
	if(typeof(flagShow) == "undefined") {//zcs=only show once
		document.getElementById("rating_box").style.display = "";
		document.location.href = "#write_review";
		
		$('span.StarRatingSpan_wholeStart').stars({
			starWidth: 32,
			inputType: "select",
			cancelShow: true,
			cancelAfterStars: true,
	...
	flagShow = true;//zcs=only show once
}
...
//lguan_20100617: Check if the rating selection has been selected or not
function checkRatingSel(elementName) {
	var value = $('input[name='+elementName+']').val();
	if (value>=1 && value<=5) 
		return true;
	else
		return false;
...
function check_product_review_form() {
	...
	//lguan_20100709: Ratings are no longer mandatory
//	if(!checkRatingSel('revrating')) {
//		alert(lang.ReviewNoRatingOverall);
//		return false;
//	}
//	
//	//lguan_20100612: Verify for the extra ratings
//	if(!checkRatingSel('qualityrating')) {
//		alert(lang.ReviewNoRatingQuality);
//		return false;
//	}
//	if(!checkRatingSel('installrating')) {
//		alert(lang.ReviewNoRatingInstall);
//		return false;
//	}
//	if(!checkRatingSel('valuerating')) {
//		alert(lang.ReviewNoRatingValue);
//		return false;
//	}
//	if(!checkRatingSel('supportrating')) {
//		alert(lang.ReviewNoRatingSupport);
//		return false;
//	}
//	if(!checkRatingSel('deliveryrating')) {
//		alert(lang.ReviewNoRatingDelivery);
//		return false;
//	}
...
function showcallbestprice(dv,pleft){
	var p=$(dv).offset();
	if(pleft==null) {
        pleft=400;
    }
	$('#callbestprice').css({position:'Absolute',left:pleft,top:p.top-150}).find(".bestPricePopupMessage").html($(dv).attr('data')).end().show();
}
function hidecallbestprice(dv){
	$("#callbestprice").hide();
}


/**
 * prevent default for event object
 */
...
function showPqVqHelp(dv,pleft,qimg,qcmt){
	$("#pqvqhelp").show();
	var p=$(dv).offset();
	if(pleft==null) pleft=400;
	$('#pqvqhelp').css({position:'Absolute',left:pleft,top:p.top-150});
	$('#pqvqhelpimg').attr("src","/product_images/"+qimg);
	$('#pqvqhelpcmt').html(qcmt);
}

function hidePqVqHelp(dv){
	$("#pqvqhelp").hide();
}