	...
	class CHECKOUT_MIGSREDIR extends ISC_CHECKOUT_PROVIDER
	{
		...
		*/
		...
		public function TransferToProvider()
		{
			...
			$post['vpc_Merchant'] 		= $this->GetValue('merchantid');
			$post['vpc_Locale']			= 'en';
		...
		public function VerifyOrderPayment()
		{
			...
			if ($amount != number_format($this->GetGatewayAmount()*100, 0, '','')) {
				$GLOBALS['ISC_CLASS_LOG']->LogSystemError(array('payment', $this->GetName()), sprintf(GetLang('MIGSRedirectAmountIncorrect'), $amount, number_format($this->GetGatewayAmount()*100, 0, '','')));
