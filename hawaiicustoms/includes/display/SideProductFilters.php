...
CLASS ISC_SIDEPRODUCTFILTERS_PANEL extends PANEL
{
	...
	public function __construct()
	{
	    ...
	    if(isset($GLOBALS['ISC_CLASS_NEWSEARCH'])) {
		    //$GLOBALS['ISC_CLASS_NEWSEARCH'] = GetClass('ISC_SEARCH');
		    $this->searchterms = $GLOBALS['ISC_CLASS_NEWSEARCH']-> _searchterms;
		    ...
		    {
			    $_SESSION['searchterms']['subcategory'] = $this->searchterms['subcategory'];
		    ...
		    unset($_SESSION['searchterms']['subcategory']);
		    }
    
		    if(isset($params['series'])) // this is assigned to get the brand series value in other pages
		    $_SESSION['searchterms']['series'] = $params['series'];
		    else
		    unset($_SESSION['searchterms']['series']);
		    ...
		    if(isset($_SESSION['searchterms']['subcategory'])) {
			    $_GET['subcategory'] = $_REQUEST['subcategory'] = $_SESSION['searchterms']['subcategory'];
			    $sub_catg_qry = "select categoryid , catname from isc_categories where catname = '".$_SESSION['searchterms']['subcategory']."'";
			    $sub_catg_res = $GLOBALS['ISC_CLASS_DB']->Query($sub_catg_qry);
			    $sub_catg_arr = $GLOBALS['ISC_CLASS_DB']->Fetch($sub_catg_res);
			    $this->subcatgid = $sub_catg_arr['categoryid'];
		    }
    
		    if(isset($_SESSION['searchterms']['series'])) {
			    $_GET['brand_series'] = $_SESSION['searchterms']['series'];
		    ...
		    if(isset($_SESSION['catg_name']))
		    {
				$CurrentUrl .= MakeURLSafe(strtolower($_SESSION['catg_name'])).'/';
				if(isset($_SESSION['searchterms']['brand']))
				{
					$CurrentUrl .= "brand/".MakeURLSafe(strtolower($_SESSION['searchterms']['brand'])).'/';
				}
		    }
		    else 
		    {
		    	if(isset($_SESSION['searchterms']['brand']))
				{
					$CurrentUrl .= MakeURLSafe(strtolower($_SESSION['searchterms']['brand'])).'/';
				}
		    }
		    foreach($_SESSION['searchterms']['dynfilters'] as $key=>$value)
		    {
		    	$CurrentUrl .= strtolower($key).'/'.MakeURLSafe(strtolower($value)).'/';
		    }
		    $GLOBALS["LeftYmmBaseSearchUrl"] = GetConfig('ShopPath').'/'.$CurrentUrl;
	    }
	...
	function SetPanelSettings()
	{
		...
		//wirror_yin20101102: define the join statement
		$table_join = " LEFT JOIN isc_import_variations AS v ON v.productid = p.productid  
		...
		//wirror_yin20101102: get the join statement from $_GLOBALS
		$searchQueries['query'] = str_replace($GLOBALS['join_query'],$table_join,$searchQueries['query']);
		...
		if( isset($params['make']) ||  ( isset($params['make']) && isset($params['model']) && $model_flag_link == 1 )  || isset($params['year']) ) {
			...
			if(!$changeYMM )
			{
				...
				$GLOBALS['DynMsg'] = "<a href='#' onclick=\"deleteMMYcookies('".$this->path.$clear_mmy_filter_link."');\">Change My Vehicle</a>";
...
P3;
		...
		if( isset($params['search']) )
		{
			...
			$GLOBALS['QualiferValue'] .=	" var SearchStringVal = '".MakeURLSafe(strtolower($params['search']))."/'; ";
                ...
                //wirror_20101124: modify the ymm display by new mockup
                //alandy_2012-2-28,add year,make,model not show in this selector.
		//$dyn_top = $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("MyVehicleArea");
		$dyn_top = $GLOBALS['ISC_CLASS_TEMPLATE']->GetPanelContent("MyVehicleArea");
		...
		if(count($brand_id) > 0 || isset($params['brand'])) {
			...
			if(isset($params['brand']) && !empty($params['brand'])) {
				$dyn_flag = 1;
				//alandy_2012-1-12  modify.
				if(isset($GLOBALS['ISC_CLASS_PRODUCT'])){
					$params['brand'] = ucwords($GLOBALS['ISC_CLASS_PRODUCT']->GetBrandName());
				}
...
P2;
	...
	//zcs=>Return its parent name if it's a sub-category
	function _transCategoryName($subCategoryId){
		$sql = "
			SELECT p.catname AS parentName, sub.catname AS subName
			FROM [|PREFIX|]categories sub
			LEFT JOIN [|PREFIX|]categories p ON(sub.catparentid=p.categoryid)
			WHERE sub.categoryid='{$subCategoryId}'
		";
	 	$result = $GLOBALS['ISC_CLASS_DB']->Query($sql);
	 	$row = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
		return $row['parentName'] ? $row['parentName'] : $row['subName'];
	}
	//<==zcs
	...
	function GetCategoryListing($catgids , $catgcount, $str_link, $numrows)
	{
		...
		//alandy_2012-2-22 modify.
		if(isset($GLOBALS['ISC_SRCH_PCATG_NAME']) && !empty($GLOBALS['ISC_SRCH_PCATG_NAME'])){
			 $catg_id_selected = $GLOBALS['ISC_SRCH_CATG_ID'];
			 $catg_selected = $GLOBALS['ISC_SRCH_PCATG_NAME'];
		}elseif(isset($GLOBALS['ISC_SRCH_CATG_ID']) && isset($GLOBALS['ISC_SRCH_CATG_NAME'])) {    // this condition is when any main category is selected from the side bar
		    
			$catg_id_selected = $GLOBALS['ISC_SRCH_CATG_ID'];
		    //$catg_selected = $GLOBALS['ISC_SRCH_CATG_NAME'];//zcs=[Old version]
		    $catg_selected = $this->_transCategoryName($GLOBALS['ISC_SRCH_CATG_ID']);//zcs=Show his parent's name if he is a sub-category
		}
		
		//alandy_2012-2-21 add catvisible=1.
		if(count($catgids)>1){
			$sql="select catparentid from [|PREFIX|]categories where categoryid in(".implode(',', $catgids).") and catvisible=1 group by catparentid";
			$result=$GLOBALS['ISC_CLASS_DB']->Query($sql);
			while($rs=$GLOBALS['ISC_CLASS_DB']->Fetch($result)){
				$multicatg_selected[]=$rs['catparentid'];
			}
		}
		...
		if(empty($catg_selected)) { // this condition is used to show all categories   
			...
			$GLOBALS['LastChildClass']='';
			//alandy mark.category name list.
			//$GLOBALS['CategoryName'] = "<img src='$path/templates/default/images/check.gif'>&nbsp;&nbsp;".isc_html_escape($catg_selected);
			//var_dump($multicatg_selected);exit;
			$GLOBALS['multicatg_name'] = "";
			if(isset($multicatg_selected) && !empty($multicatg_selected)){
				foreach($multicatg_selected as $val){
					$catname=$this->GetCategoryname($val);
					//$catlist .="<li class='".$GLOBALS['LastChildClass']."'><a href='".$this->LeftCatLink($catname).$mmy_links."/category/".strtolower(MakeURLSafe($catname))."'>&nbsp;".isc_html_escape($catname)."</a></li>";
					$catlist .="<li class='".$GLOBALS['LastChildClass']."'><a href='".$this->LeftCatLink($catname)."'>&nbsp;".isc_html_escape($catname)."</a></li>";
					$GLOBALS['multicatg_name'] .= isc_html_escape($catname)." ";
				}
				
			}else{
				$catlist="<li class='".$GLOBALS['LastChildClass']."'><img src='$path/templates/default/images/check.gif'>&nbsp;&nbsp;".isc_html_escape($catg_selected)."</li>";
			}
			
			
			$GLOBALS['CategoryName'] = "&nbsp;&nbsp;".isc_html_escape($catg_selected);
			$GLOBALS['CategoryLink'] = $this->LeftCatLink($catg_selected).$mmy_links."/category/".strtolower(MakeURLSafe($catg_selected));
	 ...
	 */
	 ...
	 function GetCategoryname($categoryid)
	 {
	 	...
	 	$catname="";
	 	$sql= 'select catname from isc_categories where categoryid='.$categoryid;
	 	$result=$GLOBALS['ISC_CLASS_DB']->Query($sql);
	 	while($rs=$GLOBALS['ISC_CLASS_DB']->Fetch($result)){
	 		$catname=$rs['catname'];
	 	}
