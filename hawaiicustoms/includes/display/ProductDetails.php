<?php
	...
	CLASS ISC_PRODUCTDETAILS_PANEL extends PANEL
	{
		...
		public function SetPanelSettings()
		{
			...
			$GLOBALS['DetailProdallowpurchases']=0;
			$querytemp = "SELECT prodbrandid,prodallowpurchases FROM  [|PREFIX|]products where productid = ".$pid."  ";
			$resulttemp = $GLOBALS['ISC_CLASS_DB']->Query($querytemp);
			$brand = $GLOBALS['ISC_CLASS_DB']->Fetch($resulttemp);
			
			if($brand['prodallowpurchases']>0){
				$GLOBALS['DetailProdallowpurchases']=$brand['prodallowpurchases'];
			}
...
//            $GLOBALS['ISC_CLASS_PRODUCT']->GetProductId();
            ...
            //dada.wang_2011-12-30 add.
            $pqvqYear = isset($_COOKIE['last_search_selection']['year']) ? $_COOKIE['last_search_selection']['year'] : 0;
            $pqvqMake = isset($_COOKIE['last_search_selection']['make']) ? $_COOKIE['last_search_selection']['make'] : '';
            $pqvqModel = isset($_COOKIE['last_search_selection']['model']) ? $_COOKIE['last_search_selection']['model'] : '';
            
            $GLOBALS['Impvariations'] = ISC_REMOTE::getProductPQVQ($productid, $pqvqYear, $pqvqMake, $pqvqModel);
        ...
        $params = array();
        //$GLOBALS['newsearchpage'] = GetClass('ISC_NEWSEARCHPAGE_PANEL');   
                ...
                if($comp != '') {
                    ...
                    for($i = 0; $i < $cntproducts; $i++){
                        ...
                        if(($GLOBALS["ISC_CLASS_DB"]->countResult($sku) == 1) and $split[0] != '0') {
                            if(in_array($split[0],$tempArr))continue;
                                $originalarray[] = $split[0].",".$split[1].",".$split[2].",".$split[3];//zcs=add 3rd element
                                $tempArr[] = $split[0];
                        }                                                          
                    ...
                    for($i = 0; $i < $cntoriginal; $i++) {
                        ...
                        if(($i == 0) and ($orsplit[0] == 0)) {
                            $arrManipulate[] = $orsplit[0].",".$orsplit[1].",".$orsplit[2].",".$orsplit[3];//zcs=add 3rd element
                        }
                        else if(($i != 0) and ($orsplit[0] == '0')) {
                            $nothanks[] = $orsplit[0].",".$orsplit[1].",".$orsplit[2].",".$orsplit[3];//zcs=add 3rd element
                        }
                        else {
                            $arrManipulate[] = $orsplit[0].",".$orsplit[1].",".$orsplit[2].",".$orsplit[3];//zcs=add 3rd element
                        }
                      ...
                      $seltype  = $product['comp_type']; //blessen
						...
						if($i == 0 and $productid != 0) {
                            ...
                            <td><div $styleval id='compqty_".$productid."'>".
                                $this->BuildOption('compqty['.$productid.']', $sel, $split[3])//zcs=add limit
...
//						$complementary .= "<tr><td><input type='checkbox' name='rdprod[]' id='rdprod_".$i."' value='".$productid."' checked = 'checked'>"."<label for='rdprod_".$i."'>&nbsp;$split[2]</label>"."</td></tr>";
					    	...
					    	//zcs=>Record "limit" into session 
						$_SESSION['complementary'][$productid]['limit'] = $split[3];
						//<=zcs
...
//blessen
                ...
                if(isset($_SERVER["HTTP_REFERER"]) && trim(strtolower($_SERVER["HTTP_REFERER"]), '/')) {
                    ...
                    if($ref_arr['couponcodes'] != NULL && $ref_arr['couponcodes'] != '') {
						...
						if($GLOBALS['ISC_CLASS_DB']->CountResult($querycode) > 0)
						{
...
$GLOBALS['SNIPPETS']['ProductAddToCart'] = $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("ProductAddToCart");
		...
		//zcs=>
		if($GLOBALS['HideOfferButton']){
		    $GLOBALS['SNIPPETS']['OfferButton'] = '';
		}else{
		    $GLOBALS['SNIPPETS']['OfferButton'] = $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet('OfferButton');
		}
		//<=zcs
            ...
            $callForBestPrice = GetClass('ISC_PRODUCT_HELPER')->GetProductCallForBestPrice($product);
            $GLOBALS['DisplayScript'] = $GLOBALS['BestPrice'] = '';
            
            if ($callForBestPrice) {
                $GLOBALS['DisplayScript'] = $callForBestPrice['displayScript'];
...
//blessen
        ...
        */
        ...
        public function BuildOption($name, $selected=0, $limit = 10) {//zcs= add limit
	    //zcs=>limit check
	    if($limit <= 0){
		$limit = 10;
	    }
