			...
			if($GLOBALS['ISC_CLASS_SEARCH']->GetNumResults() > 0) {
				...
				while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($GLOBALS['SearchResults'])) {
					...
					else if($GLOBALS['results_page_flag'] == 0) // subcategory page
					{
							...
							$tiplink = "<a class='thickbox1' href='".$GLOBALS['ShopPath']."/catgbrand.php?categoryid=".$row['categoryid']."&url=$subcatg_link'  title=''><img src='$path/templates/default/images/fastlook_red.gif' border=0></a>";

							$imagelink = "<a class='thickbox' href='".$GLOBALS['ShopPath']."/catgbrand.php?categoryid=".$row['categoryid']."&url=$subcatg_link' title='' onmouseover='createtip(".$row['categoryid'].")' onmouseout='UnTip()'>";
							...
							if($row['seriesids'] != "") 
							{
								...
								if( $GLOBALS['ISC_CLASS_DB']->CountResult($seriesids_res) > 0) {
									while($seriesids_arr = $GLOBALS['ISC_CLASS_DB']->Fetch($seriesids_res))
									{
										...
										{
											$GLOBALS['CatgSeriesList'] .= "<br><a href='javascript:' onclick=\"checkanimate('".$row['categoryid']."')\">View Brands ></a><div id='".$row['categoryid']."' style='display:none'>";
										}
										else
										{	
										...
										$GLOBALS['CatgSeriesList'] .= "<span id='span".$row['categoryid'].$seriesids_arr['seriesid']."' style='display:none'><a class='thickbox1'  href='".$GLOBALS['ShopPath']."/catgbrand.php?seriesid=".$seriesids_arr['seriesid']."&catname=".MakeURLSafe($row['catname'])."&url=$series_link' class='' ><img src='$path/templates/default/images/fastlook_red.gif' border=0></a></span>";
							...
							$GLOBALS['ViewDetailsImage'] = "<a href='$path/catgbrand.php?categoryid=".$row['categoryid']."&url=$subcatg_link' class='thickbox'><img src='$path/templates/default/images/fastlook_red.gif'></a> ";
					...
					{      
							...
							$tiplink = "<a class='thickbox1' href='".$GLOBALS['ShopPath']."/catgbrand.php?seriesid=".$row['brandseriesid']."&catname=".MakeURLSafe($row['parentcatname'])."&url=$series_link'  title=''><img src='$path/templates/default/images/fastlook_red.gif' border=0></a>";

							$imagelink = "<a class='thickbox' href='".$GLOBALS['ShopPath']."/catgbrand.php?seriesid=".$row['brandseriesid']."&catname=".MakeURLSafe($row['parentcatname'])."&url=$series_link' title='' onmouseover='createtip(".$row['brandseriesid'].")' onmouseout='UnTip()'>";
							...
							if($row['subcatgids'] != "") 
							{
								...
								$subcatgids_qry = "select c.categoryid , c.catname as childcatname , p.catname as parentcatname from [|PREFIX|]categories c left join [|PREFIX|]categories p on c.catparentid = p.categoryid where c.categoryid in (".$subcatgids.")";
								...
								if( $GLOBALS['ISC_CLASS_DB']->CountResult($subcatgids_res) > 0) {
									while($subcatgids_arr = $GLOBALS['ISC_CLASS_DB']->Fetch($subcatgids_res))
									{
										...
										if($GLOBALS['EnableSEOUrls'] == 1)
										{
											...
											$GLOBALS['CatgSeriesList'] .= "<a href='".$series_link."/category/".MakeURLSafe(Strtolower($subcatgids_arr['parentcatname']))."/subcategory/".MakeURLSafe(Strtolower($subcatgids_arr['childcatname']))."' $tooltipscript>".$subcatgids_arr['childcatname']."</a>";		
										} 
										else
										{
											$catgs_link = $series_link."&category=".MakeURLSafe(Strtolower($subcatgids_arr['parentcatname']))."&subcategory=".MakeURLSafe(Strtolower($subcatgids_arr['childcatname']));
										...
										$GLOBALS['CatgSeriesList'] .= "<span id='span".$subcatgids_arr['categoryid'].$row['brandseriesid']."' style='display:none'><a class='thickbox1'  href='".$GLOBALS['ShopPath']."/catgbrand.php?categoryid=".$subcatgids_arr['categoryid']."&url=$catgs_link' class='' ><img src='$path/templates/default/images/fastlook_red.gif' border=0></a></span>";
							...
							$GLOBALS['ViewDetailsImage'] = "<a class='thickbox' href='".$GLOBALS['ShopPath']."/catgbrand.php?seriesid=".$row['brandseriesid']."&catname=".MakeURLSafe($row['parentcatname'])."&url=$series_link'><img src='$path/templates/default/images/fastlook_red.gif'></a> ";
...
//		                $GLOBALS['SearchResults'] = "<div>".$GLOBALS['SearchResultList']."</div>";
	...
	function IsProductMakeanOffer($seriesid=0,$brandname='',$categoryid=0) 
	{
		...
		$brand =  $GLOBALS['ISC_CLASS_DB']->Query("SELECT offer FROM [|PREFIX|]brands where brandname = '".$brandname."' and offer = 'yes' "); 
		if($brow = $GLOBALS['ISC_CLASS_DB']->Fetch($brand)) {
			return $makeanoffer = $brow['offer'];
		}

		$category = $GLOBALS['ISC_CLASS_DB']->Query("SELECT c.offer as childoffer , p.offer as parentoffer FROM [|PREFIX|]categories c LEFT JOIN [|PREFIX|]categories p on c.catparentid = p.categoryid where c.categoryid = '$categoryid' ");
		if($crow = $GLOBALS['ISC_CLASS_DB']->Fetch($category)) {
			if($crow['childoffer'] == 'yes') {
				return $makeanoffer = 'yes';
			}
			else if($crow['parentoffer'] == 'yes') {
				return $makeanoffer = 'yes';
			}
			else {
				return $makeanoffer = 'no';
			}
		}
		return $makeanoffer;
	}
