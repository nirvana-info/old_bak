...
include_once dirname(__FILE__).'/class.file_cache.php';
	...
	class ISC_REDEFINE_SEARCH
	{
		...
		function __construct()
		{
			...
			$GLOBALS['ISC_CLASS_DB']->Query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( 'column: ".$this->column."')");
		}
		...
		function _ProcessSearch()
		{
			...
			switch($column) {
				...
				default:
					...
					$fn = 'column='.$column.'_year='.(isset($params['year'])?$params['year']:'').'_make='.str_replace(' ', '_', isset($params['make'])?$params['make']:'').'_model='.str_replace('/', '_', str_replace(' ', '_', isset($params['model'])?$params['model']:''));
					$fc = new file_cache($fn);
					$fc->file_dir = isset($GLOBALS['ISC_CFG']["cache_file_folder"]) ? $GLOBALS['ISC_CFG']["cache_file_folder"]: '/var/tmp/tc_ymm';
					if( isset($GLOBALS['ISC_CFG']["cache_file_expire_time"]))
						$fc->$expire_time = $GLOBALS['ISC_CFG']["cache_file_expire_time"];
					$cache = $fc->get_cache();
					$GLOBALS['ISC_CLASS_DB']->Query("INSERT INTO isc_orderlogs(ordervalue) VALUES( 'cache1: ')");
...
$GLOBALS['ISC_CLASS_DB']->Query("INSERT INTO isc_orderlogs(ordervalue) VALUES( 'cache-op :')");
			return $output;
		...
		function searchotherfilters($params,$selected_value) {
				...
				$column = $this->column;
				unset($this->items['column'],$this->items['random'],$this->items['search_query'],$this->items[$column]);
				...
				$GLOBALS['ISC_CLASS_DB']->Query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( 'cache2:')");
				...
				if($column == 'make' || $column == 'model' || $column == 'submodel' || $column == 'year') {
					...
					/* 
						The below condition is added as the bedsize and cabsize values are reatained when clicking on YMM links.
					*/
		...
		function showfiltervalues($filter_array,$count_filter_array,$params)
		{
			...
			/*try
			{
				$fn = 'column='.$column.'_year='.$params['year'].'_make='.str_replace(' ', '_', $params['make']).'_model='.str_replace('/', '_', str_replace(' ', '_', $params['model']));
				if( !file_exists('/var/tmp/tc_ymm/'.$fn.'php') )
				{
				$fc = new file_cache($fn);
				$fc->file_dir = isset($GLOBALS['ISC_CFG']["cache_file_folder"]) ? $GLOBALS['ISC_CFG']["cache_file_folder"]: '/var/tmp/tc_ymm';
