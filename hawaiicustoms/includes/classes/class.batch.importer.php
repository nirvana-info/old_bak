     ...
     */
		...
		if($existing = $GLOBALS["ISC_CLASS_DB"]->Fetch($result)) {
			...
			*/
			...
			{
				//error_log("XXXXXXXXXXXXX  prodid " . $existing['productid'] ."  insert this record");
				$sqlinsert2 = "INSERT INTO `isc_import_variations` ( `productid`, `prodcode` , prodstartyear,prodendyear,prodmodel,prodsubmodel,prodmake,";
				$values = ") VALUES (".$existing['productid'].", '".$record['partnumber']."' ,'".$prodstartyear."','".$prodendyear."','".$prodmodel."','".$prodsubmodel."','".$prodmake."',";
				for($i = 0; $i < count($selected); $i++) {
...
*/
		...
		$defaults = array(
			...
			if ($added_or_not == 1)
			{
				...
				{
					$sqlinsert2 = "INSERT INTO `isc_import_variations` ( `productid`, `prodcode` ,prodstartyear,prodendyear,prodmodel,prodsubmodel,prodmake, ";
					$values = ") VALUES (".$GLOBALS['NewProductId'].", '".$record['partnumber']."' ,'".$prodstartyear."','".$prodendyear."','".$prodmodel."','".$prodsubmodel."','".$prodmake."',";
					for($i = 0; $i < count($selected); $i++) {
