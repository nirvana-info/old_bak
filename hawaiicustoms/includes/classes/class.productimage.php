 ...
 AND ti.imageisthumb='3' WHERE pi.imageprodid='%d' AND pi.imageisthumb='0' order by pi.imagesort asc", $GLOBALS['ISC_CLASS_DB']->Quote($this->GetProductId()));
        ...
        function CreateImageGallery()
        {
            ...
            for($z=0; $z<count($this->_prodimages); $z++)   {        
                ...
                $ThumbImage = $GLOBALS['ImageDirectory'].'/'.$this->_prodimages[$z]['thumbfile']; 
                    
                if(file_exists($ThumbImage) && $this->_prodimages[$z]['thumbfile'] != '')    {
                    $ThumbImage = $GLOBALS['ShopPath'].'/'.$GLOBALS['ImageDirectory'].'/'.$this->_prodimages[$z]['thumbfile'];        
                    $ThumbImage = '<img src="'.$ThumbImage.'" alt="Image '.($z+1).'" />'; 
                }
                else    {
                    if(file_exists(ISC_BASE_PATH."/".$GLOBALS['ImageDirectory'].'/'.$this->_prodimages[$z]['imagefile']))                     {
                          $ThumbImage = $this->ScaleImage($GLOBALS['ShopPath']."/".$GLOBALS['ImageDirectory'].'/'.$this->_prodimages[$z]['imagefile'], 70, 70, ISC_BASE_PATH."/".$GLOBALS['ImageDirectory'].'/'.$this->_prodimages[$z]['imagefile']);
                    }
                    else    {             
                ...
                $ImageGallery   .= '<li>
                                    <a class="thumb" href="'.$GLOBALS['ShopPath'].'/'.$GLOBALS['ImageDirectory'].'/'.$this->_prodimages[$z]['imagefile'].'" title="Image '.($z+1).'">
                                        <span class="span_cls">
                                        '.$ThumbImage.'                   
                                        </span>
                                    </a>       
            ...
            $GLOBALS['ImageGallery']  = $ImageGallery;
            
            if(count($this->_prodimages) <= 1)    {
                $GLOBALS['HideControls'] = "none";
            }
            else    {
                $GLOBALS['HideControls'] = "";   
            }
            //
        }
