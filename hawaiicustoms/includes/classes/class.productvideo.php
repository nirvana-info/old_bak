	...
	class ISC_PRODUCTVIDEO
	{
		...
		function _SetVideoData()
		{
			if(isset($_GET['product_id'])) {
				...
				$query = "SELECT videoid, videoprodid, videofile, videotype, isdownloaded, systemvideofile FROM [|PREFIX|]product_videos WHERE videoprodid='".$GLOBALS['ISC_CLASS_DB']->Quote($this->GetProductId())."' AND (videofile LIKE '%.flv' || systemvideofile LIKE '%.flv' || videofile LIKE '%.swf' || systemvideofile LIKE '%.swf') order by videosort asc";
            ...
            */
            for($z=0; $z<count($this->_prodvideos); $z++)   { 
                ...
                if(strpos($filename, 'http://')!==false)    {
                    ...
                    $VideoLink = $GLOBALS['ShopPath'].'/'.GetConfig('VideoDirectory').'/'.$filename; 
                }
                $VideoGallery   .= '<tr>
                                        <td>
                                            <a style="color:#CCCCCC"  href="javascript:loadVideo(\''.$VideoLink.'\');" title="Video '.($z+1).'">  
                                                Video'.($z+1).'
                                            </a>
                                        </td>
                                    </tr>';
                if($z==0)    {
                       //$FirstVideo = $GLOBALS['ShopPath'].'/'.GetConfig('VideoDirectory').'/'.$filename; 
                       $FirstVideo = $VideoLink;          
                       $GLOBALS['FirstVideo'] = 'loadVideo(\''.$FirstVideo.'\')';
                }
            }
            $GLOBALS['VideoGallery']  = $VideoGallery;
            //
        }
		...
		function ShowVideo()
		{
			if($video = $this->GetVideo()) {
				...
				if($this->GetCurrentVideo() == 0) {
					...
					$GLOBALS['PrevLink'] = sprintf("%s/productvideo.php?product_id=%d&current_video=%d", $GLOBALS['ShopPath'], $this->GetProductId(), $this->GetCurrentVideo()-1);
				}
				...
				if($this->GetNumVideos()-1 == $this->GetCurrentVideo()) {
					...
					$GLOBALS['NextLink'] = sprintf("%s/productvideo.php?product_id=%d&current_video=%d", $GLOBALS['ShopPath'], $this->GetProductId(), $this->GetCurrentVideo()+1);
				}
				...
				$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("productvideo");
