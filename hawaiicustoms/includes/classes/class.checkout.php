...
class ISC_CHECKOUT
{
	...
	public function HandlePage()
	{
		...
		switch($action) {
			...
			case "choose_addonproducts": {
				$this->ChooseAddonProducts();
				break;
			}
			case "save_addonproducts": {
				$this->SaveAddonProducts();
				break;
			}
	...
	private function Checkout()
	{
		...
		// Wirror_20110525: ensure there are other products except add-on products
		$this->CheckAddonProducts();
	...
	private function ExpressCheckout()
	{
		...
		if(!isset($_SESSION['CHECKOUT']['SUBTOTAL_COST'])) {
			...
			// Reload this page
			header('Location: '.GetConfig('AppPath').'/checkout.php');
			exit;
		}
		...
		/* Added the google checkout button in checkout first step  */
		...
		if (!empty($product_array)) {
			foreach (GetAvailableModules('checkout', true, true) as $module) {
				if($module['id'] == 'checkout_googlecheckout')
				{
	...
	{
		$phone_msg = " <em><font size='1pt'>Never used for marketing. Only if we need to call you about an order.</font></em></dd>";
		...
		foreach (array_keys($fields) as $fieldId) {
			...
			/*
			 * if phone,then show msg.
			 * 2011-12-12 alandy add.
			 */
			if($fields[$fieldId]->record['formfieldprivateid'] == 'Phone'){
				$GLOBALS['CompiledFormFields'] .= substr_replace($fields[$fieldId]->loadForFrontend(),'',strripos($fields[$fieldId]->loadForFrontend(),'</dd>')) . $phone_msg . "\n";
			}else{
	...
	/**
	 * Generate the add-on products options for the express checkout.
	 */
	private function GenerateAddonProductOptions($inventory, $selected){
		$options = "";
		$i = 1;
		while($i <= $inventory){
			if($i == $selected){
	...
	/**
	 * Show the add-on products in the checkout steps
	 */
	private function ChooseAddonProducts(){
		...
		if(!GetConfig('GuestCheckoutEnabled') && !CustomerIsSignedIn()) {
			...
			if(isset($_SESSION['CHECKOUT']['CHECKOUT_TYPE']) && $_SESSION['CHECKOUT']['CHECKOUT_TYPE'] == 'express') {
				header("Location: ".GetConfig('ShopPath').'/checkout.php?action=express');
			}
			else {
				header("Location: ".GetConfig('ShopPath').'/checkout.php?action=checkout');
			}
			exit;
		}
		...
		$GLOBALS['AddonProductList'] = $this->GenerateExpressCheckoutAddonProducts(true);
		...
		if('' == $GLOBALS['AddonProductList']){
			...
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetPageTitle(GetConfig('StoreName') . " - " . GetLang('ChooseAddonProducts'));
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("checkout_addonproducts");
			$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
		}
	}
	
	/**
	 * Save the chosen add-on products in the checkout steps
	 */
	private function SaveAddonProducts(){
		// If guest checkout is not enabled and the customer isn't signed in then send the customer
		// back to the beginning of the checkout process.
		if(!GetConfig('GuestCheckoutEnabled') && !CustomerIsSignedIn()) {
			@ob_end_clean();
		...
		$this->AddAddonProductsToCart();

		// We've saved the add-on products- to the next step we go!
		@ob_end_clean();
		header(sprintf("location: %s/checkout.php?action=confirm_order", $GLOBALS['ShopPath']));
		die();
	}
	...
	/**
	 * Add add-on products to the shopping cart
	 */
	public function AddAddonProductsToCart(){
		
		if(isset($_REQUEST['addonProduct'])){
			$this->RemoveAddonProductsInCart();
			$addonProduct = $_REQUEST['addonProduct'];
			$addonProductQuantity = $_REQUEST['addonProductQuantity'];
			$addonProductPrice = $_REQUEST['addonProductPrice'];
			$addonProducts = array();
			$GLOBALS['ISC_CLASS_CART'] = GetClass('ISC_CART');
			
			foreach($addonProduct as $key=>$val){
				$addonProducts[$key]['productid'] = $val;
				$addonProducts[$key]['quantity'] = $addonProductQuantity[$key];
				$addonProducts[$key]['price'] = $addonProductPrice[$key];
				
				$itemId = $GLOBALS['ISC_CLASS_CART']->api->AddItem($addonProducts[$key]['productid'],
										   $addonProducts[$key]['quantity'],
										   null, array(), null,
										   array('priceOverride'=>$addonProducts[$key]['price']));
			
			
				$GLOBALS['ISC_CLASS_CART']->api->SetItemValue($itemId, 'addon_product_id', $key);
		...
		}else if(isset($_REQUEST['addonProductPrice'])){
			$this->RemoveAddonProductsInCart();
		}
	...
	public function GenerateExpressCheckoutAddonProducts($onlyList = false)
	{
		...
		if(!empty($related_ymm)){
			...
			if(!empty($related_ymm['make']))$ymmCond[] = sprintf(" (v.prodmake in ('%s')) ", implode("','", $related_ymm['make']));
			if(!empty($related_ymm['model']))$ymmCond[] = sprintf(" (v.prodmodel in ('%s')) ", implode("','", $related_ymm['model']));
		...
		if($configedNum > 0){
			...
			while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)){
				...
				$GLOBALS['AddonProductThumbnail'] = ImageThumb($thumbnail_path,'javascript:void(0)','',
									       //$row['imagefile'],ProdLink($row['prodname']),'_blank',
									       array(
										//'title' => isc_html_escape($row['proddesc']),
										'onmouseover' => 'ExpressCheckout.ShowAddonProductImg(this)',
										'onmouseout' => 'ExpressCheckout.HideAddonProductImg(this)'
									       ));
				...
				$GLOBALS['AddonProductDescription'] = empty($row['description']) 
									? ''
			...
			}else if('' != $GLOBALS['AddonProductList']){
	...
	/**
	 * Get all related ymm of products in shopping cart
	 */
	public function GetYmmsInCart(){
		$ymms= array();
		if(!isset($GLOBALS['ISC_CLASS_CART']))
			$GLOBALS['ISC_CLASS_CART'] = GetClass('ISC_CART');
		$product_array = $GLOBALS['ISC_CLASS_CART']->api->GetProductsInCart();
		
		foreach($product_array as $item=>$prod){
			if(!isset($prod['addon_product_id'])){
				if($prod['year'] && !in_array($prod['year'], $ymms['year']))$ymms['year'][] = $prod['year'];
				if($prod['make'] && !in_array($prod['make'], $ymms['make']))$ymms['make'][] = $prod['make'];
				if($prod['model'] && !in_array($prod['model'], $ymms['model']))$ymms['model'][] = $prod['model'];
			}
		}
	...
	/**
	 * Get all add-on products in shopping cart
	 */
	public function GetAddonProductsInCart(){
		$products = array();
		if(!isset($GLOBALS['ISC_CLASS_CART']))
			$GLOBALS['ISC_CLASS_CART'] = GetClass('ISC_CART');
		$product_array = $GLOBALS['ISC_CLASS_CART']->api->GetProductsInCart();
		...
		foreach($product_array as $item=>$prod){
			if(isset($prod['addon_product_id'])){
				$products[$prod['product_id']] = array(
								       'item_id' => $item,
								       'addon_id'=> $prod['addon_product_id'],
	...
	/**
	 * Remove all add-on products in shopping cart
	 */
	public function RemoveAddonProductsInCart(){
		if(!isset($GLOBALS['ISC_CLASS_CART']))
			$GLOBALS['ISC_CLASS_CART'] = GetClass('ISC_CART');
		$product_array = $GLOBALS['ISC_CLASS_CART']->api->GetProductsInCart();
		...
		foreach($product_array as $item=>$prod){
			if(isset($prod['addon_product_id'])){
				$GLOBALS['ISC_CLASS_CART']->api->RemoveItem($item);
			}
		}
	}
	...
	private function ConfirmOrder()
	{
		...
		if (!$GLOBALS['ISC_CLASS_CART']->api->AllProductsInCartAreIntangible()) {
			...
			if(!isset($_SESSION['CHECKOUT']['SHIPPING'])) {
				@ob_end_clean();
				header(sprintf("location:%s/checkout.php?action=choose_shipper", $GLOBALS['ShopPath']));
				die();
			}
		}
	...
	*/
	private function SaveShippingProvider()
	{
		...
		@ob_end_clean();
		header(sprintf("location: %s/checkout.php?action=choose_addonproducts", $GLOBALS['ShopPath']));
		die();
	}
	...
	private function SaveMultipleShippingAddresses()
	{
		...
		if(!gzte11(ISC_MEDIUMPRINT) || !GetConfig('MultipleShippingAddresses') || !isset($_POST['multiaddress'])) {
			@ob_end_clean();
			header("Location: ".GetConfig('ShopPath').'/checkout.php?action=choose_shipping_address&type=single');
			exit;
		}
	...
	**/
	...
	{
        $cart = GetClass('ISC_CART');
		//New Checking added by Simha to reconcile duplicate coupons applied
	...
	private function ChooseShippingProvider()
	{
		...
		if(!isset($_SESSION['CHECKOUT']['SUBTOTAL_COST'])) {
			$GLOBALS['ISC_CLASS_CART']->SetCartValues();
			// Reload this page
			header('Location: '.GetConfig('AppPath').'/checkout.php?action=choose_shipper');
			exit;
		}
...
// 			$GLOBALS['HideFullNameField'] = 'display: none';
	...
	public function SavePendingOrder()
	{
        ...
        $companyGiftCertificateAmount = $_SESSION['CHECKOUT']['PENDING_DATA']['COMPANYGIFTCERTIFICATE_AMOUNT'];
        ...
        if(isset($_SESSION['CART']['COMPANYGIFTCERTIFICATES']) && is_array($_SESSION['CART']['COMPANYGIFTCERTIFICATES'])) {
			$companyGiftCertificates = $_SESSION['CART']['COMPANYGIFTCERTIFICATES'];
			// Now we check that the gift certificates can actually be applied to the order
			$GLOBALS['ISC_CLASS_COMPANY_GIFT_CERTIFICATES'] = GetClass('ISC_COMPANYGIFTCERTIFICATES');
			$badCertificates = array();
			$remainingBalance = 0;
			$GLOBALS['ISC_CLASS_COMPANY_GIFT_CERTIFICATES']->CompanyGiftCertificatesApplicableToOrder($orderTotal, $companyGiftCertificates, $remainingBalance, $badCertificates);
			// One or more gift certificates were invalid so this order is now invalid
			if(count($badCertificates) > 0) {
				$badCertificatesList = '<strong>'.GetLang('BadGiftCertificates').'</strong><ul>';
				foreach($badCertificates as $code => $reason) {
			...
			// This order was entirely paid for using gift certificates but the totals don't add up
			else if($orderTotal == $companyGiftCertificateAmount && $remainingBalance > 0) {
				$pendingOrderResult = array(
					'error' => GetLang('OrderTotalStillRemainingCertificates')
				);
				return $pendingOrderResult;
			...
			// Order was entirely paid for using gift certificates
			else if($orderTotal == $companyGiftCertificateAmount) {
				$providerId = 'companygiftcertificate';
				$verifyPaymentProvider = false;
				$redirectToFinishOrder = true;
			}
		}
		...
		if($verifyPaymentProvider) {
			...
			if (isset($_POST['store_credit']) && $_POST['store_credit'] == 1 && $customer['custstorecredit'] > 0) {
				...
				// Otherwise we can use the store credit. Subtract store credit from users account and send them to the finished page
					...
					$creditToUse = $orderTotal-$giftCertificateAmount-$companyGiftCertificateAmount;
		...
		$cartItems = $GLOBALS['ISC_CLASS_CART']->api->GetProductsInCart();
			...
			if($GLOBALS['ISC_CLASS_DB']->CountResult($result) > '0') {
				$customergroupid = $groupid['customergroupid'];
			}
			else {
				$customergroupid = 0;
			}
		...
		if(isset($_SESSION['CHECKOUT']['CREATE_ACCOUNT'])) {
			...
			// 20110613 johnny modify ---- check guest email
			$sql="select customerid from [|PREFIX|]customers where isguest = 1 AND LOWER(custconemail)='" . $GLOBALS['ISC_CLASS_DB']->Quote(isc_strtolower($customerData['email']))."'";
			...
			/* Added for when the existing user checks guest user option the the a/c can't be created, only for the new customer a/c will be created and the mail will be sent -- Baskaran */
			...
			if($custId == ''){
			   ...
			   if($_SESSION['CHECKOUT']['PENDING_DATA']['GUEST_CHECKOUT'] && !$GLOBALS['Hasemail']){
		...
		$pendingOrder = array(
            ...
            "companygiftcertificateamount" => $companyGiftCertificateAmount,
			"giftcertificates" => $giftCertificates,
            "companygiftcertificates" => $companyGiftCertificates,
	...
	public function BuildOrderConfirmation()
	{
		...
		$query=$GLOBALS['ISC_CLASS_DB']->Query($sql);
		/*  2011-06-13 johnny remove ---- don't check email for guest
		while($rs=$GLOBALS['ISC_CLASS_DB']->Fetch($query)){
		    $GLOBALS['Hasemailflag2']="yes";
		}
		*/
	        if($_SESSION['Haslogin']==1){ $GLOBALS['Hasemailflag2']="no"; }	
		...
		if (isset($_REQUEST['couponcode']) && $_REQUEST['couponcode'] != '') {
			...
			elseif (!(CheckExistsCouponCode($code)) && CheckExistsCompanyGiftCertificateCode($code)){
				$cart = GetClass('ISC_CART');
				if($cart->api->ApplyCompanyGiftCertificate($code)) {
					// If successful show a message
					$GLOBALS['CheckoutSuccessMsg'] = GetLang('GiftCertificateAppliedToCart');
				}
				else {
    ...
    //                        echo $compproductid." a2 ".$compmainproductid;
                            $compprodname       = isc_html_escape($product['complementary'][$y]['comp_product_name']);
                            $compsku            = isc_html_escape($product['complementary'][$y]['comp_product_code']);
                            $compprice          = $product['complementary'][$y]['comp_original_price'];
                            $comppriceformat    = CurrencyConvertFormatPrice($product['complementary'][$y]['comp_original_price']);
    //                        $comptotal += $compprice;

                            $GLOBALS['CompProductTotal'] = CurrencyConvertFormatPrice(($compprice * $product['complementary'][$y]['quantity']));
                            $GLOBALS['complementaryrow'] .= "<tr>
                            <td>$compprodname</td>
                            <td align='center'>".$product['complementary'][$y]['quantity']."</td>
                            <td align='center' class='CartItemIndividualPrice'>$comppriceformat</td>
                            <td align='right' class='CartItemTotalPrice'><em class='ProductPrice'>".$GLOBALS['CompProductTotal']."</em></td>
                            </tr>";
		...
		// If we have any gift certificates, list those too
		if(!empty($orderSummary['companyGiftCertificates'])) {
			foreach($orderSummary['companyGiftCertificates'] as $certificate) {
				...
				$GLOBALS['CompanyGiftCertificateBalance'] = CurrencyConvertFormatPrice($certificate['cgcbalance']);
				$GLOBALS['CompanyGiftCertificateRemaining'] = CurrencyConvertFormatPrice($certificate['balanceremaining']);
				$GLOBALS['CompanyCertificateAmountUsed'] = CurrencyConvertFormatPrice($certificate['amountused']);
				//d($certificate);
				$GLOBALS['SNIPPETS']['CompanyGiftCertificates'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("ConfirmOrderCompanyGiftCertificate");
			}
		}
		...
		$_SESSION['CHECKOUT']['PENDING_DATA'] = array(
            ...
            "COMPANYGIFTCERTIFICATE_AMOUNT" => number_format($orderSummary['companyGiftCertificateTotal'], GetConfig('DecimalPlaces'), ".", "")
		);
...
//blessen
		...
		$adjustedTotal = $orderSummary['total']; // Add By NI_20100827_Jack
        //Add by NI_20100825_Jack
		...
		if (isset($_SESSION['CART']['COMPANYGIFTCERTIFICATES']) && is_array($_SESSION['CART']['COMPANYGIFTCERTIFICATES'])) {
			...
			$giftCertificateTotal = 0;
            // Because the session data's discount price wrong due to using discountrules -20% and product on sale
            // and GetProductIncat if do not update info , data is wrong too, cgc can not applied right first time
            //$product_array = $_SESSION['CART']['ITEMS']; 
			...
			foreach ($certificates as $certificate) {
                ...
                //var_dump($cgcproductused);
                //var_dump($cgcproductremain);
                //var_dump($hash_producttotal);
                //d(1);
//				if ($certificate['cgcbalance'] > $adjustedTotal) {
//					$certificate['balanceremaining'] = $certificate['cgcbalance'] - $adjustedTotal;
//					$certificate['amountused'] = $certificate['cgcbalance'] -$certificate['balanceremaining'];
//				}
//				else {
//					$certificate['amountused'] = $certificate['cgcbalance'];
//					$certificate['balanceremaining'] = 0;
//				}
                $certificate['balanceremaining'] = $cgcproductremain;
                $certificate['amountused'] = $cgcproductused;
				// Subtract from the adjusted order total
				$giftCertificateTotal += $certificate['amountused'];
				//$adjustedTotal -= $certificate['cgcbalance'];
                $adjustedTotal -= $certificate['amountused'];
				if($adjustedTotal < 0) {
					$adjustedTotal = 0;
				}
				$orderSummary['companyGiftCertificates'][] = $certificate;
			}
	...
	*/
	...
	/**
	 * Check if there are only add-on products in the cart
	 * @author Wirror Yin
	 */
	private function CheckAddonProducts(){
		
		$hasProducts = false;
		$addonProducts = array();
		foreach($_SESSION['CART']['ITEMS'] as $item){
			if(isset($item['addon_product_id'])){
				$addonProducts[] = $item['addon_product_id'];
			}else if(!$hasProducts){
				$hasProducts = true;
			}
		}
		...
		if(!$hasProducts) {
			$errors = GetLang('CheckoutNeedProducts');
		}
		/*
		else if(!empty($addonProducts)){
			$addonProductIds = implode(',', $addonProducts);
			$query = sprintf("SELECT p.prodname
					  FROM [|PREFIX|]addon_products ap
					  LEFT JOIN [|PREFIX|]products p ON p.productid=ap.productid
					  WHERE ap.id IN($addonProductIds)
					  AND (ap.status=0 OR
					        ap.id NOT IN
					        (
							SELECT t.id FROM
					  ...
					  ");
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
			$exist = $GLOBALS['ISC_CLASS_DB']->FetchOne($result);
			
			if($exist){
				$errors = GetLang('AddonProductsExpired');
			}
		}*/
