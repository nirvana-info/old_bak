	...
	<tr>
		<td>
			<ul id="tabnav">
                ...
                <li><a href="#" id="tab2" onclick="ShowTab(2)">%%LNG_YMMDetails%%</a></li>
			</ul>
		</td>
	</tr>
	<tr>
			...
			<div id="div0" style="padding-top: 10px;">
				...
				<table width="100%" class="Panel">
					...
					<tr>
						...
						<td>
							<input type="checkbox" id="subscribed" name="subscribed" %%GLOBAL_SubscribedChecked%% value="%%GLOBAL_Subscribed%%"/>
							%%LNG_Subscribed%%
						</td>
					</tr>
					<!--zcs=>-->
					<tr>
						...
						<td>
							<select id="status" name="status" class="Field300">
								%%GLOBAL_CustomerStatusOptions%%
							</select>
						</td>
					</tr>
					<!--<=zcs-->
					...
					<tr style="display: %%GLOBAL_HideStoreCredit%%;">
						...
						<td>
							%%GLOBAL_CurrencyTokenLeft%% <input type="text" id="custStoreCredit" name="custStoreCredit" class="Field50" value="%%GLOBAL_CustomerStoreCredit%%" %%GLOBAL_DisableStoreCredit%%> %%GLOBAL_CurrencyTokenRight%%
					...
					<tr>
						...
						<td>
							<input type="password" id="custPasswordConfirm" name="custPasswordConfirm" class="Field250" value="%%GLOBAL_CustomerPasswordConfirm%%">
        ...
        <div id="div2" style="padding-top:10px;">
            ...
            <div style="padding-bottom:5px">%%LNG_YMMDescription%%</div>
            
            <div class="MessageBox MessageBoxInfo" style="display: %%GLOBAL_CustomerYMMEmptyShow%%">%%GLOBAL_YMMNoDetailsWaring%%</div>
			<p class="Intro" style="display: %%GLOBAL_HideCustomerAddressButtons%%;">
				<input type="button" value="%%LNG_YMMAdd%%" onclick="addYMM();" class="SmallButton Field150" %%GLOBAL_CustomerAddressAddDisabled%% />
				<input type="button" value="%%LNG_DeleteSelected%%" name="DeleteAddressesButton" onclick="confirmDeleteYMM('','multi');" class="SmallButton Field150" %%GLOBAL_CustomerAddressDeleteDisabled%% />
			</p>
			<div class="GridContainer" style="display: %%GLOBAL_CustomerYMMEmptyHide%%">
				%%GLOBAL_CustomerYMMGrid%%
			</div><br />
			<br />
...
<script type="text/javascript"><!--
	...
	{
		return $("#IndexGrid").find("input[type='checkbox'][name='addresses[]']");
	}

	function selectedAddressBoxes()
	{
		return getAddressBoxes().filter("[checked='1']");
	}
	...
	function toggleYMMBoxes(status)
	{
		
		getYMMBoxes().each(function() { $(this).attr("checked", status); });
	}
	function getYMMBoxes()
	{
		
		return $("#IndexGrid1").find("input[type='checkbox'][name='ymms[]']");
	}
	...
	function confirmDeleteYMM(addressId,mode)
	{
		if(mode=='multi') {
			if (confirm("%%LNG_ConfirmDeleteCustomerYMM%%")) {
			MakeHidden('ymmdelete', addressId, 'frmCustomer');
			MakeHidden('ymmmode', mode, 'frmCustomer');
			document.getElementById("frmCustomer").action = "index.php?ToDo=deleteCustomerYMM";
			document.getElementById("frmCustomer").submit();
			return;
			}
	...
	function addYMM()
	{
		document.getElementById('frmCustomer').action = 'index.php?ToDo=addCustomerYMM';
		document.getElementById('frmCustomer').submit();
		return false;
	}
