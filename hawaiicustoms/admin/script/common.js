	...
	//zcs=>
	function display_message(id,message,timeout, type){//zcs=add 'type' param
		type = type || 'Success';//zcs=
		if($('#'+id+' .MessageBox').get() != "") {
			$('#'+id).fadeOut('slow');
			$('#'+id).html('<div class="MessageBox MessageBox' + type + '">'+message+'</div>').fadeIn('slow');//zcs=
		}
		else {
			$('#'+id).hide().html('<div class="MessageBox MessageBox' + type + '">'+message+'</div>').show('slow');//zcs=
		}
		if(timeout > 0) {
			window.setTimeout(function() { $('#'+id).hide('slow'); }, timeout);
		}
...
function openProductSelect(type, select, idlist, single, closeFocus) {
	var l = (screen.availWidth/2) - (1000/2) + 50;
	var t = (screen.availHeight/2) - (1080/2) + 50;
	...
	var w = window.open(windowLocation, 'productSelect'+select+'type'+type, "width="+width+",height=680,left="+l+",top="+t+",scrollbars=1");
	w.focus();
	return false;
}
...
function SelectSubCategory3(cid) {
    var phpUrl = 'getselectedsubcategory.php';
    $.ajax(
    {
        url: phpUrl,
...
function ProductSeries3(sid) {
    ...
    $.ajax(
    {
        ...
        success: function(data)
        {
            $('#selectseries3').html(data);
        }
    }
    ); 
}

function SelectSeriesBrands(sid) {
    var phpUrl = 'getseriesbrands.php';
    $.ajax(
    {
        url: phpUrl,
        type: 'GET', 
...
function SelectTestingSubCategory(cid) {
    var phpUrl = 'gettestselectedsubcategory.php';
    $.ajax(
    {
        url: phpUrl,
        type: 'GET', 
...
$(document).ready(function(){
	//allow datepicker accepts input
	$('.datepicker').each(function(){
		$(this).attr('readonly',false).attr("title","eg: 01/30/2011");
		$(this).keydown(function(event){
			if(event.keyCode == 13 || event.keyCode == 191 || event.keyCode == 8 || event.keyCode == 46) return true;
			if(!((event.keyCode>=48 && event.keyCode<=57)||(event.keyCode>=96 && event.keyCode<=105 ))){
			//include num btns on small keyboard.
				return false;
			}
		});
		$(this).blur(function(){
			var d = $(this).val();
			if((d != "") && (/^((0[0-9])|(1[0-2]))\/((0[1-9])|([1-2][0-9])|(3[0-1]))\/\d{4}$/.test(d) == false))
			{
				alert('Please input a valid date. Eg: 01/30/2011.');
			}
		});
	});
	$("img[src='images/calbtn.gif']").each(function(){
		$(this).parent().prev().attr("readonly",false).attr("title","eg: 01/30/2011");
		$(this).parent().prev().attr("onfocus","");
		$(this).parent().prev().keydown(function(event){
			if(event.keyCode == 13 || event.keyCode == 191 || event.keyCode == 8 || event.keyCode == 46) return true;
			if(!((event.keyCode>=48 && event.keyCode<=57)||(event.keyCode>=96 && event.keyCode<=105 ))){
			//include num btns on small keyboard.
				return false;
		...
		$(this).parent().prev().blur(function(){
			var ds = $(this).val();
			if((ds != "") && (/^((0[0-9])|(1[0-2]))\/((0[1-9])|([1-2][0-9])|(3[0-1]))\/\d{4}$/.test(ds) == false))
			{
				alert('Please input a valid date. Eg: 01/30/2011.');
			}
		});
