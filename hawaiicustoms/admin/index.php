<?php
	...
	*/
    ...
    $str = substr($querystring, 5,8);
    $stroccurance = substr($querystring, 5,12);
    if($self == '/admin/index.php' && $str != 'payOrder' && $stroccurance != 'paymentOrder') {
        if($_SERVER['HTTPS'] != 'off') {
			$http = "http://".$_SERVER['HTTP_HOST'];
			$loc = $http.$_SERVER['REQUEST_URI'];
