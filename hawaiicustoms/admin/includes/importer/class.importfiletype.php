...
*/
abstract class ISC_ADMIN_IMPORTFILETYPE
{
...
$query2 = "SELECT count(*) as count FROM [|PREFIX|]products  p LEFT JOIN [|PREFIX|]import_variations iv ON p.productid = iv.productid	 ".$where;
...
if ($new_fields[$id]['label'] == "") $new_fields[$id]['label'] = $id;
	...
	*/
	protected function LoadFieldData($fields, $templateid)
	{
		...
		$query = "SELECT * FROM [|PREFIX|]import_template_fields WHERE importtemplateid = '" . $GLOBALS['ISC_CLASS_DB']->Quote($templateid) . "' AND fieldtype = '" . $this->type_name . "'";
		...
		foreach ($row_data as $id => $row) {
			if (!isset($fields[$id])) {
				...
				//$query = "DELETE FROM [|PREFIX|]import_template_fields WHERE importtemplatefieldid = " . $row['importtemplatefieldid'];
