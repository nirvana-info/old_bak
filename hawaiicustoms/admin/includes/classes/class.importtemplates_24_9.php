	...
	public function HandleToDo($do)
	{
		...
		switch ($do) {
			...
			case 'editimporttemplate':
				...
				$query = "SELECT  importtemplatename  FROM [|PREFIX|]import_templates  WHERE importtemplateid = '" . $GLOBALS['ISC_CLASS_DB']->Quote($templateid) . "' ";
		    ...
		    case "listqualifiersimporttemplate":
 ...
 private function _BuildCategoryRows($parentid=0)
        {
            ...
            foreach($categorycache[$parentid] as $category) {
                ...
                if($GLOBALS['SubCats']) {
                    $GLOBALS['SubCats'] = sprintf('<ul class="SortableRowproducts SortableRow">%s</ul>', $GLOBALS['SubCats']);
                }
		...
		public function ManageQualifierAssociationsGrid(&$numQualifierAssociations)
		{
			...
			if($numQualifierAssociations > 0) {
				...
				$tempId = $_SESSION['TemplateId'];
if (isset($tempId))
					{
				$query = "SELECT fieldname FROM [|PREFIX|]import_template_fields where includeinexport = 1 and importtemplateid = ".$tempId." and fieldname = '".$row['column_name']."' ";
                $result = $GLOBALS['ISC_CLASS_DB']->Query($query);

				$GLOBALS['checked'] = "";
                if ( mysql_num_rows($result) > 0) $GLOBALS['checked'] = "checked";


					}               
                    
                    $GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("import.associations.manage.row");
