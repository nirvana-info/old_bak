...
class ISC_ADMIN_DESIGNMODE extends DesignMode
{
	...
	{
		$this->Set('BasePath', ISC_BASE_PATH."/templates/" . GetConfig('template') . "/");
		$this->Set('MasterBasePath', ISC_BASE_PATH.'/templates/__master/');
		$this->Set('DirList', 	array (
				'StyleSheet'=> "Styles",
				'Layout'	=> '',
				'Snippet'	=> "Snippets",
				'Panel'		=> "Panels"
   ...
   */
	protected function EditFile()
	{
		if (isset($_REQUEST['File'])) {
			...
			//if template files is found in the master template folder
			else if(!file_exists($this->BasePath.$this->FileName) && file_exists($this->MasterBasePath.$this->FileName)) {
				$FilePath = $this->MasterBasePath.$this->FileName;
			}
			else if(!file_exists($this->MasterBasePath.$this->FileName)) {
				exit;
			}
