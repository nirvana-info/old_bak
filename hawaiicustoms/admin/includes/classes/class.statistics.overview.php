...
class ISC_ADMIN_STATISTICS_OVERVIEW extends ISC_ADMIN_STATISTICS
{
	...
	*/
	public function Overview()
	{
        ...
        $GLOBALS['GraphToDo']   = "overviewStatsTop20Prods";  
        $GLOBALS['ProdSel'] = $GLOBALS['CatSel'] = $GLOBALS['SubcatSel'] = $GLOBALS['BrandSel'] = $GLOBALS['SeriesSel']  = '';         
        
        $GLOBALS['BestSellers']  = 'products';
        
        if(isset($_POST['bestsellers']) && $_POST['bestsellers']=='category')    {
		    // Load the top 20 categories grid
		    $GLOBALS['TopProductsGrid'] = $this->_Top20CategoriesGrid($from_stamp, $to_stamp, $num);
            $GLOBALS['GraphToDo'] = "overviewStatsTop20Cats"; 
            $GLOBALS['CatSel']    = "selected=selected"; 
            $GLOBALS['BestSellers']  = 'category';   
        }
        else if(isset($_POST['bestsellers']) && $_POST['bestsellers']=='subcategory')    {
            // Load the top 20 categories grid
            $GLOBALS['TopProductsGrid'] = $this->_Top20SubcategoriesGrid($from_stamp, $to_stamp, $num);
            $GLOBALS['GraphToDo']       = "overviewStatsTop20Subcats"; 
            $GLOBALS['SubcatSel']       = "selected=selected"; 
            $GLOBALS['BestSellers']     = 'subcategory';   
        }
        else if(isset($_POST['bestsellers']) && $_POST['bestsellers']=='brand')    {
            // Load the top 20 brands grid
            $GLOBALS['TopProductsGrid'] = $this->_Top20BrandsGrid($from_stamp, $to_stamp, $num);
            $GLOBALS['GraphToDo'] = "overviewStatsTop20Brds";
            $GLOBALS['BrandSel']  = "selected=selected"; 
            $GLOBALS['BestSellers']  = 'brand';   
        }
        else if(isset($_POST['bestsellers']) && $_POST['bestsellers']=='series')    {
            // Load the top 20 series grid
            $GLOBALS['TopProductsGrid'] = $this->_Top20SeriesGrid($from_stamp, $to_stamp, $num);
            $GLOBALS['GraphToDo'] = "overviewStatsTop20Sers";
            $GLOBALS['SeriesSel'] = "selected=selected";
            $GLOBALS['BestSellers']  = 'series';   
        }
        else    {
	...
	*/
	public function GetOverviewStatsData()
	{
		if(isset($_GET['from']) && is_numeric($_GET['from']) && isset($_GET['to']) && is_numeric($_GET['to'])) {
			...
			else if($num_days > 1 and $num_days <= 60) {
			...
			else if($num_days > 182 and $num_days <= 730) {
			...
			if($num_days <= 1) {
				...
				$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
				while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
					$isc_date_stamp =  mktime(0, 0, 0, date("m", $row['datestamp']), date("d", $row['datestamp']), date("Y", $row['datestamp']));
				...
				for($i = 0; $i < count($visitor_dates); $i++) {
					...
					$date_stamp = mktime(0, 0, 0, date("m", $visitor_dates[$i]['stamp']), date("d", $visitor_dates[$i]['stamp']), date("Y", $visitor_dates[$i]['stamp']));
	...
	*/
    ...
    public function GetOverviewStatsTop20CategoriesData()
    {
        ...
        if(isset($_GET['from']) && is_numeric($_GET['from']) && isset($_GET['to']) && is_numeric($_GET['to'])) {
            ...
            $query = "
                SELECT
                    ordprodname, rc.catname, rc.catcombine,
                    SUM(ordprodqty) AS numsold
                FROM
                    [|PREFIX|]order_products
                    INNER JOIN [|PREFIX|]orders ON (orderorderid=orderid)
               ...
               WHERE
                ...
                ORDER BY
                    numsold DESC
            ";      //
               //The query commented which uses subcategory results as well
            /*
            $query = "
                SELECT
                    ordprodname, c.catname, c.catcombine,
                    SUM(ordprodqty) AS numsold
                FROM
                    [|PREFIX|]order_products
                    INNER JOIN [|PREFIX|]orders ON (orderorderid=orderid)
            ...
            */
            ...
            if($GLOBALS['ISC_CLASS_DB']->CountResult($result) > 0) {
                while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
                    if(trim($row['catcombine']) !='' )    {
                        $CatName = trim($row['catcombine']);
                    }
                    else    {
                        $CatName = trim($row['catname']);
                    }   
                    $output .= sprintf("\t<slice title=\"%s\" pull_out=\"false\">%s</slice>\n", isc_html_escape(isc_convert_charset(GetConfig('CharacterSet'), 'UTF-8', $CatName)), (int) $row['numsold']);
                }
            }
    ...
    public function GetOverviewStatsTop20SubcategoriesData()
    {
    ...
    public function GetOverviewStatsTop20BrandsData()
    {
        ...
        if(isset($_GET['from']) && is_numeric($_GET['from']) && isset($_GET['to']) && is_numeric($_GET['to'])) {
            ...
            if($GLOBALS['ISC_CLASS_DB']->CountResult($result) > 0) {
                while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
                    $output .= sprintf("\t<slice title=\"%s\" pull_out=\"false\">%s</slice>\n", isc_html_escape(isc_convert_charset(GetConfig('CharacterSet'), 'UTF-8', $row['brandname'])), (int) $row['numsold']);
                }
            }
    ...
    public function GetOverviewStatsTop20SeriesData()
    {
        ...
        if(isset($_GET['from']) && is_numeric($_GET['from']) && isset($_GET['to']) && is_numeric($_GET['to'])) {
            ...
            if($GLOBALS['ISC_CLASS_DB']->CountResult($result) > 0) {
                while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
                    $CombinedSeriesName = $row['brandname'];
                    if(trim($row['seriesname']) != '')    {
                        $CombinedSeriesName.= " - ".$row['seriesname'];
                    }
                    $output .= sprintf("\t<slice title=\"%s\" pull_out=\"false\">%s</slice>\n", isc_html_escape(isc_convert_charset(GetConfig('CharacterSet'), 'UTF-8', $CombinedSeriesName)), (int) $row['numsold']);
                }
            }
	...
	*/
    ...
    /**
    *    Get a list of the top 20 categories and return them as <tr> tags
    */
    public function _Top20CategoriesGrid($FromStamp, $ToStamp, &$NumResults)
    {
        // Only fetch products this user can actually see
        $vendorRestriction = '';
        if($GLOBALS['ISC_CLASS_ADMIN_AUTH']->GetVendorId()) {
        ...
        */
        ...
        ", GetLang('StatsCategoryName'), GetLang('StatsNumSold'), GetLang('StatsRevenue'));
    ...
    *    Get a list of the top 20 categories and return them as <tr> tags
    */
    public function _Top20SubcategoriesGrid($FromStamp, $ToStamp, &$NumResults)
    {
        // Only fetch products this user can actually see
        $vendorRestriction = '';
        if($GLOBALS['ISC_CLASS_ADMIN_AUTH']->GetVendorId()) {
        ...
        ", GetLang('StatsSubcategoryName'), GetLang('StatsNumSold'), GetLang('StatsRevenue'));
        ...
        if($NumResults > 0) {
            ...
            while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
                ...
                ", GetLang('StatsNoTopSubcategories'));
        }
    ...
    *    Get a list of the top 20 categories and return them as <tr> tags
    */
    public function _Top20BrandsGrid($FromStamp, $ToStamp, &$NumResults)
    {
        // Only fetch products this user can actually see
        $vendorRestriction = '';
        if($GLOBALS['ISC_CLASS_ADMIN_AUTH']->GetVendorId()) {
        ...
        ", GetLang('StatsBrandName'), GetLang('StatsNumSold'), GetLang('StatsRevenue'));

        $NumResults = $GLOBALS['ISC_CLASS_DB']->CountResult($result);

        if($NumResults > 0) {      
            while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
                $prodlink = $row['brandname'];        
                $output .= sprintf("
                    <tr class=\"GridRow\" onmouseover=\"this.className='GridRowOver';\" onmouseout=\"this.className='GridRow';\">
                        <td nowrap height=\"22\">
                            %s
    ...
    *    Get a list of the top 20 categories and return them as <tr> tags
    */
    public function _Top20SeriesGrid($FromStamp, $ToStamp, &$NumResults)
    {
        // Only fetch products this user can actually see
        $vendorRestriction = '';
        if($GLOBALS['ISC_CLASS_ADMIN_AUTH']->GetVendorId()) {
        ...
        ", GetLang('StatsSeriesName'), GetLang('StatsNumSold'), GetLang('StatsRevenue'));
