<?php
	class ISC_ADMIN_SETTINGS
	{
		public $all_vars = array (
			'AllowPurchasing',
			'LimitCustomerLoginTimes',//zcs=
			'UserQA',//zcs=
			'AddonProductShownNum',//zcs=number of items shown on front-end
			//zcs=>-----Image Uploader Settings-----
			'LimitCustomerUploadImageSize', // johnny
			'LimitCustomerUploadImageNum',//zcs=
			'LimitCustomerUploadImagePerNum',//zcs=
			'LimitCustomerUploadImageFileType',//zcs=
			'ImageUploaderSettingsInstructions',//zcs=
			'ImageUploaderSettingsAssignment',//zcs=
			'ImageUploaderSettingsNotifyEmail',//zcs=
			//<=zcs---------------------------------
			'Language',
			'serverStamp',
			'HostingProvider',
			...
			'DefaultProductImage',
			'EnableCompanyGiftCertificates',
			'CompanyGiftCertificateExpiry',
		);
		...
		public function HandleToDo($Do)
		{
			...
			switch (isc_strtolower($Do))
			{
				...
				case "viewredirectsettings":
                {
                    $GLOBALS['BreadcrumEntries'][GetLang('RedirectSettings')] = "index.php?ToDo=viewRedirectSettings";
                    if(!isset($_GET['ajax']))    {
                        $GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintHeader();
                    }
				...
				case "viewcompanygiftcertificatesettings":
				{
					if (!gzte11(ISC_LARGEPRINT)) {
						exit;
					}

					$GLOBALS['BreadcrumEntries'][GetLang('CompanyGiftCertificateSettings')] = "index.php?ToDo=viewCompanyGiftCertificateSettings";
					$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintHeader();
					$this->ManageCompanyGiftCertificateSettings();
					$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintFooter();
					break;
				}
				case "saveupdatedcompanygiftcertificatesettings":
				{
					if (!gzte11(ISC_LARGEPRINT)) {
						exit;
					}

					$this->SaveUpdatedCompanyGiftCertificateSettings();
					break;
				}
		...
		private function SaveUpdatedCompanyGiftCertificateSettings()
		{
			$boolean = 'EnableGiftCertificates';

			if (isset($_POST[$boolean]) && $_POST[$boolean] == 1) {
					$GLOBALS['ISC_NEW_CFG']['EnableCompanyGiftCertificates'] = 1;
				} else {
					$GLOBALS['ISC_NEW_CFG']['EnableCompanyGiftCertificates'] = 0;
				}
			...
			if (isset($_POST['GiftCertificateExpiry']) && isset($_POST['EnableGiftCertificateExpiry'])) {
				...
				$GLOBALS['ISC_NEW_CFG']['CompanyGiftCertificateExpiry'] = $_POST['GiftCertificateExpiry'] * 86400;
			}
			else {
				$GLOBALS['ISC_NEW_CFG']['CompanyGiftCertificateExpiry'] = 0;
			}

			if ($this->CommitSettings($messages)) {
				// Log this action
				$GLOBALS['ISC_CLASS_LOG']->LogAdminAction();
				FlashMessage(GetLang('CompanyGiftCertificateSettingsSavedSuccessfully'), MSG_SUCCESS, 'index.php?ToDo=viewCompanyGiftCertificateSettings');
			} else {
				FlashMessage(sprintf(GetLang('CompanyGiftCertificateSettingsNotSaved'), $messages), MSG_ERROR, 'index.php?ToDo=viewCompanyGiftCertificateSettings');
			}
		}
		...
		private function ManageCompanyGiftCertificateSettings($messages=array())
		{
			$GLOBALS['Message'] = GetFlashMessageBoxes();

			// Are gift certificates enabled?
			if (GetConfig('EnableCompanyGiftCertificates') == 1) {
				$GLOBALS['IsEnableGiftCertificates'] = "checked=\"checked\"";
			}

			// Are gift certificates set to expire after a certain time period?
			if (GetConfig('CompanyGiftCertificateExpiry') > 0) {
				$GLOBALS['IsGiftCertificateExpiry'] = "checked=\"checked\"";
				if (GetConfig('CompanyGiftCertificateExpiry')) {
					$days = GetConfig('CompanyGiftCertificateExpiry')/86400;
					if (($days % 365) == 0) {
						$GLOBALS['ExpiresAfter'] = $days/365;
						$GLOBALS['RangeYearsSelected'] = "selected=\"selected\"";
					}
			...
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("settings.companygiftcertificates.manage");
			$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
		}
		...
		private function ManageSettings($messages=array())
		{
			...
			//zcs=>limit customer's fault login times
			($GLOBALS['LimitCustomerLoginTimes'] = intval(GetConfig('LimitCustomerLoginTimes'))) < 0 && $GLOBALS['LimitCustomerLoginTimes'] = 0;
			//<=zcs
			//zcs=>
			$GLOBALS['UserQA'] = GetConfig('UserQA');
			//<=zcs
			...
			if(GetConfig('DisplayPriceRange')) {
                $GLOBALS['IsDisplayPriceRange'] = 'CHECKED';
            }
        ...
        */
		...
		// blessen
		 private function Redirectsettings() {
           ...
           // $GLOBALS['Message'] = GetFlashMessageBoxes(); 
            $GLOBALS['urlgrid'] = '';
			$query4 = "SELECT *  FROM [|PREFIX|]redirect_pages ";
			...
			while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result2))
			{
				...
				$id = $row['id'];
				$RecordLocatorNumber = $row['RecordLocatorNumber'];

				$GLOBALS['urlgrid'].= '<tr>   <td>   <input type="text" name="source_url[]" id="source_url[]" value="'.$source_url.'" class="Field400"> </td>  <td>   <input type="text" name="destination_url[]" id="destination_url[]" value="'.$destination_url.'" class="Field400">  </td><td>   <input type="text" name="RecordLocatorNumber_url[]" id="RecordLocatorNumber_url[]" value="'.$RecordLocatorNumber.'" class="Field50" > </td> <td>   <INPUT TYPE="hidden" name = "urlid[]" value = "'.$id.'"> </td></tr> ';
			}

			$GLOBALS['urlgrid'].= '<INPUT TYPE="hidden" name = "totalrows" value = "'.$i.'">';
                   
            $GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("301.manage");
            $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
		...
		public function SaveUpdated301()
        {
			$GLOBALS['Message'] = '';
			$tempmsg = '';
			$totalrows = $_POST['totalrows']; 
			$newsource_url = $_POST['newsource_url'];
			$newdestination_url = $_POST['newdestination_url'];
			$RecordLocatorNumber = $_POST['RecordLocatorNumber'];
			if ($newsource_url != "" and $newdestination_url != "")
			{

						$pos = strpos($newsource_url, "http://");
						if($pos===false) $pos = strpos($newsource_url, "https://");
						if ($pos === false) {
							 $newsource_url = "http://".$newsource_url;
						} 

						$pos = strpos($newdestination_url, "http://");
						if($pos===false) $pos = strpos($newdestination_url, "https://");
						if ($pos === false) {
							 $newdestination_url = "http://".$newdestination_url;
						} 




			$query = "SELECT * FROM [|PREFIX|]redirect_pages where  source_url = '".$newsource_url."' ";
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
			$NumResults = $GLOBALS['ISC_CLASS_DB']->CountResult($result);
			if ($NumResults > 0)
				{
				$tempmsg = "The newly added Source URL is already exist,Please add another one.\n";
				FlashMessage($tempmsg, MSG_ERROR, '');
				}
			else 
				{
				$sql = "INSERT INTO [|PREFIX|]redirect_pages (`source_url`, `destination_url`,`RecordLocatorNumber`) VALUES ('".$newsource_url."', '".$newdestination_url."', '".$RecordLocatorNumber."')";
				$result2 = $GLOBALS['ISC_CLASS_DB']->Query($sql);
					...
					$source_url = $_POST['source_url'][$i];
					$destination_url = $_POST['destination_url'][$i];
					$RecordLocatorNumber = $_POST['RecordLocatorNumber_url'][$i];
					$urlid = $_POST['urlid'][$i];

					if ($source_url == "" || $destination_url == "")
					{
						$sql = "delete from  [|PREFIX|]redirect_pages  where id = ".$urlid."  ";
						$result2 = $GLOBALS['ISC_CLASS_DB']->Query($sql);
					}
					else
					{
					 
						$pos = strpos($source_url, "http://");
						if($pos===false) $pos = strpos($source_url, "https://");
						if ($pos === false) {
							$source_url = "http://".$source_url;
						} 

						$pos = strpos($destination_url, "http://");
						if($pos===false) $pos = strpos($destination_url, "https://");
						if ($pos === false) {
							$destination_url = "http://".$destination_url;
						} 

			$query = "SELECT * FROM [|PREFIX|]redirect_pages where  source_url = '".$source_url."' ";
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
			$NumResults = $GLOBALS['ISC_CLASS_DB']->CountResult($result);
			if ($NumResults > 1)
						{
			
				$tempmsg = "The Source URL ".$source_url." is already exist, Please add another one.\n";
				FlashMessage($tempmsg, MSG_ERROR, '');
						}
			else
						{	
				
						$sql = "update  [|PREFIX|]redirect_pages set `source_url` = '".$source_url."' , `destination_url` = '".$destination_url."'  , `RecordLocatorNumber` = '".$RecordLocatorNumber."' where id = ".$urlid."  ";
						$result2 = $GLOBALS['ISC_CLASS_DB']->Query($sql);
						}	
					}
				}
