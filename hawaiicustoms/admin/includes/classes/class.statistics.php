...
class ISC_ADMIN_STATISTICS
{
	...
	public function HandleToDo($Do)
	{
		...
		switch (isc_strtolower($Do)) {
            ...
            case "overviewstatstop20cats": {
                if(!$GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Statistics_Overview)) {
                    exit;
                }
                $this->SendGraphDataHeaders();
                $statsClass = GetClass('ISC_ADMIN_STATISTICS_OVERVIEW');
                $statsClass->GetOverviewStatsTop20CategoriesData();
                break;
            }   
            case "overviewstatstop20subcats": {
                if(!$GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Statistics_Overview)) {
                    exit;
                }
                $this->SendGraphDataHeaders();
                $statsClass = GetClass('ISC_ADMIN_STATISTICS_OVERVIEW');
                $statsClass->GetOverviewStatsTop20SubcategoriesData();
                break;
            }
            case "overviewstatstop20brds": {
                if(!$GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Statistics_Overview)) {
                    exit;
                }
                $this->SendGraphDataHeaders();
                $statsClass = GetClass('ISC_ADMIN_STATISTICS_OVERVIEW');
                $statsClass->GetOverviewStatsTop20BrandsData();
                break;
            }
            case "overviewstatstop20sers": {
                if(!$GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Statistics_Overview)) {
                    exit;
                }
                $this->SendGraphDataHeaders();
                $statsClass = GetClass('ISC_ADMIN_STATISTICS_OVERVIEW');
                $statsClass->GetOverviewStatsTop20SeriesData();
                break;
            }
            ...
            case "viewsalesstats": {
                ...
                $GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintHeader();
                $statsClass = GetClass('ISC_ADMIN_SALES');
                $statsClass->SalesStats();
                $GLOBALS['ISC_CLASS_ADMIN_ENGINE']->PrintFooter();
                break;
            }
            case "overviewynumgridstats": {
                if(!$GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Statistics_Products)) {
                    exit;
                }
                $statsClass = GetClass('ISC_ADMIN_SALES');
                $statsClass->OverviewGrid();
                break;
            }
            case "salesstatsviewsgridstats": {
                if(!$GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Statistics_Products)) {
                    exit;
                }
                $statsClass = GetClass('ISC_ADMIN_SALES');
                $statsClass->SalesStatsByNumViewsGrid();
                break;
            }
            /* Baskaran */
			default:
	...
	protected function CalculateCalendarRestrictions($calendarinfo=array())
	{
		...
		if(isset($calendarinfo['DateType'])) {
			...
			//$startdate = isc_gmmktime(0, 0, 0, isc_date("m"), isc_date("d")-30, isc_date("Y"));
			//$enddate = $today;
			//Default to Last 24 Hours -- set due to client request -- Baskaran Aug03 10
			$startdate = $rightnow - 86400;
			$enddate = $rightnow;
		}
