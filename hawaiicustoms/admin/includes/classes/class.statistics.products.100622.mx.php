	...
	*/
	public function ProductStatsByNumViewsGrid()
	{
		...
		if ($total_products > 0) {
			...
			$addSql = '';
			if($vendorSql !='')
				$addSql  = $vendorSql ."AND enable = '1'";
			else
				$addSql  = "WHERE enable = '1'";
			$query = "
				SELECT
					productid,
					prodcode,
					...
					IF(prodnumviews > 0, prodnumsold / prodnumviews, 0) AS unitssoldpercent
				FROM
					[|PREFIX|]products_statistics
				" . $addSql . "
				ORDER BY
					" . $sortField . " " . $sortOrder;
			//$file = fopen(ISC_CACHE_DIRECTORY."/text.txt", "w");
				...
				while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
					...
					$GLOBALS['OrderGrid'] .= sprintf("
						<tr class=\"GridRow\" onmouseover=\"this.className='GridRowOver';\" onmouseout=\"this.className='GridRow';\"> 
							...
							<td nowrap class=\"".$GLOBALS['SortedFieldUnitsSoldPercentClass']."\">
							%s
							</td>
