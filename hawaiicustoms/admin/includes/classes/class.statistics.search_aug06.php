...
class ISC_ADMIN_STATISTICS_SEARCH extends ISC_ADMIN_STATISTICS
{
	...
	*/
	public function _GetSearchStatsForPeriod($FromStamp, $ToStamp)
	{
		...
		$query = sprintf("select count(0) as num from [|PREFIX|]searches_extended where searchdate >= '%s' and searchdate <= '%s'", $FromStamp, $ToStamp);
		...
		$query = sprintf("select searchtext, count(0) as num from [|PREFIX|]searches_extended where numresults > 0 and searchdate >= '%s' and searchdate <= '%s' group by searchtext order by num desc limit 1", $FromStamp, $ToStamp);
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		$row = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
		...
		$query = sprintf("select searchtext, count(0) as num from [|PREFIX|]searches_extended where numresults = 0 and searchdate >= '%s' and searchdate <= '%s' group by searchtext order by num asc limit 1", $FromStamp, $ToStamp);
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		$row = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
		...
		$query = sprintf("select count(0) as numsearches from [|PREFIX|]searches_extended where searchdate >= '%s' and searchdate <= '%s' group by date(from_unixtime(searchdate)) order by numsearches desc limit 1", $FromStamp, $ToStamp);
		$vals['max_searches_day'] = $GLOBALS['ISC_CLASS_DB']->FetchOne($query);

		// Average number of searches a day
		$query = sprintf("select count(0) / count(DISTINCT(date(from_unixtime(searchdate)))) from [|PREFIX|]searches_extended where searchdate >= '%s' and searchdate <= '%s'", $FromStamp, $ToStamp);
		$result= $GLOBALS['ISC_CLASS_DB']->Query($query);
		...
		if(isset($_GET['from']) && is_numeric($_GET['from']) && isset($_GET['to']) && is_numeric($_GET['to'])) {
			...
			if($GLOBALS['ISC_CLASS_DB']->CountResult($result) > 0) {
				while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
					$ymm = ( isset($row['year']) && $row['year'] != "" ? " ".$row['year'] : '').( isset($row['maker']) && $row['maker'] != "" ? " ".$row['maker'] : '').( isset($row['model'] ) && $row['model'] != "" ? " ".$row['model'] : '');
					if( isset($ymm) && $ymm != '' )
					{
						$ymm = "&nbsp;($ymm)";
					}
					...
					$output .= sprintf("\t<slice title=\"%s\" pull_out=\"false\">%s</slice>\n", isc_html_escape(isc_convert_charset(GetConfig('CharacterSet'), 'UTF-8', 
					$row['searchtext'].$ymm)), 
					$row['numsearches']);
	...
	public function SearchStatsWithResultsGrid()
	{
		...
		if(isset($_GET['From']) && isset($_GET['To'])) {
			...
			if($GLOBALS['ISC_CLASS_DB']->CountResult($result) > 0) {
				while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
					...
					", isc_html_escape($row['searchtext']), 
						((isset($row['searchurl']) && $row['searchurl'] !== '') ? $row['searchurl'] : "/search/".MakeURLSafe($row['searchtext']).( isset($row['year']) && $row['year'] != "" ? "/year/".$row['year'] : '').( isset($row['maker']) && $row['maker'] != "" ? "/make/".MakeURLSafe($row['maker']) : '').( isset($row['model'] ) && $row['model'] != "" ? "/model/".MakeURLSafe($row['model']) : '')),
						((isset($row['searchurl']) && $row['searchurl'] !== '') ? $GLOBALS["ShopPath"].$row['searchurl'] : $GLOBALS["ShopPath"]."/search/".MakeURLSafe($row['searchtext']).( isset($row['year']) && $row['year'] != "" ? "/year/".$row['year'] : '').( isset($row['maker']) && $row['maker'] != "" ? "/make/".MakeURLSafe($row['maker']) : '').( isset($row['model'] ) && $row['model'] != "" ? "/model/".MakeURLSafe($row['model']) : '')),
			...
			$GLOBALS['ResultsGrid'] .= '<br /><input type="button" value="Export to Microsoft Excel (CSV)" onclick=" window.open( \' '.$GLOBALS["ShopPath"].'/admin/index.php?ToDo=exsearchstats&FromDate='.$from_stamp.'&ToDate='.$to_stamp.'&Search_Type=KeywordWithResults \'); " />';
	...
	public function SearchStatsWithoutResultsGrid()
	{
		...
		if(isset($_GET['From']) && isset($_GET['To'])) {
			...
			// NI cloud 2010-06-08 export search statistic to csv file
			$GLOBALS['ResultsGrid'] .= '<br /><input type="button" value="Export to Microsoft Excel (CSV)" onclick=" window.open( \' '.$GLOBALS["ShopPath"].'/admin/index.php?ToDo=exsearchstats&FromDate='.$from_stamp.'&ToDate='.$to_stamp.'&Search_Type=KeywordWithoutResults \'); " />';
