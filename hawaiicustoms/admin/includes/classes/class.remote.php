	...
	class ISC_ADMIN_REMOTE extends ISC_XML_PARSER
	{
		...
		public function HandleToDo()
		{
			...
			switch  ($what) {
				...
				//zcs=>
				case "updateaddonproductorders": {
					if ($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Manage_AddonProducts)) {
						$this->UpdateAddonProductOrders();
					}
					break;
				}
				//<=zcs
				...
				case "updateexchangerate":
					...
					break;
				//NI Cloud 2010-06-24
				// add logic to process cgc quickview and status update
				case "companygiftcertificatequickview": {
					if ($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Manage_Company_GiftCertificates)) {
						$this->CompanyGiftCertificateQuickView();
					}
					break;
				}
				case "updatecompanygiftcertificatestatus": {
					if ($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Manage_Company_GiftCertificates)) {
						$this->UpdateCompanyGiftCertificateStatus();
					}
					break;
				}
				...
				case "getstates":
					$this->GetStateList();
					break;
				case "offerquickview": {  //blessen ,for make a offer
					if ($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Manage_Orders)) {
						$this->GetOfferQuickView();
					}
					}
					break;
				case "updateofferstatus": { //blessen for make a offer
				...
				case "getcustomernameandemail":
					if ($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Manage_Company_GiftCertificates)) {
						$this->GetCustomerNameandEmail();
					}
					break;
				//wirror_20100817: update the custom content dragable list
				case "updatecustomorders": {
					if ($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Manage_Categories)) {
						$this->UpdateCustomContentOrders();
					}
					break;
				}
				//ronnie_20101109: Update Abtesting the custom content dragable list
				case "updateabtestingcustomorders": {
					if ($GLOBALS['ISC_CLASS_ADMIN_AUTH']->HasPermission(AUTH_Manage_Categories)) {
						$this->UpdateAbtestingCustomContentOrders();
					}
					break;
				}
			}
		 ...
		 * wirror_20100817: Update the custom content dragable list
		 */
		private function UpdateCustomContentOrders()
		{
			$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->LoadLangFile('categories');
			$list = $_POST['CustomContentList'];

			foreach($list as $order => $customItem) {
				$updatedItem= array(
					"displayorder" => $order+1
				);
				$GLOBALS['ISC_CLASS_DB']->UpdateQuery("custom_products", $updatedItem, "itemid='".$GLOBALS['ISC_CLASS_DB']->Quote((int)$customItem['id'])."'");
			}

			$tags[] = $this->MakeXMLTag('status', 1);
			$tags[] = $this->MakeXMLTag('message', GetLang('CustomContentOrdersUpdated'), true);
			$this->SendXMLHeader();
		 ...
		 * ronnie_20101109: Update Abtesting the custom content dragable list
		 */
		private function UpdateAbtestingCustomContentOrders()
		{
			$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->LoadLangFile('categories');
			$list = $_POST['CustomContentList'];
			...
			foreach($list as $order => $customItem) {
				...
				$GLOBALS['ISC_CLASS_DB']->UpdateQuery("abtesting_custom_products", $updatedItem, "itemid='".$GLOBALS['ISC_CLASS_DB']->Quote((int)$customItem['id'])."'");
			}
		...
		//NI CLOUD 2010-06-24
		/**
		 * Show the quick view for company gift certificates
		 *
		 * @return void
		 **/
		private function CompanyGiftCertificateQuickView()
		{
			$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->LoadLangFile('company.giftcertificates');
			if(!isset($_REQUEST['cgcId'])) {
				exit;
			}
	
			$query = sprintf("SELECT * FROM [|PREFIX|]company_gift_certificates WHERE cgcid='%d'", $GLOBALS['ISC_CLASS_DB']->Quote($_REQUEST['cgcId']));
			...
			$query = "
					...
					FROM [|PREFIX|]company_gift_certificate_history h
					LEFT JOIN [|PREFIX|]customers c ON (c.customerid=h.histcustomerid)
					WHERE h.histcgcid='" . $certificate['cgcid'] . "'
					ORDER BY historddate ASC";
	
			$GLOBALS['Message'] = isc_html_escape($certificate['cgcmessage']);
			
			$narray = explode('$',$certificate['cgcto']);
			$earray = explode('$',$certificate['cgctoemail']);
			for($i=0; $i<count($narray);$i++)
			{
				if( $i > 0 )
					$GLOBALS['ToName'] .= "<br />";				
				$GLOBALS['ToName'] .= "<a href=mailto:".isc_html_escape($earray[$i]).">".isc_html_escape($narray[$i])."</a>";
			...
			while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
				...
				$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("company.giftcertificate.quickview.item");
				$GLOBALS['GiftCertificateHistory'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate(true);
			}
	
			if($GLOBALS['GiftCertificateHistory'] == '') {
				$GLOBALS['GiftCertificateHistory'] = sprintf('<div style="padding-left: 10px;">%s</div>', GetLang('CompanyGiftCertificateNotUsed'));
			}
	
			$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("company.giftcertificate.quickview");
			$GLOBALS['ISC_CLASS_TEMPLATE']->ParseTemplate();
		}
		
		//NI CLOUD 2010-06-24
		/**
		* Update the status of a company gift certificate from the manage page
		*
		* @return void
		*/
		private function UpdateCompanyGiftCertificateStatus()
		{
			if(!isset($_REQUEST['cgcId'])) {
				exit;
			}
			$query = sprintf("SELECT * FROM [|PREFIX|]company_gift_certificates WHERE cgcid='%d'", $_REQUEST['cgcId']);
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
			$certificate = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
			if(!$certificate['cgcid']) {
				exit;
			}
	
			$updatedStatus = array(
				"cgcstatus" => (int)$_REQUEST['status']
			);
        if ($_REQUEST['status'] == 4) {
          $updatedStatus['cgcexpirydate'] = time() - 3600 * 24;
      } elseif($_REQUEST['status'] == 2) {
				// Are gift certificates set to expire?
        $expiry = 0;
        if (isset($_REQUEST['expiry'])) {
            $expiry = ($_REQUEST['expiry'] == 0 ? 0 : strtotime($_REQUEST['expiry']));
        }
				$updatedStatus['cgcexpirydate'] = $expiry;
			}
			$GLOBALS['ISC_CLASS_DB']->UpdateQuery("company_gift_certificates", $updatedStatus, "cgcid='".$GLOBALS['ISC_CLASS_DB']->Quote($certificate['cgcid'])."'");
			echo 1;
			exit;
		}
      ...
      if ($_REQUEST['status'] == 4) {
          $updatedStatus['giftcertexpirydate'] = time() - 3600 * 24;
      } elseif ($_REQUEST['status'] == 2) {
				// Are gift certificates set to expire?
          $expiry = 0;
        if (isset($_REQUEST['expiry'])) {
		...
		/**
		*get customer name and email for creating cgc
		**/
		function GetCustomerNameandEmail()
		{
			$str = '';
			$query = "select custconfirstname, custconlastname, custconemail from [|PREFIX|]customers 
					where custconfirstname like '".$_REQUEST['name']."%' or custconlastname like '".$_REQUEST['name']."%' LIMIT 0 , 10 ";
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
			while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
				$str .= "<li onclick=' CustomNameClick(\"".$_REQUEST['index']."\", \"".$row["custconlastname"].' '.$row["custconfirstname"]."\", \"".$row["custconemail"]."\"); ' onMouseOver=' this.style.background = \"gray\";' onMouseOut=' this.style.background = \"white\";' >
							Name:".$row["custconlastname"].' '.$row["custconfirstname"]."&nbsp;&nbsp;&nbsp;&nbsp;Email:".$row["custconemail"]."</li>";
			}
			echo $str;
		}
...
$products = $resultsortquery;
		...
		//zcs=>----------------Add-on Product Orders------------------
		/**
		 * Update the sort order of the add-on product when they are reordered
		 *
		 * @return void
		 **/
		private function updateAddonProductOrders()
		{
			$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->LoadLangFile('addon_products');

			$this->_BuildAddonProductOrders($_POST['SortList']);

			$tags[] = $this->MakeXMLTag('status', 1);
			$tags[] = $this->MakeXMLTag('message', GetLang('AddonProductOrdersUpdated'), true);
			$this->SendXMLHeader();
			$this->SendXMLResponse($tags);
			die();
		}
		 ...
		 * @return void
		 **/
		private function _BuildAddonProductOrders($list)
		{
			if(!is_array($list)) {
				return;
			}
			...
			foreach($list as $order => $obj) {
				$updated = array(
					"sequence" => $total-$order,
				);
				$GLOBALS['ISC_CLASS_DB']->UpdateQuery("addon_products", $updated, "id='".$GLOBALS['ISC_CLASS_DB']->Quote((int)$obj['id'])."'");
			}
		}
		//<=zcs------------------------------------------------------------
		...
		**/
		...
		public function GetOrderQuickView()
		{
			...
			if ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
				...
				if($row['orderowner']=='0') {
					$GLOBALS['AddedBy']='System';
				}
				else {
					$loggeduser = $GLOBALS['ISC_CLASS_ADMIN_AUTH']->GetUserById($row['orderowner']);
						$GLOBALS['AddedBy']=$loggeduser['username'];
				}
				...
				if($row['orderpaymentmethod'] != "storecredit" && $row['orderpaymentmethod'] != "giftcertificate"  && $row['orderpaymentmethod'] != "companygiftcertificate") {
                ...
                if($row['ordcompanygiftcertificateamount'] > 0 && gzte11(ISC_LARGEPRINT)) {
					$GLOBALS['PaymentMethod'][] = sprintf(GetLang('PaymentCompanyGiftCertificates'), $row['orderid']) . " (".FormatPriceInCurrency($row['ordcompanygiftcertificateamount'], $row['orddefaultcurrencyid']) . ")";
				}
				...
				$GLOBALS['CouponsUsed'] = $GLOBALS['GiftCertificatesUsed'] = $GLOBALS['CompanyGiftCertificatesUsed'] = $GLOBALS['GiftCertificatesUsed'] = '';
				$GLOBALS['HideCouponsUsed'] = $GLOBALS['HideGiftCertificatesUsed'] = $GLOBALS['HideCompanyGiftCertificatesUsed'] = 'display: none';
                ...
                if($row['ordgiftcertificateamount'] > 0 && gzte11(ISC_LARGEPRINT)) {
                    $giftcertificatesUsed = $orderClass->GetOrderGiftCertificatesUsed($orderData);
                    foreach ($giftcertificatesUsed as $item) {
                        $GLOBALS['GiftCertificatesUsed'] .= "Code : {$item['giftcertcode']}<br/>";
                        $GLOBALS['GiftCertificatesUsed'] .= 'Amount : ' . FormatPriceInCurrency($item['giftcertamountused'], $row['orddefaultcurrencyid']);
                    }
					$GLOBALS['HideGiftCertificatesUsed'] = '';
				}
                if($row['ordcompanygiftcertificateamount'] > 0 && gzte11(ISC_LARGEPRINT)) {
                    $companygiftcertificatesUsed = $orderClass->GetOrderCompanyGiftCertificatesUsed($orderData);
                    foreach ($companygiftcertificatesUsed as $item) {
                        $GLOBALS['CompanyGiftCertificatesUsed'] .= "Code : {$item['cgccode']}<br/>";
                        $GLOBALS['CompanyGiftCertificatesUsed'] .= 'Amount : ' . FormatPriceInCurrency($item['amountused'], $row['orddefaultcurrencyid']);
                    }
                    $GLOBALS['HideCompanyGiftCertificatesUsed'] = '';
				}
                ...
                */
				...
				while($pRow = $GLOBALS['ISC_CLASS_DB']->Fetch($pResult)) {
                    ...
                    */
					...
					$origPrice = $pRow['ordprodcost'] * $pRow['ordprodqty']; // initializing to prodcost
                    if(count($allcoupons))    {                       //$pRow['ordoriginalprice'] < $pRow['ordprodcost'] 
                        if($pRow['ordoriginalprice'] > $pRow['ordprodcost'])    { 
                            ...
                            $prodYMMInfo .= '<u>Original Price : '. FormatPriceInCurrency($pRow['ordoriginalprice'], $row['orddefaultcurrencyid'])."<br />";
				...
				$GLOBALS['ProductsTable'] .= sprintf("<tr><td height='18' class='text' align='right'>%s:</td><td class='text' align='right'>%s</td></tr>", GetLang('SubTotal'), FormatPriceInCurrency($originaltotal_temp, $row['orddefaultcurrencyid']));
				...
				// johnny add 
				$orderId = $row['orderid'];
				$sql = "select ordtrackingno, DATE_FORMAT(orddateshipped,'%D %b %Y') AS orddateshipped from [|PREFIX|]order_products where orderorderid = $orderId";
				$result = $GLOBALS['ISC_CLASS_DB']->Query($sql);
				while($val = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
					
					if(!empty($val['orddateshipped']) && $val['orddateshipped'] != "1st Jan 1900"){
						//alandy_2011-9-13.fetch datatime from db ,don't format.
						//$GLOBALS['ShippingDate'] .= isc_date(GetConfig('DisplayDateFormat'), $val['orddateshipped']+18000);
						$GLOBALS['ShippingDate'] .= $val['orddateshipped'];
						
					}
					if(!empty($val['ordtrackingno'])){
						$GLOBALS['ShippingDate'] .= '/'. $val['ordtrackingno'];
					}
					
					if(!empty($val['orddateshipped']) && $val['orddateshipped'] != "1st Jan 1900"){
					   $GLOBALS['ShippingDate'] .= "<br/>";
					}
					
				}

				/*if ($row['orddateshipped'] > 0) {
					$GLOBALS['ShippingDate'] = isc_date(GetConfig('DisplayDateFormat'), $row['orddateshipped']+18000);
				} else {
					$GLOBALS['ShippingDate'] = GetLang('NA');
				}*/

				if ($row['ordnotes'] != '') { # Added to display Staff notes, while clicking '+' symbol in order page -- Baskaran
                    $GLOBALS['StaffNotes'] = $row['ordnotes'];
                } else {
                    $GLOBALS['StaffNotes'] = GetLang('NA');
                }
                ...
                $GLOBALS['ProductsTable'] .= sprintf("<tr><td height='18' class='QuickTotal text' align='right'>%s:</td><td class='QuickTotal text' align='right'>%s</td></tr>", GetLang('GatewayTotal'), FormatPriceInCurrency($row['ordgatewayamount'], $row['orddefaultcurrencyid']));
...
//added by blessen lib/order.php
	private function UpdateOfferStatus()
		...
		function PaymentStatus($order_id,$type) { # Baskaran
            $query = $GLOBALS['ISC_CLASS_DB']->Query("SELECT * FROM [|PREFIX|]offers where orderid = '$order_id'");
            $row = $GLOBALS['ISC_CLASS_DB']->Fetch($query);
//                print_r($row);exit;
		// Load the Authorize.net merchant ID
        $query_merchtid = $GLOBALS['ISC_CLASS_DB']->Query("SELECT * FROM [|PREFIX|]module_vars where variablename = '".merchantid."' ");
        $row_merchtid = $GLOBALS['ISC_CLASS_DB']->Fetch($query_merchtid);
        $merchant_id = $row_merchtid['variableval'];

        // Load the tranaction key
        $query_key = $GLOBALS['ISC_CLASS_DB']->Query("SELECT * FROM [|PREFIX|]module_vars where variablename = '".transactionkey."' ");
        ...
        $transaction_key = $row_key['variableval'];
            ...
            $statuscode = $response_array[0];
            $response = $response_array[3];
            return $statuscode;
       }

		//blessen 
		public function GetOfferQuickView()
		{
			$GLOBALS['ISC_CLASS_ADMIN_ENGINE']->LoadLangFile('orders');
