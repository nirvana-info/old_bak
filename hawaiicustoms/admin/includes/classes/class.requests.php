...
class ISC_ADMIN_REQUESTS
{
	...
	{
		/*$this->paserRequestTemplate((int)$_GET['templateId'], 'email');
		echo "1";
		return ;*/
     ...
     * Baskaran
     */
    public function SendRequestMultyCron()
    {        
        $success = (int)@$_REQUEST['success'];
        $failed = (int)@$_REQUEST['failed'];
        if(!isset($_REQUEST['orderIds'])) {
	...
	public function RecordReviewRequest($orderId, $canResend,$templateId,$addlog=true)
	{
		...
		if($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)){
			...
			if($canResend){
				//2010-11-03 Ronnie Modify Add $curAdmin for update
				//$this->UpdateReviewRequestStatus($orderId, 1,$templateId);
				$this->UpdateReviewRequestStatus($orderId, 1,$templateId,$curAdmin);
	...
	public function UpdateReviewRequestStatus($orderIds, $status,$templateId,$curAdmin)
	{
	...
	//echo "in update process.2.";
			...
			/* Added to get the email counter of the orderid -- Baskaran */
	        $query = $GLOBALS['ISC_CLASS_DB']->Query("SELECT emailcounter FROM [|PREFIX|]requests WHERE orderid = $orderId ");
            $row = $GLOBALS['ISC_CLASS_DB']->Fetch($query);
            $emailcounter = $row['emailcounter'] + 1;
            /* Ends */
			$updatedStatus = array(
				'requestdate' => date('Y-m-d H:i:s',time()),
			...
			//2010-11-03 Ronnie Modify add requestowner update
	...
	public function GetOrderDetail($orderId,$type="review")
	{
		...
		//2010-12-6 Ronnie modify,order encode
		//$GLOBALS['OrderReviewLink'] = GetConfig('ShopPath')."/order/review/".$orderId;
		$GLOBALS['OrderReviewLink'] = GetConfig('ShopPath')."/order/review/".base64_encode($orderId);
		$query = "
		...
		if ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
			...
			if(!isset($GLOBALS['contactEmail']) || empty($GLOBALS['contactEmail']))
			{
				$GLOBALS['contactEmail']=$row['ordbillemail'];
			}
			if(!isset($GLOBALS['contactEmail']) || empty($GLOBALS['contactEmail']))
			{
				$GLOBALS['contactEmail']=$row['ordshipemail'];
			}
			...
			//2010-11-25 Ronnie add
			if($GLOBALS['CouponsUsedDetails']==''){
				$GLOBALS['CouponsUsedDetails']="None";
			}

			$prodFieldsArray=$GLOBALS['ISC_CLASS_ADMIN_ORDERS']->GetOrderProductFieldsData($orderId);

			// Get the products in the order
			//2010-11-12 ronnie ,no produst no request
			/*$query = "
				SELECT o.*, p.prodname
				FROM [|PREFIX|]order_products o
				LEFT JOIN [|PREFIX|]products p ON (p.productid=o.ordprodid)
				WHERE orderorderid='" . $orderId . "' and p.prodvisible=1
			...
			while($pRow = $GLOBALS['ISC_CLASS_DB']->Fetch($pResult)) {
				...
				if($pRow['prodname']) {
					...
					$pRow['ordprodname'] = "<a href='".$GLOBALS['OrderReviewLink']."' target='_blank'>".isc_html_escape($pRow['ordprodname'])."</a>";
				}
			...
			//2010-11-12 Ronnie add, no product no request view
			$query = "
				SELECT o.*, p.prodname
				FROM [|PREFIX|]order_products o
				LEFT JOIN [|PREFIX|]products p ON (p.productid=o.ordprodid)
	...
	public function paserRequestTemplate($templateId, $type='preview'){
		...
		if($type == 'email'){
			$this->GetOrderDetail($_REQUEST['o'],"email");
		}else{
		...
		$showContent = '';
		$requestTemplate = '';
		$query = "select request_script_value,coupon_code,emailsubject from [|PREFIX|]order_review_request where id='".$templateId."'";
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		
		//$GLOBALS['ROW_COUPON_CODE']="";
		...
		if ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
			...
			{
				//Set coupon Code Ronnie
				//$coupon_code_hint = GetLang('CouponCodeHint').$row['coupon_code'];
				$coupon_code_hint="";//GetLang('CouponCodeHint');//.$row['coupon_code'];
		    	$Seqno=$this->getSeqNoByOrderID($orderId,$row['coupon_code']);
		    	$coupon_code_hint.=$Seqno;//.$GLOBALS['contactEmail'];
		    	//$GLOBALS['ROW_COUPON_CODE']=$row['coupon_code'];
		    	//Create New Coupon Ronnie
				//$this->CreateNewCoupon($_REQUEST['orderId'],$GLOBALS['ROW_COUPON_CODE']);
				if($type == "email"){
					//Create New Coupon Ronnie
					$this->CreateNewCoupon($orderId,$row['coupon_code']);	
				}	
			}else{
				//No coupon No new coupon
				if($type == "email"){
					$this->CancelNewCoupon($this->getSeqNoByOrderID($orderId,''), $orderId);
				}
			}
			
			//2010-12-06 Ronnie add,direct url
			$row['request_script_value']=str_ireplace("src=\"","src=\"".$GLOBALS['ShopPath'],$row['request_script_value']);
			
			$GLOBALS['COUPON_EMAIL_SUBJECT']=$row['emailsubject'];
			...
			//replace Seqno Ronnie
		    
		    //$coupon_code_hint.="-".$Seqno;
		    //$query = "select * from [|PREFIX|]coupons where couponcode='".$row['coupon_code']."'";
			//$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
			//$row1 = $GLOBALS['ISC_CLASS_DB']->fetch($result);
		    //$coupon_code_hint.="--".$query."--".count($result)."--".$row1['couponcode'];
		    
		    //$this->DeleteNewCoupon($Seqno,$_REQUEST['orderId'],$row['coupon_code']);
		    $query2 = "select concat(a.ordbillfirstname,' ',a.ordbilllastname)custname from [|PREFIX|]orders a ";
			$query2 .="where a.orderid='".$orderId."'";
			$result2 = $GLOBALS['ISC_CLASS_DB']->Query($query2);
			if($row2 = $GLOBALS['ISC_CLASS_DB']->fetch($result2)){
			...
			'<!--ORDER_COUPON_CODE-->' => $coupon_code_hint==""?"None":$coupon_code_hint
			);
			
			$requestTemplate = $row['request_script_value'];
			
			///return str_replace("ORDER_DATE", "abc", htmlspecialchars_decode($requestTemplate));
	...
	//Cancel new coupon base on old coupon Ronnie
	public function CancelNewCoupon($seqno,$OrderID){
		
		//set unused
		$Data = array(
				'used' => 0,
			);
		$GLOBALS['ISC_CLASS_DB']->UpdateQuery("seqno",$Data,"seqno='".$this->getRealseqno($seqno)."'");
			
		//Delete new coupon
		$query = "select * from [|PREFIX|]coupons where couponenabled=1 and orderid='".$OrderID."'";
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		...
		//delete coupon for use Order
		if($row = $GLOBALS['ISC_CLASS_DB']->fetch($result)){
			$GLOBALS['ISC_CLASS_DB']->UpdateQuery('coupons', $Data ," couponcode = '".$row['couponcode']."'");	
		}
	}
	
	//Delete new coupon base on old coupon Ronnie
	public function DeleteNewCoupon($seqno,$OrderID){
		...
		//Delete new coupon
		$query = "select * from [|PREFIX|]coupons where orderid='".$OrderID."'";
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		//delete coupon for use Order
		if($row = $GLOBALS['ISC_CLASS_DB']->fetch($result)){
			$GLOBALS['ISC_CLASS_DB']->DeleteQuery('coupons', "WHERE couponenabled=1 and couponcode = '".$row['couponcode']."'");	
		}
	}
	
	public function getRealseqno($seqno){
		list($code, $sq)=split('-', $seqno);
		return $sq;
	}
	
	//create new coupon base on old coupon Ronnie
	public function CreateNewCoupon($OrderID,$couponcode){
		
		$seqno=$this->getSeqNoByOrderID($OrderID,$couponcode);
		
		//delete exist New Coupon
		$this->CancelNewCoupon($seqno,$OrderID);
		
		//if CouponCode is Empty ,do not have to create new coupon
		if(empty($couponcode)){
			return ;
		}
		...
		//create new coupon base on old coupon
		$query = "select * from [|PREFIX|]coupons where couponcode='".$couponcode."'";
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		if($row = $GLOBALS['ISC_CLASS_DB']->fetch($result)){
			$Data = array (
		    	'couponid'=>0,
				...
				'couponminpurchase'=>$row['couponminpurchase'],
				'couponexpires'=>$row['couponexpires'],
				'couponenabled'=>1,
				'couponcode'=>$seqno,
				'couponappliesto'=>$row['couponappliesto'],
				'couponappliestovalues'=>$row['couponappliestovalues'],
		    	'couponnumuses'=>0,
				'couponmaxuses'=>1,
				'coupongeneratetype'=>1,
				'customerid'=>1,
				'orderid'=>$OrderID,
				'coupongeneratedate'=>date('Y-m-d H:i:s',time())
			);	
			$GLOBALS['ISC_CLASS_DB']->InsertQuery("coupons", $Data);
		}
	}
	
	//Use SeqNo and this Seqno can't for another Ronnie
	public function UseSeqNo($seqno){
	...
	//get SeqNo by OrderID Ronnie
	public function getSeqNoByOrderID($OrderID,$couponcd)
	{
		//first from coupons get it
		/*$query = "select * from [|PREFIX|]coupons where couponcode!='' and couponenabled=1 and orderid='".$OrderID."'";
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		$seqno="";
		//exist return couponcode ,other return create new number
		if($row = $GLOBALS['ISC_CLASS_DB']->fetch($result)){
			//when delete,direct return couponcode
			if($couponcd=='') return $row['couponcode'];
			list($code, $sq)=split('-', $row['couponcode']);
			$seqno =$couponcd."-".$sq;
		}else{
			$seqno =$couponcd."-".$this->getSeqNo($OrderID);
		}*/
		$seqno =$couponcd."-".$this->getSeqNo($OrderID);
		return $seqno;
	}

	//get not to use of number Ronnie
	public function getSeqNo($OrderID)
	{
		$seqno="seqno";
		$query = "select * from [|PREFIX|]seqno where used=0 and orderid=".$OrderID;;
		//return $query;
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		//return count($result);
		...
		if($row = $GLOBALS['ISC_CLASS_DB']->fetch($result)){
			...
			//create new number
			$a=1;
			//$seqno=GenerateCouponCode().GenerateCouponCode();
			//$seqno=$this->CreateMaxNumber();
			$seqno=$this->CreateUniqueCode($this->GetRandomNumber(),$a);
			$Data = array(
				'seqno' => $seqno,
				'used' => 0,
				'orderid' => $OrderID,
			);
			$GLOBALS['ISC_CLASS_DB']->InsertQuery("seqno", $Data);
		}
		return $seqno;
	}
	
	public function CreateUniqueCode($seqno,&$Count){
		$query = "select * from [|PREFIX|]seqno where seqno='".$seqno."'";
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		if($row = $GLOBALS['ISC_CLASS_DB']->fetch($result)){
			if($Count>3){
				return $this->CreateMaxNumber();
			}
			$Count++;
			return $this->Createuniquecode($this->GetRandomNumber(),$Count);
		}else{
			return $seqno;
		}
	}
	
	//Create Max Number base on id
	public function CreateMaxNumber(){
		$query = "select * from [|PREFIX|]seqno order by id desc ";
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		$No="1";
		if($row = $GLOBALS['ISC_CLASS_DB']->fetch($result)){
			return $this->FormatNumber("".$row['id']+1);
		}
		return $this->FormatNumber($No);
	}
	
	//Format Number ,max length is 5
	public function FormatNumber($No){
		$o="00000";
		echo substr($o,0,count($o)-count($No));
		return substr($o,0,count($o)-count($No)).$No;
	}
	...
	public function PreviewRequest()
	{
		...
		*/
		...
		//$this->GetOrderDetail($GLOBALS['OrderId']);
		//$GLOBALS['RemindMessage'].= $GLOBALS['contactEmail'];
		//$GLOBALS['bc258']="1234567890";
		//$GLOBALS['bc258'].=$this->paserRequestTemplate($templateId, 'email');
		//2010-11-22 Ronnie add ,no product don't send mail
		if($GLOBALS['OrderDetails']==''){
			$GLOBALS['ShowSendBtn'] = 'none';
			$GLOBALS['RemindMessage'] = GetLang('NoProductResendWarning');
		}
