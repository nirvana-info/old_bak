	...
	class ISC_ADMIN_REMOTE_ORDERS extends ISC_XML_PARSER
	{
		...
		**/
		private function OrderCouponList()
		{
			...
			{
				$prod_catg_arr = "";
				$prod_catg_qry = "SELECT group_concat( DISTINCT categoryid) as prodcatids from [|PREFIX|]categoryassociations where productid in (".implode(",",$productids).")";
				$prod_catg_res = $GLOBALS['ISC_CLASS_DB']->Query($prod_catg_qry);
				$prod_catg_val = $GLOBALS['ISC_CLASS_DB']->FetchOne($prod_catg_res);
				$prod_catg_arr = explode(",",$prod_catg_val);
			}
			...
			while ($arr = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) 
			{
				if( $arr['couponappliesto'] == 'categories')
				{
					...
					else
					{
						...
						{
							$catids = explode(",",$arr['couponappliestovalues']);
							
							for($j=0;$j<count($catids);$j++)
							{
								if( in_array($catids[$j],$prod_catg_arr) )
								{
									$valid_couponcodes[] = $arr['couponcode'];
									break;
								}
		...
		private function orderLoadCustomerYMM()
		{
			...
			echo $orderClass->fnLoadCustomerYMM($_REQUEST['customerId']);
			exit;
		}
