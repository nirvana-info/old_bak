...
class ISC_ADMIN_SETTINGS_SHIPPING
{
	...
	private function AddShippingZoneMethod()
	{
        ...
        $GLOBALS['WYSIWYG1'] = GetClass('ISC_ADMIN_EDITOR')->GetWysiwygEditor(array(
					'id'		=> 'display_message',
					'width'		=> '60%',
					'height'	=> '20px',
					'value'		=> $method['display_message']
				));
	...
	public function CommitShippingZoneMethod($data, $methodId = 0)
	{
		...
		if(!GetConfig('ShippingConfigured')) {
			$GLOBALS['ISC_NEW_CFG']['ShippingConfigured'] = true;
			$settings = GetClass('ISC_ADMIN_SETTINGS');
			$messages = array();
			$settings->CommitSettings($messages);
		}
	...
	private function SaveUpdatedShippingSettings()
	{
		...
		if ($settings->CommitSettings($messages)) {
			...
			FlashMessage(GetLang('ShippingSettingsSavedSuccessfully'), MSG_SUCCESS, 'index.php?ToDo=viewShippingSettings&currentTab='.((int) $_POST['currentTab']));
		}
		else {
