...
class ISC_ADMIN_VENDORS
{
	...
	private function CommitVendor($data, $vendorId=0)
	{
		...
		$vendorData = array(
            ...
            'vendorprefix' => $data['vendorprefix'],
		);
		...
		if($vendorId == 0) {
            ...
            # To insert record in the brands table -- Baskaran
            $vid = $GLOBALS['ISC_CLASS_DB']->LastId();
            $BrandData = array(
            'brandname'=>$data['vendorname'],
            'vendorid' => $vid
            );
            $GLOBALS['ISC_CLASS_DB']->InsertQuery('brands', $BrandData);
			...
			foreach(array(self::VENDOR_LOGO, self::VENDOR_PHOTO) as $image) {
				...
				if(isset($vendorData['deletevendor'.$image])) {
            ...
            # To update the Brands -- Baskaran
            $UpdateBrand = array(
                'brandname' => $data['vendorname']
            );
            $GLOBALS['ISC_CLASS_DB']->UpdateQuery('brands', $UpdateBrand, "vendorid='".(int)$vendorId."'");
		}
	...
	private function EditVendor()
	{
        ...
        $GLOBALS['VendorPrefix']    = isc_html_escape($vendor['vendorprefix']);
	...
	private function DeleteVendors()
	{
        ...
        $query = $GLOBALS['ISC_CLASS_DB']->Query("SELECT brandid FROM [|PREFIX|]brands where vendorid in('".$vendorIds."') ");
        $brandid = array();
        ...
        $GLOBALS['ISC_CLASS_DB']->DeleteQuery('brands', "WHERE vendorid IN ('".$vendorIds."')"); # To delete vendorid in brands -- Baskaran
         $brandimp = implode("','",$brandid);
         # Update brandid to '0' and vendorid to '0' in products table -- Baskaran
         $GLOBALS['ISC_CLASS_DB']->UpdateQuery("products",array('prodvendorid'=>0),"prodvendorid in ('".$vendorIds."')");
         $GLOBALS['ISC_CLASS_DB']->UpdateQuery("products",array('prodbrandid'=>0),"prodbrandid in ('".$brandimp."')");
         $squery = $GLOBALS['ISC_CLASS_DB']->Query("SELECT seriesid FROM [|PREFIX|]brand_series where brandid in ('".$brandimp."')");
         $seriesid = array();
         while($srow = $GLOBALS['ISC_CLASS_DB']->Fetch($squery)){
             $seriesid[] = $srow['seriesid'];
         }
         $GLOBALS['ISC_CLASS_DB']->Query(sprintf("DELETE FROM [|PREFIX|]brand_series where brandid in ('%s')",$brandimp));
         if(count($seriesid) > 0){
             $seriesimp = implode("','",$seriesid);
             #Update seriesid to '0' in products table -- Baskaran
             $GLOBALS['ISC_CLASS_DB']->UpdateQuery("products",array('brandseriesid'=>0),"brandseriesid in ('".$seriesimp."')");
         }    
