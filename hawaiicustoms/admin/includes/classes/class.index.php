...
class ISC_ADMIN_INDEX
{
	...
	*/
	...
	private function SetupVersionCheck()
	{
		...
		else if(PRODUCT_VERSION == $latestVersion) {
			$this->template->Assign('HideDashboardVersionCheck', 'display: none');
		}
	 ...
	 */
	public function GenerateGettingStartedSteps()
	{
		...
		$query = "
			...
			LIMIT 1
		";
		$hasProducts = (bool)$this->db->FetchOne($query);
		...
		$query = "
			...
			LIMIT 1
		";
		$hasPages = (bool)$this->db->FetchOne($query);

		$gettingStartedSteps = array(
			array(
				'text' => GetLang('GettingStartedStep1'),
				'completed' => (bool)GetConfig('StoreName'),
			),
			array(
				'text' => GetLang('GettingStartedStep2'),
				'completed' => $hasProducts
			),
			array(
				'text' => GetLang('GettingStartedStep3'),
				'completed' => (GetConfig('CheckoutMethods') && GetConfig('NotificationMethods') && GetConfig('ShippingConfigured'))
			),
			array(
				'text' => GetLang('GettingStartedStep4'),
				'completed' => GetConfig('TaxConfigured'),
			),
			array(
				'text' => GetLang('GettingStartedStep5'),
				'completed' => GetConfig('CurrencyConfigured'),
			),
			array(
				'text' => GetLang('GettingStartedStep6'),
				'completed' => $hasPages
			),
		);
		...
		$hasIncomplete = false;
		$stepsList = '';
		// Loop through all of the getting started steps and build the HTML to display them
		foreach($gettingStartedSteps as $stepNumber => $step) {
			$this->template->Assign('Instruction', $step['text']);
			if($step['completed']) {
				$this->template->Assign('CompletedClass', 'GettingStartedItemComplete');
				$this->template->Assign('StepNumber', '&nbsp;');
			}
			else {
				$this->template->Assign('StepNumber', $stepNumber+1);
				$this->template->Assign('CompletedClass', '');
				$hasIncomplete = true;
			}
			$stepsList .= $this->template->GetSnippet('DashboardGettingStartedItem');
		}
	...
	public function LoadRecentOrders()
	{
		...
		{
			case 'pending':
				// added ORDER_STATUS_PAYPAL_PENDING in the below array to show it in dashboard - vikas
				$statusIn = array(
	...
	private function GetCurrentNotifications()
	{
		...
		// Select the number paypal pending orders - added by vikas
		$query = "
			SELECT COUNT(orderid)
			FROM [|PREFIX|]orders
			WHERE ordstatus=14
		";
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		$numPaypalPendingOrders = $GLOBALS['ISC_CLASS_DB']->FetchOne($result);
		if ($numPaypalPendingOrders > 0) {
			$langString = 'StoreNotificationPaypalPendingOrder';
			if ($numPendingReviews > 1) {
				$langString .= 'Multiple';
			}

			$notifications[] = sprintf(GetLang($langString), $numPaypalPendingOrders);
		}
	...
	public function GenerateOrderBreakdownGraph()
	{
		...
		//$graphFrom = mktime(0, 0, 0, date('m'), date('d')-6, date('y'));
		$maximumValue = 0;
		$graphValues = array();
...
//added by blessen to fix the diff b/n breakdown and other stat

$newstartDate = isc_gmmktime(0, 0, 0, isc_date("m",$graphFrom), isc_date("d",$graphFrom), isc_date("Y",$graphFrom));
$newendDate = isc_gmmktime(0, 0, 0, isc_date("m",$graphFrom), isc_date("d",$graphFrom)+ 1, isc_date("Y",$graphFrom));
$dateformat =  isc_date("Y-m-d",$newstartDate); 

for ($i = 1; $i <= 7; $i++) {

$newquery = "SELECT count(*) as numorders , SUM(ordtotalamount) as totalrevenue FROM [|PREFIX|]orders WHERE orddate >= ".$newstartDate." AND orddate < ".$newendDate." AND ordstatus IN (2, 3, 8, 9, 10, 11 ) ";

$newresult = $this->db->Query($newquery);
$newday = $this->db->Fetch($newresult);
$maximumValue = max($maximumValue, $newday['totalrevenue']);
$graphValues[$dateformat]['formatteddate'] = $dateformat;
$graphValues[$dateformat]['totalrevenue'] = $newday['totalrevenue'];
$graphValues[$dateformat]['numorders'] = $newday['numorders'];
$graphValues[$dateformat]['orddate'] = $newstartDate;

$newstartDate = isc_gmmktime(0, 0, 0, isc_date("m",$graphFrom), isc_date("d",$graphFrom)+ $i, isc_date("Y",$graphFrom));
$newendDate = isc_gmmktime(0, 0, 0, isc_date("m",$graphFrom), isc_date("d",$graphFrom)+ ($i+1), isc_date("Y",$graphFrom));
$dateformat =  isc_date("Y-m-d",$newstartDate); 
