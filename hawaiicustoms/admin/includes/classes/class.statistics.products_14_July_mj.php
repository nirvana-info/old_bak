		...
		if(isset($_POST['Calendar'])) {
			...
			if (isset($_POST['Calendar']['DateType'])) {
				...
				if(isset($_POST['Calendar']['From'])) {
					...
					$calendarInfo['From']['Yr'] = isc_date('Y', $datestamp);
					$calendarInfo['From']['Mth'] = isc_date('m', $datestamp);
					$calendarInfo['From']['Day'] = isc_date('d', $datestamp);
				}
				if(isset($_POST['Calendar']['To'])){
					$datestamp = strtotime($_POST['Calendar']['To']);
					$calendarInfo['To']['Yr'] = isc_date('Y', $datestamp);
					$calendarInfo['To']['Mth'] = isc_date('m', $datestamp);
					$calendarInfo['To']['Day'] = isc_date('d', $datestamp);
				}
			else {
	            ...
	            if($vendorSql=="")    {   
                    ...
                    $mainQuery = "
                                SELECT
                                    ...
                                    p.prodname,
                                    c.categoryid itemid,
				    CONCAT( rc.catname, ' - ', c.catname)  AS commonnamefield,
                                    SUM(ordprodqty) AS numsold,
                                    SUM(p.prodnumviews) AS prodnumviews,
                                    IF(SUM(p.prodnumratings) > 0, (SUM(p.prodratingtotal) / SUM(p.prodnumratings)), 0) AS avgrating,
                                    IF(SUM(p.prodnumviews) > 0, SUM(ordprodqty) / SUM(p.prodnumviews), 0) AS unitssoldpercent
                                FROM
                                    ...
                                    INNER JOIN [|PREFIX|]categoryassociations ca ON ( ca.productid = p.productid )
                                    LEFT JOIN [|PREFIX|]categories c ON ( c.categoryid = ca.categoryid ) 
				    LEFT JOIN [|PREFIX|]categories rc ON (rc.categoryid = c.catparentid)
                                    LEFT JOIN 
                                    (
                                    SELECT 
                                        SUM(ordprodqty) ordprodqty, ordprodid FROM isc_order_products
