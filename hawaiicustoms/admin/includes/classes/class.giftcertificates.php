...
class ISC_ADMIN_GIFTCERTIFICATES
{
	...
	private function ManageGiftCertificatesGrid(&$numGiftCertificates)
	{
		...
		if($numGiftCertificates > ISC_GIFTCERTIFICATES_PER_PAGE) {
			...
			$GLOBALS['Nav'] = "";
      $GLOBALS['CurrentPageLink'] = "index.php?ToDo=viewGiftCertificates";
		}
		...
		while ($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($certificateResult)) {
			...
			if($row['giftcertexpirydate'] != 0) {
				...
				$GLOBALS['GiftCertificateExpiryDate'] = GetLang('GiftCertificateExpireNA');
			}
...
public function ActivateGiftCertificates($orderId)
    ...
    public function RemoveAppliedGCFromOrder($gcId, $orderId) {
        $sql = sprintf("SELECT histgiftcertid,histbalanceused FROM [|PREFIX|]gift_certificate_history h WHERE histgiftcertid=%d AND h.historderid=%d", $gcId, $orderId);
               
        $gc = $GLOBALS['ISC_CLASS_DB']->Fetch($GLOBALS['ISC_CLASS_DB']->Query($sql));

        if ($gc && $gc['histbalanceused'] > 0) {
            $GLOBALS['ISC_CLASS_DB']->Query("UPDATE [|PREFIX|]gift_certificates SET giftcertbalance = giftcertbalance +{$gc['histbalanceused']} WHERE giftcertid={$gc['histgiftcertid']}");
        }
                
        $GLOBALS['ISC_CLASS_DB']->DeleteQuery('gift_certificate_history', "WHERE histgiftcertid=$gcId AND historderid=$orderId");
    }
