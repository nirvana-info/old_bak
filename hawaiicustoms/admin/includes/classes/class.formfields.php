<?php
	...
	class ISC_ADMIN_FORMFIELDS
	{
		...
		public function ManageFormFields($msgDesc='', $msgStatus='')
		{
			...
			if (!$GLOBALS["ISC_CLASS_ADMIN_AUTH"]->HasPermission(AUTH_Add_FormFields)) {
		...
		public function ManageFormFieldsGrid($formFieldFormId=FORMFIELDS_FORM_ACCOUNT)
		{
			...
			foreach (array_keys($fields) as $fieldId) {
				...
				if ($field->record['formfieldisimmutable']) {
					...
					$GLOBALS['DeleteStatus'] = 'disabled="1"';
				} else {
				...
				if (!$GLOBALS["ISC_CLASS_ADMIN_AUTH"]->HasPermission(AUTH_Edit_FormFields)) {
					$disabled = 'disabled="1" style="color: gray;"';
				...
				if (!$GLOBALS["ISC_CLASS_ADMIN_AUTH"]->HasPermission(AUTH_Edit_FormFields) || isc_strtolower($field->record['formfieldtype']) == 'selectortext') {
					$disabled = 'disabled="1" style="color: gray;"';
				...
				if (!$GLOBALS["ISC_CLASS_ADMIN_AUTH"]->HasPermission(AUTH_Delete_FormFields) || $field->record['formfieldisimmutable']) {
					$disabled = 'disabled="1" style="color: gray;"';
