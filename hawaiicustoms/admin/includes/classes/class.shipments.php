...
class ISC_ADMIN_SHIPMENTS
{
	 ...
	 */
	public function GeneratePackingSlip($orderId, $shipmentId=0)
	{
		$products = array();

		if($shipmentId != 0) {
			$shipmentDetails = $this->GetShipmentById($shipmentId);
			...
			$GLOBALS['PackingSlipTitle'] = sprintf(GetLang('PackingSlipTitleShipment'), $shipmentDetails['shipmentid']);
			$GLOBALS['PackingSlipComments'] = nl2br(isc_html_escape($shipmentDetails['shipcomments']));
			$GLOBALS['OrderId'] = $shipmentDetails['shiporderid'];
			$GLOBALS['OrderDate'] = CDate($shipmentDetails['shiporderdate']);
			$GLOBALS['ShippingMethod'] = isc_html_escape($shipmentDetails['shipmethod']);
			$GLOBALS['TrackingNo'] = isc_html_escape($shipmentDetails['shiptrackno']);
			$GLOBALS['DateShipped'] = CDate($shipmentDetails['shipdate']);
			$GLOBALS['Comments'] = nl2br(isc_html_escape($shipmentDetails['shipcomments']));
		...
		// Printing a packing slip for an entire order
			...
			$GLOBALS['PackingSlipTitle'] = sprintf(GetLang('PackingSlipTitleOrder'), $shipmentDetails['orderid']);
			$GLOBALS['PackingSlipComments'] = nl2br(isc_html_escape($shipmentDetails['ordcustmessage']));

			$GLOBALS['OrderId'] = $shipmentDetails['orderid'];
			$GLOBALS['OrderDate'] = CDate($shipmentDetails['orddate']);
			$GLOBALS['ShippingMethod'] = isc_html_escape($shipmentDetails['ordshipmethod']);
			$GLOBALS['TrackingNo'] = isc_html_escape($shipmentDetails['ordtrackingno']);
			$GLOBALS['Comments'] = nl2br(isc_html_escape($shipmentDetails['ordcustmessage']));

			$addressColumnPrefix = 'ord';
		}

		$GLOBALS['HideShippingMethod'] = "";
		$GLOBALS['HideTrackingNo'] = "";
		$GLOBALS['HideShippingDate'] = "";
		$GLOBALS['HideComments'] = "";

		if(!$GLOBALS['ShippingMethod']) {
			$GLOBALS['HideShippingMethod'] = 'display: none';
		}

		if(!$GLOBALS['TrackingNo']) {
			$GLOBALS['HideTrackingNo'] = 'display: none';
		}

		if(!isset($GLOBALS['DateShipped'])) {
			$GLOBALS['HideShippingDate'] = 'display: none';
		}
		...
		$query = "
				SELECT CONCAT(custconfirstname, ' ', custconlastname) AS ordcustname, custconemail AS ordcustemail, custconphone AS ordcustphone
				FROM [|PREFIX|]customers
				WHERE customerid = '".$GLOBALS['ISC_CLASS_DB']->Quote($shipmentDetails[$addressColumnPrefix.'custid'])."'
			";
		...
		if($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
				...
				if ($row['ordcustemail'] != '') {
					...
					$GLOBALS['CustomerEmail'] = '<span style="width: 55px; float:left;">' . GetLang('OrderInvoiceEmail') . ':</span> '. isc_html_escape($shipmentDetails['ordbillemail']);
				}

				if ($row['ordcustphone'] != '') {
					$GLOBALS['CustomerPhone'] = '<span style="width: 55px; float:left;">' . GetLang('OrderInvoicePhone') . ':</span> '. isc_html_escape($row['ordcustphone']);
				} else {
					$GLOBALS['CustomerPhone'] = '<span style="width: 55px; float:left;">' . GetLang('OrderInvoicePhone') . ':</span> '. isc_html_escape($shipmentDetails['ordbillphone']);
				}
		...
		$GLOBALS['ShippingAddress'] = ISC_ADMIN_ORDERS::BuildOrderAddressDetails($addressDetails, false);

		$GLOBALS['ISC_CLASS_ADMIN_ORDERS'] = GetClass('ISC_ADMIN_ORDERS');
		$fieldsArray = $GLOBALS['ISC_CLASS_ADMIN_ORDERS']->GetOrderProductFieldsData($orderId);

		// Build the list of products that are being shipped
		$GLOBALS['ProductsTable'] = "";
		foreach($products as $product) {
			if($product['prodcode']) {
				$GLOBALS['ProductSKU'] = isc_html_escape($product['prodcode']);
			}
			else {
				$GLOBALS['ProductSKU'] = GetLang('NA');
			}

			$GLOBALS['ProductName'] = isc_html_escape($product['prodname']);
			$GLOBALS['ProductQty'] = $product['prodqty'];
			...
			if($product['prodoptions'] != '') {
				...
				if(!empty($options)) {
					...
					foreach($options as $name => $value) {
						$GLOBALS['ProductOptions'] .= isc_html_escape($name).': '.isc_html_escape($value).', ';
					}
					$GLOBALS['ProductOptions'] = rtrim($GLOBALS['ProductOptions'], ', ');
			...
			$orderProdFields = '';
			$productFields = '';
			if(isset($fieldsArray[$product['prodordprodid']])) {
				$productFields = $GLOBALS['ISC_CLASS_ADMIN_ORDERS']->LoadOrderProductFieldRow($fieldsArray[$product['prodordprodid']], false);
			}
			if($productFields != '') {
				$GLOBALS['OrderProductFields'] = $productFields;
				$orderProdFields = $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet('OrderProductFields');
			}
			$GLOBALS['OrderProductFields'] = $orderProdFields;

			$GLOBALS['ProductsTable'] .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet('PackingSlipItem');
		}
		if(!$GLOBALS['ProductsTable']) {
			return false;
		}

		$GLOBALS['ISC_CLASS_TEMPLATE']->SetTemplate("order.packingslip");
	...
	private function PrintOrderPackingSlips()
	{
		...
		foreach($_POST['orders'] as $i => $orderId) {
			...
			$packingSlips .= $this->GeneratePackingSlip($orderId, 0);
		}
