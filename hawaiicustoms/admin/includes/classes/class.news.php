<?php
	...
	class ISC_ADMIN_NEWS
	{
		...
		private function _GetNewsList(&$Query, $Start, $SortField, $SortOrder, &$NumResults)
		{
			...
			$queryWhere = '';
			if($Query != '') {
				$queryWhere .= " WHERE LOWER(newstitle) LIKE '%".$GLOBALS['ISC_CLASS_DB']->Quote($Query)."%' OR LOWER(newscontent) LIKE '%".$GLOBALS['ISC_CLASS_DB']->Quote($Query)."%'";
			}
		...
		{
			if (isset($_POST['news'])) {
				$newsids = implode("','", $GLOBALS['ISC_CLASS_DB']->Quote($_POST['news']));
				$query = sprintf("delete from [|PREFIX|]news where newsid in ('%s')", $newsids);
