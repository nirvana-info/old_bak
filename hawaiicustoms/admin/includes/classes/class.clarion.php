...
class ISC_ADMIN_CLARION  
{
	...
	function fnLoadCustomerYMM($customerID, $type) { # $type=1 is Existing user, $type=0 is Anonymous user -- Baskaran
		$query = '';
		if($type == '1') {
			$query="SELECT * FROM isc_user_ymm where user_id='".$customerID."'";
		}
		else  {
			$query="SELECT * FROM isc_user_ymm where order_id='".$customerID."'";
		}
