<?php
    ...
    $GLOBALS['DiscountRules'] = GetDiscountRuleModuleInfo(); 
    ...
    {    
		...
		$DiscountInfoSub['discountname']  = $data['discountname']; // storing discount name                                      
	...
	// added new param $discountname as need to show clearance icon if clearance discount is applied.
    function CalculateDiscountPrice($price, $discountprice, $categoryid, $seriesid, &$discountpolicy=0, &$discountname='')   {
        ...
        foreach($Rules as $Rule)   {  
            if((float)$price >= (float)$Rule['minprice'])    {
                ...
                if($discountprice < $price)    {
					 $discountname = isset($Rule['discountname']) ? $Rule['discountname'] : '';
    ...
    function IsDiscountAvailable($type, $id, &$discountname='')   {
        //Added by Simha for onsale addition
        $Rules = $GLOBALS['DiscountRules'];
        foreach($Rules as $Rule)   {                           
