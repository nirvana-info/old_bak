<?php
	...
	*/
	...
	function GetStateInfoByName($stateName)
	{
		...
		$query = "
			SELECT *
			FROM [|PREFIX|]country_states
			WHERE LOWER(statename)='".$GLOBALS['ISC_CLASS_DB']->Quote($stateName)."'
		";
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
	...
	*/
	...
	function GetCountryInfoByName($countryName)
	{
		...
		$query = "
			SELECT *
			FROM [|PREFIX|]countries
			WHERE LOWER(countryname)='".$GLOBALS['ISC_CLASS_DB']->Quote($countryName)."'
		";
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
	...
	*/
	function GetStateListAsOptions($country, $selectedState = 0, $IncludeFirst = true, $FirstText = "ChooseAState", $FirstValue = "0", $FirstSelected = false, $useNamesAsValues = false)
	{
		...
		$list = "";
		$query = sprintf("select stateid, statename from [|PREFIX|]country_states where statecountry='%d' order by stateabbrv asc", $GLOBALS['ISC_CLASS_DB']->Quote($country));
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
