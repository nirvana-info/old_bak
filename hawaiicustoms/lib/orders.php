		...
		if (mysql_num_rows($result2) > 0 )
			{
				...
				{
					$sql = "update isc_orders set ordstatus = 11 where orderid = ".$realorderid." ";
					mysql_query($sql);
			...
			else
			{
				if ($updatedOrder['ordstatus'] == 12 ) 
				{
					...
					foreach ($order[0] as $key => $value) {
							...
							if ($key != "orderid" and $key != "products" and $key != "ordformsessionid" and $key != "transactionid")
							{
							 $name.= $key." , ";  
							  $values.= "'".$value."' , "; 
							}
					...
					$sql = "update isc_order_products set orderorderid =  ".mysql_insert_id()." where offerid = ".$orderId." ";
					mysql_query($sql);
...
//check order status call back 2011-4-20, ronnie add
		function checkUpdateOrderStatus($orderId, $status){
			
			$query = sprintf("SELECT ordstatus FROM [|PREFIX|]orders WHERE orderid=$orderId");
			...
			if($Orderstatus[$statusName]){
				$statusLevel=$Orderstatus[$statusName];
			}
			if($Orderstatus[$status]){
				$changeLevel=$Orderstatus[$status];
			}
			
			//var_dump($changeLevel>=$statusLevel);
			//exit;
			
			if($changeLevel>=$statusLevel){
				return true;
			}
			
			$_SESSION["canupdateorder"].=$orderId;
			//return true;
			return false;
		}
...
function UpdateOrderStatus($orderIds, $status, $email=true, $preventModuleUpdateCallback=false)
{
	...
	foreach($orderIds as $orderId) {
		...
		if(checkUpdateOrderStatus($orderId,$status)==false){
			Continue;
		}
		...
		if ($GLOBALS['ISC_CLASS_DB']->UpdateQuery("orders", $updatedOrder, "orderid='".$GLOBALS['ISC_CLASS_DB']->Quote($orderId)."'")) {
            ...
            if ($status == ORDER_STATUS_REFUNDED || $status == ORDER_STATUS_CANCELLED || $status == ORDER_STATUS_DECLINED) {
                // refunded cgc
                $sql = sprintf("SELECT histcgcid,histbalanceused FROM [|PREFIX|]company_gift_certificate_history h WHERE h.historderid=%d", $orderId);
                ...
                if ($cgc && $cgc['histbalanceused'] > 0) {
                    ...
                    $GLOBALS['ISC_CLASS_DB']->Query("UPDATE [|PREFIX|]company_gift_certificate_history SET histbalanceused = 0 WHERE histcgcid={$cgc['histcgcid']}");
                    $GLOBALS['ISC_CLASS_DB']->Query("UPDATE [|PREFIX|]orders SET ordcompanygiftcertificateamount = 0, ordgatewayamount = ordgatewayamount + {$order['ordcompanygiftcertificateamount']} WHERE orderid=$orderId");
                }
            }
...
function VerifyPendingOrder($pendingOrderToken)
{
	...
	// 2011-06-23 johnny modify
	if($orderData['paymentmethod'] == "giftcertificate") {
		$status = ORDER_STATUS_AWAITING_FULFILLMENT;
	}
    // This order was paid for entirely using a company gift certificate, it's automatically valid
	else if($orderData['paymentmethod'] == "companygiftcertificate") {
		$status = ORDER_STATUS_AWAITING_FULFILLMENT;
	}
...
function CompletePendingOrder($pendingOrderToken, $status, $sendInvoice=true)
{
    ...
    $processedCompanyGiftCertificates = false;
	$orderStoreCredit = 0;
	$orderTotalAmount = 0;
	...
	foreach($orderData['orders'] as $order) {
        ...
        // If one or more gift certificates were used we need to apply them to this order and subtract the total
		if($order['ordcompanygiftcertificateamount'] > 0 && isset($extraInfo['companygiftcertificates']) && !empty($extraInfo['companygiftcertificates']) && $processedCompanyGiftCertificates == false) {
			$usedCompanyCertificates = array();
			$GLOBALS['ISC_CLASS_COMPANY_GIFT_CERTIFICATES'] = GetClass('ISC_COMPANYGIFTCERTIFICATES');
			$GLOBALS['ISC_CLASS_COMPANY_GIFT_CERTIFICATES']->ApplyCompanyGiftCertificatesToOrder($order['orderid'], $orderTotalAmount, $extraInfo['companygiftcertificates'], $usedCompanyCertificates);
			unset($extraInfo['companygiftcertificates']);
			$processedCompanyGiftCertificates = true;
		}
...
function LoadPendingOrdersByToken($token='', $hardRefresh=false)
{
	...
	// Add 'companygiftcertificateamount' by NI_20100826_Jack
	$pendingArray = array(
		'orders'				=> array(),
		'total'					=> 0,
		'gatewayamount'			=> 0,
		...
		'companygiftcertificateamount'	=> 0,
		'status'				=> 0,
		'currencyid'			=> 0,
		'customerid'			=> 0,
		'isdigital'				=> 0,
	...
	$query = "
		SELECT r.*,c.isguest
		FROM [|PREFIX|]orders r LEFT JOIN [|PREFIX|]customers c ON r.ordcustid=customerid
		WHERE r.ordtoken='".$GLOBALS['ISC_CLASS_DB']->Quote($token)."'
	";
	$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
	while($order = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
        ...
        $pendingArray['isguest'] = $order['isguest'];
	}
 ...
 * @return array An array of information about the pending orders.
 */
function LoadPendingOffersByToken($token='', $hardRefresh=false)
{
    static $pendingCache = array();
    if($token == '' && isset($_COOKIE['SHOP_ORDER_TOKEN'])) {
        $token = $_COOKIE['SHOP_ORDER_TOKEN'];
 ...
 * @return integer
 **/
function CompanyGiftCertificateSort($a, $b)
{
	if ($a['cgcbalance'] == $b['cgcbalance']) {
		return 0;
	}
	if ($a['cgcbalance'] < $b['cgcbalance']) {
		return -1;
