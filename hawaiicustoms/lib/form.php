...
class ISC_FORM
{
		...
		if (is_array($fieldData) && !empty($fieldData)) {
			 ...
			 */
				...
				if (is_array($sessdata) && array_key_exists($fieldData['formfieldid'], $sessdata)) {
					$field->setValue($sessdata[$formFieldId], true);
				}
			}
	...
	public function deleteFormField($formId, $formFieldId, $clearSessionData=true)
	{
		...
		if ($GLOBALS['ISC_CLASS_DB']->DeleteQuery('formfields', 'WHERE formfieldid = ' . (int)$formFieldId) === false) {
			return false;
		}
	 ...
	 *
	 * @access public
	 * @param int $typeId The address type (billing formID or shipping)
