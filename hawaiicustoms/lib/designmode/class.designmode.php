...
class DesignMode
{
   ...
   */
	protected function SaveFile()
	{
		...
		$MasterFilePath = $this->MasterBasePath . $this->FileName;

		// File doesn't exist in the local template and in the master template. Exit
		if(!file_exists($FilePath) && !file_exists($MasterFilePath)) {
		...
		if(!file_exists($FilePath)) {
			...
			if(!is_dir($parentDir) && !@mkdir($parentDir, ISC_WRITEABLE_DIR_PERM, true)) {
				$this->SetError('Unable to create directory for '.$FilePath);
			}
			if(!@touch($FilePath)) {
	...
	{
		$FilePath = $this->BasePath . $this->FileName;
		$MasterPath = $this->MasterBasePath.$this->FileName;
   ...
   */
	protected function EditFile()
	{
		...
		if(!array_key_exists('Ajax', $_REQUEST)) {
			...
			foreach($this->DirList as $FileType=>$FileDir) {
				...
				foreach(array($this->BasePath.$FileDir, $this->MasterBasePath.$FileDir) as $directory) {
					if(!is_dir($directory)) {
						continue;
					}
				...
				foreach ($FileList as $file) {
					...
					if ($file == "CVS" || $file == ".svn" || isc_substr($file, 0, 1) == "." || isc_strpos($file, ".bak") !== false  || isc_strpos($file, ".php") !== false) {
						continue;
					}
					...
					$GLOBALS[$FileType."FileList"] .= sprintf("<li><a href=\"#\" onclick=\"DesignModeEditor.load_file('%s'); this.blur(); return false;\" class=\"%s %s\">%s</a></li>", trim($FileDir."/".$file, '/'), $FileId, $class, $file);
				}
			}
		}
		...
		$backupSupport = "false";
		if(file_exists($this->MasterBasePath.$this->FileName) && file_exists($this->BasePath.$this->FileName)) {
			$backupSupport = "true";
		}
	...
	protected function MakeFileList($type)
	{
		...
		if ($type == 'snippets') {
			...
			$MasterSnippetDir = $this->MasterBasePath . $this->DirList['Snippet'].'/';
			...
			foreach ($matches[0] as $key => $val) {
				...
				if (file_exists($SnippetDir . $pattern2 . '.html') || file_exists($MasterSnippetDir . $pattern2 . '.html')) {
			...
			$MasterPanelDir = $this->MasterBasePath . $this->DirList['Panel'].'/';
			...
			foreach ($matches[0] as $key => $k) {
				...
				if (file_exists($PanelDir . $pattern2 . '.html') || file_exists($MasterPanelDir . $pattern2 . '.html')) {
   ...
   */
	protected function UpdateLayoutPanels()
	{
		...
		$MasterLayoutFilePath = $this->MasterBasePath . $LayoutFile;

		// File doesn't exist in the local template and in the master template. Exit
		if(!file_exists($MasterLayoutFilePath) && !file_exists($LayoutFilePath)) {
			return false;
		}
