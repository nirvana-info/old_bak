...
class ISC_ENTITY_ORDER extends ISC_ENTITY_BASE
{
	...
	private function parsedata(&$input, &$parsed)
	{
		...
		// Get the amount used from any gift certificates
		if (array_key_exists('companygiftcertificateamount', $input)) {
			$orderCompanyGiftCertificateAmount = $input['companygiftcertificateamount'];
		}
		else {
			$orderCompanyGiftCertificateAmount = 0;
		}
		$providerName = '';
		$providerId = '';

		// Order was paid for entirely with gift certificates
		// 2001-06-23 johnny modify
		if ($input['paymentmethod'] == "giftcertificate") {
			$providerName = "giftcertificate";
			$providerid = '';
		}
        // Add by NI_20100827_Jack
        // Order was paid for entirely with company gift certificates
		else if ($input['paymentmethod'] == "companygiftcertificate") {
			$providerName = "companygiftcertificate";
			$providerid = '';
		}
		...
		$onlyCompanyGiftCertificates = true;
		foreach ($input['products'] as $product) {
			if (isset($product['type']) && $product['type'] != 'companygiftcertificate') {
				$onlyCompanyGiftCertificates = false;
			}
		}
        ...
        if (array_key_exists('companygiftcertificates', $input) && is_array($input['companygiftcertificates'])) {
			$input['extraInfo']['companygiftcertificates'] = $input['companygiftcertificates'];
		}
		...
		$parsed	= array(
			...
			'ordcompanygiftcertificateamount' => $orderCompanyGiftCertificateAmount, // Add by NI_20100826_Jack
            ...
            'ordonlycompanygiftcerts' => (int) $onlyCompanyGiftCertificates,// Add by NI_20100827_Jack
			...
			'orderowner' => isset($input['orderowner']) ? $input['orderowner'] : 0,
		);
		...
		foreach ($input['products'] as $product) {
            ...
            */
			...
			if(is_array($existingProduct)) {
                ...
                /* Code Ends */
				/* Added the below code to check the YMM for the customer and accordingly storing it. - starts - #vikas */
				if( $product['year'] != "" || $product['make'] != "" || $product['model'] != "" )
				{
					$ymm_qry = " select id from [|PREFIX|]user_ymm where user_id = ".$input['customerid'];

					if(isset($product['year']) && $product['year'] != '')
						$ymm_qry .= " and year = '".$product['year']."' ";
					else
						$ymm_qry .= " and ( year = '' OR year is NULL ) ";

					if(isset($product['make']) && $product['make'] != '')
						$ymm_qry .= " and make = '".$product['make']."' ";
					else
						$ymm_qry .= " and ( make = '' OR make is NULL ) ";

					if(isset($product['model']) && $product['model'] != '')
						$ymm_qry .= " and model = '".$product['model']."' ";
					else
						$ymm_qry .= " and ( model = '' OR model is NULL ) ";

					if(isset($product['bedsize']) && $product['bedsize'] != '')
						$ymm_qry .= " and bed_size = '".$product['bedsize']."' ";
					else
						$ymm_qry .= " and ( bed_size = '' OR bed_size is NULL ) ";

					if(isset($product['cabsize']) && $product['cabsize'] != '')
						$ymm_qry .= " and cab_size = '".$product['cabsize']."' ";
					else
						$ymm_qry .= " and ( cab_size = '' OR cab_size is NULL ) ";

					$ymm_res = $GLOBALS['ISC_CLASS_DB']->Query($ymm_qry);

					if( $ymm_res && $GLOBALS['ISC_CLASS_DB']->CountResult($ymm_res) > 0 ) // update the order table with the YMMid
					{
						$ord_ymm = array();
						$ymm_id = $GLOBALS['ISC_CLASS_DB']->FetchOne($ymm_res);
						$ord_ymm['orderymmid'] = $ymm_id;
						$GLOBALS['ISC_CLASS_DB']->UpdateQuery('order_products', $ord_ymm, "orderprodid='".(int)$ordProdID."'");
					}
					else // Insert the new row in user_ymm table
					{
						// This line is adding by Mayank Jaitly
						// For Vikas : Sir G loop chal raha hai, orderymmid nahi replace ho raha hai...
						$ord_ymm=array();
						///////////////////////////////////////////////////////////////////////////////////////////
						
						if(isset($product['year']))
							$ord_ymm['year']  = $product['year'];
						if(isset($product['make']))
							$ord_ymm['make']  = $product['make'];
						if(isset($product['model']))
							$ord_ymm['model'] = $product['model'];
						if(isset($product['bedsize']))
							$ord_ymm['bed_size']  = $product['bedsize'];
						if(isset($product['cabsize']))
							$ord_ymm['cab_size'] = $product['cabsize'];

						$ord_ymm['user_id'] = $input['customerid'];

						if($input['customerid'] == 0) // for anonymous user in admin section
						{
							$ord_ymm['order_id'] = $input['orderid'];
						}
						...
						$usrYMMID = $GLOBALS['ISC_CLASS_DB']->InsertQuery("user_ymm", $ord_ymm);
						$usrYMM['orderymmid'] = $usrYMMID;
						$GLOBALS['ISC_CLASS_DB']->UpdateQuery('order_products', $usrYMM, "orderprodid='".(int)$ordProdID."'");
					}
				}
				/* Code Ends */
            }
...
public function offeradd($input)
	{
		...
		# Commented for not to insert record into the Orders table. -- Baskaran 
		/* if (!isId($id = $GLOBALS['ISC_CLASS_DB']->InsertQuery("orders", $parsed))) { 
		$GLOBALS['ISC_CLASS_DB']->RollbackTransaction();
			return false;
		} */

		$parsed['ordstatus'] = 0; // 11 changed to 0 -- Baskaran
		...
		$input['orderid']	= -1;  // if this value is 0 , confirm page will nor submit.
		$input['date']		= time();
		...
		if (!$this->addProducts($input)) {
            ...
            $updatecost = array("ordprodcost" => $_SESSION['the_offered_price']);
            $GLOBALS['ISC_CLASS_DB']->UpdateQuery('order_products', $updatecost, "offerid='".(int)$id."'");
        }
