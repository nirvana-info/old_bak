	...
	class CHECKOUT_USAEPAY extends ISC_GENERIC_CREDITCARD
	{
		...
		*/
		...
		public function ProcessPaymentForm()
		{
			...
			$this->_handleResponse($tran->process(), $tran);
		}
		...
		protected function _ConstructPostData($postData)
		{
			...
			if (isc_strtolower($this->_testmode) == "yes") {
				$tran->testmode=true;
			}
			else {
				$tran->testmode=false;
			...
			$tran->description	= "Order from " . $GLOBALS['StoreName'];
		...
		protected function _HandleResponse($result, $tran)
		{
			if ($result) {
				...
				$GLOBALS['ISC_CLASS_LOG']->LogSystemError(array('payment', $this->GetName()), sprintf(GetLang($this->_languagePrefix.'Failure'),  $tran->result, $tran->error.$tran->curlerror));
				$this->SetError(GetLang($this->_languagePrefix.'SomethingWentWrong').sprintf(" %s : %s %s", $tran->result, $tran->error, $tran->curlerror));
				return false;
			}
		}
	}
