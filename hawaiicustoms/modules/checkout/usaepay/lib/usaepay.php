...
//	v1.5.5 - August 28, 2007
//
// 	Copyright (c) 2001-2007 USAePay
//	Written by Tim McEwen (tim@usaepay.com)
//

define("USAEPAY_VERSION", "1.5.5");
...
class umTransaction {
	...
	function Process()
	{
		...
		$data = "UMkey=" . rawurlencode($this->key) . "&" . 
			...
			"UMchecknum=" . rawurlencode($this->checknum) . "&" .
			"UMrecurring=" . rawurlencode($this->recurring) . "&" .
			"UMbillamount=" . rawurlencode($this->billamount) . "&" .
			"UMbilltax=" . rawurlencode($this->billtax) . "&" .
			...
			($this->billsourcekey?"UMbillsourcekey=yes&":"") .
			...
			"UMcontactless=" . rawurlencode($this->contactless) . "&" . 
			"UMsoftware=" . rawurlencode($this->software) . "&" . 
			"UMignoreDuplicate=" . rawurlencode($this->ignoreduplicate) . "&" . 
