...
//           ||$ship->type == "shipping-company-calculated-shipping"
          ...
          if (isset($shipping_restrictions)) {
            ...
            if($shipping_restrictions->allowed_restrictions) {
              ...
              foreach(array_unique($shipping_restrictions->allowed_state_areas_arr) as $current) {
                $xml_data->Push('us-state-area');
                $xml_data->Element('state', $current);
                $xml_data->Pop('us-state-area');
              }
              foreach(array_unique($shipping_restrictions->allowed_zip_patterns_arr) as $current) {
              ...
              $shipping_restrictions->allowed_country_codes_arr = array_unique($shipping_restrictions->allowed_country_codes_arr);
              sort($shipping_restrictions->allowed_country_codes_arr);
            ...
            if($shipping_restrictions->excluded_restrictions) {
              ...
              foreach(array_unique($shipping_restrictions->excluded_state_areas_arr) as $current) {
                $xml_data->Push('us-state-area');
                $xml_data->Element('state', $current);
                $xml_data->Pop('us-state-area');
              }
              foreach(array_unique($shipping_restrictions->excluded_zip_patterns_arr) as $current) {
                $xml_data->Push('us-zip-area');
                $xml_data->Element('zip-pattern', $current);
                $xml_data->Pop('us-zip-area');
              }
              $shipping_restrictions->excluded_country_codes_arr = array_unique($shipping_restrictions->excluded_country_codes_arr);
			  sort($shipping_restrictions->excluded_country_codes_arr);
          ...
          if ($ship->type == "merchant-calculated-shipping") {
            ...
            if (isset($address_filters)) {
              ...
              if($address_filters->allowed_restrictions) {
                ...
                foreach(array_unique($address_filters->allowed_state_areas_arr) as $current) {
                  $xml_data->Push('us-state-area');
                  $xml_data->Element('state', $current);
                  $xml_data->Pop('us-state-area');
                }
                foreach(array_unique($address_filters->allowed_zip_patterns_arr) as $current) {
                  $xml_data->Push('us-zip-area');
                  $xml_data->Element('zip-pattern', $current);
                  $xml_data->Pop('us-zip-area');
                }
                ...
                $address_filters->allowed_country_codes_arr = array_unique($address_filters->allowed_country_codes_arr);
                sort($address_filters->allowed_country_codes_arr);
              ...
              if($address_filters->excluded_restrictions) {
                ...
                foreach(array_unique($address_filters->excluded_state_areas_arr) as $current) {
                  $xml_data->Push('us-state-area');
                  $xml_data->Element('state', $current);
                  $xml_data->Pop('us-state-area');
                }
                foreach(array_unique($address_filters->excluded_zip_patterns_arr) as $current) {
                  $xml_data->Push('us-zip-area');
                  $xml_data->Element('zip-pattern', $current);
                  $xml_data->Pop('us-zip-area');
                }
				$address_filters->excluded_country_codes_arr = array_unique($address_filters->excluded_country_codes_arr);
				sort($address_filters->excluded_country_codes_arr);
...
mysql_query("INSERT INTO isc_orderlogs(ordervalue) VALUES ( 'get : ".print_r($xml_data->GetXML(),true)."')");
...
// Due to the design in the cart page, I have disable this code , Blessen
//        if($this->googleAnalytics_id) {
//            $data .= "<!-- Start Google analytics -->
//            <script src=\"https://ssl.google-analytics.com/urchin.js\" type=\"".
//                "text/javascript\">
