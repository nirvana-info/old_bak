var Checkout = {
	...
	{
		if(skip){
			$('input[type=checkbox]:checked', '#ChooseAddonProducts').each(function(){
				$(this).attr('checked', false)
				$(this).parent().find('select').hide();
			});
			$('form', '#ChooseAddonProducts').submit();
		}else{
		var checkedProds = $('input[type=checkbox]:checked', '#ChooseAddonProducts').size();
		if(checkedProds > 0){
			$('form', '#ChooseAddonProducts').submit();
		}else{
