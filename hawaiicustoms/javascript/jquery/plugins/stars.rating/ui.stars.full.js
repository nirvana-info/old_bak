 ...
 *	
 * Histories:
 * 	lguan_20100715: Modify the original version, so that we can allow to show Cancel button behind the stars
 *
 */
;
(function($) {
			...
			.widget(
					"ui.stars",
					{
						init : function() {
							...
							this.$stars = this.$rboxs
									.map(function(i) {
										...
										o.id2title[i] = (o.isSelect ? this.text
												: this.title)
												|| this.value;

										if (o.selected == i
												|| (o.selected == -1 && (o.isSelect ? this.defaultSelected
														: this.defaultChecked))) {
											o.checked = i;
											o.value = o.id2val[i];
							...
							// Stars interface
							// lguan_20100715: By the setting, we will show the
							// stars and cancel accordingly.
							if (o.cancelAfterStars) {
							...
							if (o.checked === undefined) {
								...
								if (o.cancelShow) {
									this._disableCancel();
									//lguan_20100715: If we need to show cancel selected at initial load, do it
									if (o.cancelInitialSelected) self._selectCancel();
								}
							} else {
							...
							function fillTo(index, hover) {
								if (index != -1) {
									...
									self.$stars.eq(index).nextAll(
											...
											[ o.starHoverClass, o.starOnClass ]
													.join(' '));
							...
							this.$cancel
									.bind(
											"click.stars",
											function() {
												if (!o.forceSelect && !o.cancelSelectable
												...
												//lguan_20100715: If it's allow to select cancel, we should set cancel button
												if (o.cancelSelectable) self._selectCancel();
						...
						select : function(val) {
							var o = this.options;
							o.forceSelect = true;
							if (val == o.cancelValue)
								this.$cancel.triggerHandler("click.stars");
							else
								this.$stars.eq(o.val2id[val]).triggerHandler(
										"click.stars");
							o.forceSelect = false;
						},
						...
						_disableCancel : function() {
							var o = this.options, disabled = o.disabled
									...
									|| (o.value == o.cancelValue);
							if (disabled) {
								// lguan_20100715: If cancel can be select, it will never be disabled
								if (o.cancelSelectable) {
									this.$cancel.removeClass(o.cancelHoverClass)
										.addClass(o.cancelDisabledClass);
									disabled = false;
								}
								else {
									this.$cancel.removeClass(o.cancelHoverClass).removeClass(o.cancelOnClass)
										.addClass(o.cancelDisabledClass);
								}
							} else {
								this.$cancel.removeClass(o.cancelDisabledClass).removeClass(o.cancelOnClass);
							}
	...
	//lguan_20100715: Add an option to indicate whether the cancel button should be append after stars or before
	$.ui.stars.defaults = {
		...
		showTitles : false,
		captionEl : null,
		cancelAfterStars : false,
		cancelSelectable: false,
		cancelInitialSelected: false,
		...
		cancelOnClass : 'ui-stars-cancel-on'
	};
