		...
		<div class="span18 last">
			<div class="path">
				<ul>
					...
					<li class="last">New Items</li>
				</ul>
			</div>
			<form id="productForm" action="${base}${(productCategory.path)!"/product/new_item.jhtml"}" method="get">
				...
				[#if productCategory?? && (productCategory.children?has_content || productCategory.brands?has_content || productCategory.attributes?has_content)]
					<div id="filter" class="filter hidden">
						<div class="title hidden">${message("shop.product.filter")}</div>
						...
						<div id="moreFilter" class="moreFilter hidden">
							[#if rows > 3]
					...
					[#if page.content?has_content]
						<ul>
							[#list page.content?chunk(4) as row]
								[#list row as product]
									[#if product_index== 3 && row_has_next]
										<li class="last_down">
											<a href="${base}${product.path}">
												...
												<span>${message("shop.product.name")}:</span>
												<span title="${product.name}">${abbreviate(product.name, 60)}</span>
												<span>#${product.sn}</span>
												<span>${message("shop.cart.price")} ${currency(product.openPrice, true)}[#if product.openPrice!=product.openHighPrice]~${currency(product.openHighPrice, true)}[/#if]</span>
												<span>${message("Product.leadTime")} ${product.leadTime} day(s)</span>
											</a>
										</li>
									[#elseif product_index== 3 && !row_has_next]
										<li class="last_none">
											<a href="${base}${product.path}">
												<img src="${base}/upload/image/blank.gif" width="170" height="170" data-original="[#if product.thumbnail??]${product.thumbnail}[#else]${setting.defaultThumbnailProductImage}[/#if]" />
												<span>${message("shop.product.name")}:</span>
