...
function showSplitInfo(orderId, orderItemId){
	$.layer({
        type: 2,
        title: [
            '${message("shop.order.splitInfo")}', 
            'height:40px; color:#fff; border:none;' //customer config
        ], 
        border:[1],
        area: ['65%', '70%'],
        fadeIn: 300,
        iframe: {src: 'splitInfo.jhtml?orderId='+orderId+'&orderItemId='+orderItemId},
			...
			<div class="input order">
				...
				<table class="info">
					...
					<tr>
						...
						<td>
							<a href="combineOrderList.jhtml?modifyDate=${order.modifyDate}&sn=${order.sn}&combineHisId=${order.combineHisId}" class="current">View Detail</a>
						</td>
					</tr>
					[/#if]
				</table>
				...
				<table class="orderItem">
					...
					[#list order.orderItems as orderItem]
						<tr>
							...
							<td>${orderItem.product.weight}${orderItem.product.unit}</td>
							<td>${orderItem.product.volume}</td>
							<td>
								[#if !orderItem.isGift]
							...
							<td>
								<a href="javascript:;" onclick="showSplitInfo(${order.id},${orderItem.id})">[${message("shop.member.handle.view")}]</a>
							</td>
						</tr>
					[/#list]
				</table>
