...
public class Product extends BaseEntity {
	...
	/** 商品阶梯定价 */
	private List<ProductGradientPrice> gradientLists = new ArrayList<ProductGradientPrice>();
	...
	public BigDecimal getOpenPrice() {
		BigDecimal bd = this.price;
		for(ProductGradientPrice gradientPrice: this.gradientLists){
			if(gradientPrice.getPrice().compareTo(bd)<0){
				bd=gradientPrice.getPrice();
			}
		}
		return bd;
	}
	 ...
	 */
	@Transient
	public BigDecimal getOpenHighPrice() {
		BigDecimal bd = this.price;
		for(ProductGradientPrice gradientPrice: this.gradientLists){
			if(gradientPrice.getPrice().compareTo(bd)>0){
				bd=gradientPrice.getPrice();
	...
	public int getLowQuantity() {
		int low = 9999999;
		if(this.gradientLists.size()>0){
			for(ProductGradientPrice gradientPrice: this.gradientLists){
				if(gradientPrice.getStartQuantity().compareTo(low)<0){
					low=gradientPrice.getStartQuantity();
				}
			}
		}else{
			low = 1;
	...
	@Valid
	@ElementCollection
	@CollectionTable(name = "xx_gradient_price")
	public List<ProductGradientPrice> getGradientLists() {
		return gradientLists;
	}
	 ...
	 * @param gradientLists
	 */
	public void setGradientLists(List<ProductGradientPrice> gradientLists) {
		this.gradientLists = gradientLists;
	}
	 ...
	 */
	@Transient
	public int getRatifyNum() {
		int ratifyNum = 0;
		for(OrderItem item: orderItems){
			if(item.getOrder().getOrderStatus().name().equals("confirmed") || item.getOrder().getOrderStatus().name().equals("completed")){
				ratifyNum += item.getQuantity();
			}
		}
		return ratifyNum;
	}
	 ...
	 * 获取提交未combain的商品总数
	 * 
	 * @return 未combain的商品总数
	 */
	@Transient
	public int getNoCombain() {
		int ratifyNum = 0;
		for(OrderItem item: orderItems){
			if(item.getOrder().getOrderStatus().name().equals("confirmed") && item.getOrder().getCombine()){
				if(item.getOrder().getCombineHisId() != null && item.getOrder().getCombineHisId()>0){
					ratifyNum += item.getQuantity();
				}
			}
		} 
