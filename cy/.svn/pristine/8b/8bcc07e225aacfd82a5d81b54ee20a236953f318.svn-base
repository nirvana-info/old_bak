...
//千分位加逗号
function interceptPrice(price){
	var price1 = price.split(".")[0];//整数部分
	var price2 = price.split(".")[1];//小数部分
	var endPrice = "";//最终结果
	var len = price1.length;//整数部分长度
	var ys = len%3;//整数部分对3取余的长度
	if(ys != 0){
		endPrice = ","+price1.substr(0,ys);
	}
	price1 = price1.substr(ys);//此部分正好被3整除
	for(var i=0;i<parseInt(len/3);i++){
		endPrice += ","+price1.substr(i*3,3);
	}
	endPrice = endPrice+"."+price2;
	endPrice = endPrice.substr(1);
	return endPrice;
}
