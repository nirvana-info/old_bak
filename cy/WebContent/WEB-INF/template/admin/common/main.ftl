...
$().ready(function() {
	...
	//jackie.liu code
	$menu.eq(0).removeClass("default");
	$($nav.eq(0).attr("href")).addClass("default");
	//IF the first menu name is order
	if($nav.eq(0).attr("href")=="#order"){
		$(".home").hide();
		$nav.eq(0).trigger("click");
		$($nav.eq(0).attr("href")).find("dd:eq(0) a").trigger("click");
		$("#iframe").attr("src", $($nav.eq(0).attr("href")).find("dd:eq(0) a").attr("href"));
	}
	//end jackie.liu code
				...
				<div id="nav" class="nav">
					<ul>
						[#list ["admin:product", "admin:productCategory", "admin:parameterGroup", "admin:attribute", "admin:specification", "admin:brand", "admin:productNotify"] as permission]
							...
							[/@shiro.hasPermission]
						[/#list]
						[#list ["admin:order", "admin:specialOrder", "admin:payment", "admin:refunds", "admin:shipping", "admin:returns", "admin:deliveryCenter", "admin:deliveryTemplate"] as permission]
							[@shiro.hasPermission name = permission]
								<li>
						...
						[#list ["admin:statistics", "admin:sales", "admin:salesRanking", "admin:purchaseRanking", "admin:deposit"] as permission]
							...
							[/@shiro.hasPermission]
						[/#list]
						[#list ["admin:setting", "admin:area", "admin:paymentMethod", "admin:shippingMethod", "admin:deliveryCorp", "admin:paymentPlugin", "admin:storagePlugin", "admin:admin", "admin:adminMember", "admin:role", "admin:dict", "admin:message", "admin:log"] as permission]
							[@shiro.hasPermission name = permission]
								<li>
						...
						[/#list]
						<li class="home">
							<a href="${base}/admin/common/main.jhtml">${message("admin.main.home")}</a>
						</li>
					</ul>
				</div>
				<div class="link">
			...
			<td id="menu" class="menu">
				...
				<dl id="order">
					...
					[@shiro.hasPermission name="admin:order"]
                        ...
                        <dd>
                            <a href="../order/combineList.jhtml" target="iframe">${message("admin.main.combine")}</a>
                        </dd>
					[/@shiro.hasPermission]
					[@shiro.hasPermission name="admin:specialOrder"]
						<dd>
							<a href="../special_order/list.jhtml" target="iframe">${message("admin.specialOrder")}</a>
						</dd>
					[/@shiro.hasPermission]
				...
				<dl id="system">
					...
					[/@shiro.hasPermission]
					<!-- jackie.liu -->
					[@shiro.hasPermission name="admin:adminMember"]
						<dd>
							<a href="../admin_member/list.jhtml" target="iframe">${message("admin.admin.member")}</a>
						</dd>
					[/@shiro.hasPermission]
					...
					[@shiro.hasPermission name="admin:dict"]
						<dd>
							<a href="../dict/list.jhtml" target="iframe">${message("admin.dict.dict")}</a>
						</dd>
					[/@shiro.hasPermission]
