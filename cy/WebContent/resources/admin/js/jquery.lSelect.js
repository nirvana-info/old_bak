		...
		lSelect: function(options) {
			...
			return this.each(function() {
				...
				function addSelect($position, parentId, currentId) {
					if(typeof($position.attr("selectId"))=="undefined"){
						$position.nextAll("select[name=" + selectName + "]").remove();
					}else{
						$position.nextAll("select[selectId=" + $position.attr("selectId") + "]").remove();
					}
					...
					if(typeof($position.attr("selectId"))!="undefined"){
						select = '<select selectId="'+$position.attr("selectId")+'" name="' + selectName + '">';
					}
					...
					return $(select).css(settings.cssStyle).insertAfter($position).bind("change", function() {
						...
						if ($this.val() == "") {
							var $prev;
							if(typeof($position.attr("selectId"))=="undefined"){
								$prev = $this.prev("select[name=" + selectName + "]");
							}else{
								$prev = $this.prev("select[selectId=" + $position.attr("selectId") + "]");
							}
							if ($prev.size() > 0) {
								$input.val($prev.val());
							} else {
