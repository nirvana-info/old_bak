...
public class MemberDaoImpl extends BaseDaoImpl<Member, Long> implements MemberDao {
	...
	public List<Member> findList(Long adminId){
		String jpql = "select members from Member members where members.id not in(select am.member.id from AdminMember am where am.admin.id<>:adminId)";
		return entityManager.createQuery(jpql, Member.class).setFlushMode(FlushModeType.COMMIT).setParameter("adminId", adminId).getResultList();
	}
