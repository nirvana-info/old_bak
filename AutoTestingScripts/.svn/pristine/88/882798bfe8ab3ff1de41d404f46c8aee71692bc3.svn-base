...
namespace SystemTest
{
    //有捆包
    [TestFixture]
    public class Check1_03_thirdorderB : TestBase
    {
        [Test]
        public void chech1_03_01_login()
        {
            ie.GoTo(url);
        ...
        [Test]
        public void chech1_03_02_searchsource()
        {
            //search
            ie.Span(Find.ByText("资源查询")).Click();
            ie.Link(Find.ByText(" 清空购物车")).WaitUntilExists(120);
            Assert.IsTrue(ie.ContainsText("清空购物车"));
            ie.TextField(Find.ById("ctl00_ContentPlaceHolder1_txtMaterial")).TypeText("功能测试专用2");
