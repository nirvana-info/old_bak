...
@interface FavCompanyViewController ()

@end

@implementation FavCompanyViewController
@synthesize parentVC;
...
- (void)loadView
{
    ...
    [_favDataController getMyFollowStores:user];
...
-(NSMutableArray *)getTestingList{
    ...
    m.mname = @"company test...";
    m.photoUrl = @"http://...";
    m.gender = 1;
    m.distance = 100;
    m.comments = @"this is Company test";
    m.company = @"BaoStell";
...
-(void)return_follow_stores:(NSDictionary *)obj{
    ...
    if (obj != nil) {
        for (NSDictionary *item in obj) {
            MemberInfo *m = [[MemberInfo alloc] init];
            m.mname = [item objectForKey:@"name"];
//            m.phone = [item objectForKey:@"name"];
//            m.object_id = [item objectForKey:@"object_id"];
//            m.steels = [item objectForKey:@"steels"];
//            m.score = [item objectForKey:@"score"];
            m.photoUrl = @"http://...";
            m.gender = 1;
            m.distance = 100;
            m.comments = @"this is Company test";
